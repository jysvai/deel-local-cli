...
| `yunseok@gmail.com` | 지목으로 안 봅니다. `@` 앞에 글자가 붙어 있으면 주소입니다 |
...
오른쪽 아래는 할 일입니다. 아래에 상태줄과 입력칸이 고정으로 붙습니다.
...
| | 전체화면 | 줄 화면 (`--no-tui`) |
|---|---|---|
| 언제 | 터미널에서 그냥 켤 때 | 파이프·CI·좁은 창·`--no-tui` |
| 바뀐 파일·할 일 | 오른쪽 칸에 늘 보임 (96칸 이상) | 나올 때 줄로 흘려보냄 |
| 지난 화면 | 스크롤 대신 창 안에서 접어 보여 줌 | 터미널 스크롤 그대로 |
| 나갈 때 | **나눈 대화를 스크롤백에 되살림** | 이미 거기 있음 |
| 기록으로 남길 때 | 안 맞음 | 이쪽 |

전체화면은 vim 처럼 딴 화면(대체 화면 버퍼)을 씁니다. 그냥 나가면 방금 나눈 대화가
통째로 사라지므로, 나갈 때 최근 줄들을 원래 화면에 다시 찍어 놓습니다.
Ctrl+C 로 죽든 오류로 터지든 **터미널은 반드시 되돌려 놓습니다.**

전체화면을 못 세우면 — 유별난 터미널이거나 무엇이 됐든 — 조용히 줄 화면으로 가고
왜 그랬는지 한 줄 말합니다. **화면 하나 때문에 프로그램이 안 뜨는 일은 없습니다.**
...
src/
  ...
  ui/tui.js              전체화면 — 접어쓰기·오른쪽 칸·나갈 때 되살리기
