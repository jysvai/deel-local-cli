...
걸 전제로 속을 다시 설계한** 것은 드뭅니다. 클라우드 API를 염두에 두고 만든
도구를 그대로 로컬로 옮기면 안 보이던 문제들이고, deel 이 그걸 어떻게
풀었는지입니다.
...
검사(`test/cache.test.js`)가 지킵니다 — 눈에 안 보이는 순서라 검사가 없으면
다음 기능이 아무 데나 끼어듭니다.
...
완화)은 **100%** 입니다 — 그러면서도 엉뚱한 곳을 고친 건수는 두 방식 다
**0건**입니다. 모호하면 찾지 못했다고 말할지언정 짐작해서 고치지 않습니다.
...
있게 "됐습니다" 라고 말할 때라는 걸 알기 때문입니다. `/export` 는 바깥 주소가
하나도 없는 자기완결 HTML 이라 폐쇄망 어디서든 열립니다.
