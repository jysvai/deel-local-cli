...
| 뻔한 인자 설명 | 뺌 | 남김 | 남김 |
| **고정 몫** | **2,475토큰 (30%)** | 2,928 (18%) | 4,303 (3%) |
...
Claude Code 에 없는 것은 여섯입니다 — `Append` · `Recall` · `Remember` ·
`Outline` · `Verify` · `Task`. 도구 하나가 스키마로 150~400토큰을 먹고 그게
**매 요청마다** 나가므로, 늘릴 때마다 검사에서 한 번 멈추게 해 뒀습니다
(`test/loop.test.js`). 뒤의 셋은 그 값을 치르고도 넣을 만해서 넣은 것들이라,
아래에 왜인지를 적어 뒀습니다.
