...
어긋 중에는 **한 운영체제에서만 재지는 것**이 있습니다 — `.cmd` 를 cmd.exe 로
감싸는 줄, PATHEXT, 대소문자를 안 가리는 파일 이름, 파워셸 자동완성, 엑셀 COM,
NTFS 스트림, taskkill. 딴 곳에서는 그 줄이 아예 안 돌고 짝지은 검사도 그 자리를
건너뛰어, 어긋내도 늘 초록입니다. 그런 것은 `잴곳` 에 운영체제를 적으세요
(`"잴곳": ["win32"]`). 적으면 딴 곳에서는 **건너뛰고 건너뛰었다고 화면에
적습니다.** 안 적으면 어디서나 잽니다.

```bash
node tools/mutate.mjs --이곳만            지금 운영체제에서만 재지는 것만
```

조용히 빼는 것은 안 됩니다. `잴곳` 에 적은 운영체제마다 **그것을 도는 job 이
관문에 있어야** 하고, 없으면 `test/죽은규칙.test.js` 가 빨개집니다 — 아무
데서도 안 도는데 관문만 초록인 것이 이 저장소가 제일 오래 앓은 고장입니다.

CI 는 두 가지로 돕니다. 하나라도 새면 그 자리에서 실패합니다.

- **푸시·PR 마다** — 이 판에서 바뀐 파일에 걸린 어긋만 잽니다(`--바뀐것`).
  바뀐 파일이 어긋의 `곳` 이나 `검사` 인 것, 그리고 `test/mutants.json` 에
  새로 들거나 고친 항목만 돕니다. 재는 판(`tools/mutate.mjs`)을 손댔거나 견줄
  곳을 못 읽으면 전부 돕니다. 리눅스는 `mutants-changed`(여덟 조각 — 전부 도는
  날을 위해서입니다), 윈도는 `mutants-changed-win` 이 맡고, `npm test` 를
  기다리지 않고 같이 뜹니다.
- **밤마다·손으로·태그에서** — 전부 쓸기. 리눅스가 여덟 조각을 나눠 맡고
  (`mutants-full`), 윈도에서만 재지는 것은 `mutants-full-win` 이 맡습니다.
  커버리지 바닥도 이때만 잽니다.

바뀐 자리만 도는 것은 지름길입니다. 딴 파일의 변화가 어긋 줄의 동작을 건드리는
일은 드물지만 없지 않아서, 전부 쓸기를 끄면 안 됩니다 — 그 job 과 그것을 깨우는
방아쇠가 있는지는 `test/죽은규칙.test.js` 가 봅니다.

```bash
node tools/mutate.mjs --바뀐것 origin/main   그 뒤로 바뀐 파일에 걸린 것만
node tools/mutate.mjs --조각 3/8 --세어만     셋째 조각이 무엇을 맡는지만 (JSON)
```

조각은 **개수가 아니라 걸리는 시간으로** 나눕니다. 어긋 하나의 값은 짝지은
검사가 얼마나 느린가로 정해지는데, 검사마다 500배까지 차이가 납니다 — 개수로
나눴을 때 한 조각은 81분, 다른 조각은 1.6분이었습니다. 검사마다 한 판 시간은
`test/검사시간.json` 에 있고, 리눅스 CI 로그에서 다시 뽑습니다:
...
표가 낡아도 재는 결과는 안 틀립니다. 조각이 덜 고르게 나뉠 뿐이라, 이 표에는
관문을 걸지 않습니다.
...
node tools/review2.mjs --model <agy models 로 목록>       다른 판으로
```

이 저장소에서 되풀이해 나온 고장은 한 부류입니다 — **있는 것과 걸리는 것은
다르다.** 규칙을 넣어 두고 그 규칙이 한 번도 안 걸린 자리, 검사를 만들어
두고 아무도 안 돌린 자리, 고쳤다고 적어 두고 그 고침이 안 지켜지는 자리.

그 부류가 오래 사는 까닭은 **만든 사람이 검사도 짜기 때문**입니다. 무늬를
잘못 적은 사람은 그 무늬를 재는 문장도 같은 오해로 고릅니다. 잘못이 두 곳에
똑같이 들어가면 둘이 서로를 통과시킵니다.

그래서 1차는 만든 쪽이 보고, **2차는 다른 모델이 봅니다.** 지금은 Gemini
입니다. 저장소 사정을 모르는 쪽이니 집안 규칙(한글 식별자·의존성 0개·나가는
문 하나)을 같이 넘겨 줍니다 — 안 주면 매번 그 셋을 지적하고, 그걸 걸러 내는
데 사람 시간이 들어 2차 리뷰의 값이 그만큼 깎입니다.

우리 소스에서 부르지 않습니다. `agy`(Antigravity CLI)를 **프로세스로
띄웁니다** — rg·git 을 빌려 쓰는 것과 같은 취급입니다. 소스에서 부르면
「나가는 문은 `src/backend/http.js` 하나」 라는 약속이 깨집니다. `--mode plan`
으로 띄우니 2차 리뷰어는 **읽기만** 합니다.

집안 규칙과 diff 는 임시 파일에 적어 두고 그 경로만 넘깁니다. 명령줄에는
길이 한도가 있어서(윈도우 cmd 는 8k 언저리) diff 를 인자로 실으면
`ENAMETOOLONG` 으로 그 자리에서 죽습니다.

이건 **문지기가 아니라 눈입니다.** 찾은 것을 적을 뿐 아무것도 안 막습니다 —
2차 리뷰가 빨간불이 되면 사람이 그것부터 끄게 되고, 그러면 눈이 하나 줍니다.

### 손으로 가끔 돌리는 것들

`tools/` 안에서 npm 스크립트가 아닌 것들입니다. 한 번 돌리고 결과를
커밋하는 종류라 자동으로 안 돕니다.

```bash
node tools/make-hero.mjs     README 머리 그림 — 배너 글꼴이 바뀌면 다시 돌립니다
node tools/split-docs.mjs    두꺼워진 README 를 docs/ 로 나눕니다
node tools/polish-docs.mjs   나눈 뒤 어긋난 링크·「위 ⋯ 를 보세요」 를 고칩니다
node tools/shot.mjs          문서에 넣을 터미널 화면을 찍습니다
```

`tools/` 에 새 파일을 넣으면 npm 스크립트로 걸거나 여기에 적으세요.
둘 다 아니면 `no-bundle` 검사가 「아무도 안 부른다」 로 잡습니다 — 만들어
두고 잊은 판이 이 저장소에 이미 여럿 있었습니다.

### 눈으로 보는 판

검사는 파이프로 도니 진짜 화면을 못 봅니다. 이 셋은 실제로 띄워서 눈으로 봅니다.

```bash
npm run demo                 짧은 일 한 판 — 도구·할 일 목록·되돌리기·셈
node test/demo-tui.mjs       입력 상자 테두리 — 한 칸이라도 어긋나면 바로 보입니다
node test/demo-bigfile.mjs   큰 파일이 잘리는 자리 — 상한을 진짜로 지키는 가짜 창구
```

셋 다 가짜 게이트웨이가 모델을 대신하고, 설정은 임시 폴더(`DEEL_HOME`)에
따로 둡니다 — 이 PC 의 진짜 연결은 안 씁니다. `npm run demo` 는 여태 그
임시 집을 안 줘서 **한 번도 안 돌았습니다** — 설정을 못 찾아 「저장된 연결이
없습니다」 한 줄만 찍고 끝났는데, 종료코드가 0 이라 아무도 안 물어봤습니다.
지금은 안 돌면 **안 돌았다고 말합니다.**
