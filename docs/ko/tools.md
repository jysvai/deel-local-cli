...
### 이미 깔려 있는 `rg` 를 빌려 씁니다 — 깔지는 않습니다

`Grep` 은 이 PC 에 **`rg`(ripgrep)가 이미 있으면** 그걸 빌려 씁니다. 없으면 `git grep`,
그것도 없으면 예전처럼 자바스크립트로 파일을 하나씩 엽니다. **아무것도 새로 깔지 않습니다** —
의존성 0개는 그대로입니다. VS Code 를 쓰면 `rg` 는 이미 딸려 와 있습니다.
...
**어느 것으로 찾았는지 늘 밝힙니다.** 결과 끝에 `(rg 으로 찾았습니다 — 이 PC 에 이미 있어서
빌려 썼습니다)` 가 붙습니다. `rg` 의 정규식은 자바스크립트와 조금 달라서(되돌아보기 `(?<=…)`
가 없습니다), 결과가 이상할 때 무엇으로 찾은 것인지 알아야 합니다. `rg` 가 무늬를 못 읽으면
**「일치 없음」이 아니라** 조용히 예전 길로 내려가서 다시 찾습니다 — 못 찾은 것과 못 물어본
것은 다릅니다.

두 길은 **같은 파일을 봅니다.** `.gitignore` · `.deelignore` 를 똑같이 지키고, `*.min.js` ·
`*.map` · 그림 같은 「찾을 것이 없는 파일」도 양쪽 다 안 엽니다. 갈리면 같은 명령이 PC 마다
다른 답을 내게 되므로, 그게 어긋나지 않는지를 검사가 지킵니다.

끄려면 `DEEL_GREP=js` — 결과가 의심스러울 때 같은 자리에서 두 길을 견주라고 둔 스위치입니다.

### 못 읽는 문서도 이 PC 의 변환기를 빌려 읽습니다

`rg` 와 **같은 원칙**입니다. deel 은 hwpx·docx·pptx·xlsx·pdf 를 직접 읽지만, 사내 자료는
이름만 `.pptx` 이고 속은 옛 `.ppt` 인 경우가 흔합니다. 그때 여태 이렇게 끝났습니다.
...
길이 없으니 모델은 같은 파일을 몇 번씩 다시 열었습니다. **정작 그 PC 에는 LibreOffice 가
깔려 있었습니다** — 사람이 그 파일을 열어 보는 바로 그 프로그램입니다.
...
| | |
|---|---|
| 무엇을 빌리나 | `soffice`(LibreOffice) · `textutil`(맥). 맥에서는 빠른 `textutil` 을 먼저 씁니다 |
| 어디서 찾나 | PATH 와, PATH 에 안 걸리는 자리들 — 맥 앱 꾸러미 안, 윈도우 `Program Files` |
| 무엇을 바꿔 보나 | `.ppt` · `.doc` · `.xls` · `.rtf` · `.odt` · `.hwp`, 그리고 **겉과 속이 다른** `.pptx` 류 |
| 어디에 떨구나 | `.deel/tmp/` — 작업 폴더 **안**입니다. 읽는 즉시 지웁니다. 원본은 한 글자도 안 바뀝니다 |
| 없으면 | 없다고 말하고 끝냅니다. 아무것도 설치하지 않습니다 |
| 끄려면 | `DEEL_CONVERT=off` |

빌려 읽은 것은 **빌렸다고 화면에 적습니다.** 그리고 그 파일은 `Edit`·`Write` 로 못 고칩니다 —
글만 뽑아 본 것이라, 그 글을 되돌려 쓰면 원본 조판이 통째로 사라집니다.

### 파일이 너무 많으면 「안 봤다」고 말합니다

폴더 훑기는 **2만 개**에서 멈춥니다. 멈췄으면 결과 끝에 이렇게 나옵니다.
...
「일치 없음」이 아니라 **「본 데까지는 일치 없음」**입니다. 못 찾은 것과 안 본 것은 다르고,
그걸 안 가르면 없는 줄 알고 그냥 지나갑니다. `Glob` · `Outline` · `Verify` 도 같습니다.
상한을 올리려면 `DEEL_WALK_LIMIT=50000` (100 ~ 2,000,000 사이만 받습니다).
폴더를 통째로 `Move` 할 때 이 수를 넘으면 옮기기는 전부 되지만 `/undo` 가 앞부분만
되돌리므로, 그때도 그렇다고 적어 줍니다.
