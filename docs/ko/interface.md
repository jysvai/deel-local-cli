...
`DEEL_MOTION` 으로 고릅니다. 기본은 위의 그림들입니다.

| 값 | 무엇이 나오나 |
|---|---|
| (안 줌) | 덩어리·막대·훑는 줄 — 무엇을 하는 중인지 말한다 |
...
한 판을 통째로 보내면 9KB 가 넘는데, 판이 바뀔 때 실제로 달라지는 것은 **열두 줄
중 한 줄**뿐입니다(벽·창문·서류함·빈 책상은 그대로입니다). 그래서 상자가 바뀐
줄만 보내도록 고쳤고, 그 덕에 사무실을 안 켜는 사람도 같이 가벼워졌습니다.
