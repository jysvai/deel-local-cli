...
function 볼것() {
  ...
  if (부터) {
     ...
     */
    /*
     * git 이 실패한 것을 「안 바뀌었다」 로 읽으면 안 된다. 없는 판 이름을
     * 적은 사람은 그 말을 「그 사이에 아무것도 안 바뀌었구나」 로 읽고
     * 넘어간다 — **리뷰를 건너뛴 것을 리뷰가 끝난 것으로 안다.** 파일
     * 갈래는 이미 status 를 보고 있었는데 여기만 안 봤다.
     */
    const 범위판 = git('diff', `${부터}..${까지}`);
    if (범위판.status !== 0) {
      console.error(`\n\x1b[31m✗ 그 범위를 못 읽었습니다: ${부터}..${까지}\x1b[0m\n  ${String(범위판.stderr ?? '').trim().split('\n')[0]}\n`);
      process.exit(2);
    }
    const 사이 = 범위판.stdout;
    if (!사이.trim()) {
