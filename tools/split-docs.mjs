...
/**
 * 이름표에서 앞머리(`자세히` · `More`)를 뗀다.
 *
 * 꼬리말이 붙은 것만 떼고 있었다(`^자세히\s*[—-]\s*`). 그런데 `<summary>` 가
 * `<b>자세히</b>` 한 마디로 끝나는 덩이가 있고, 그때는 앞머리가 그대로 남아
 * README 에 `자세히 — 자세히` 가 적히고 옮겨 간 쪽에는 `### 자세히` 라는 아무
 * 뜻 없는 제목이 선다. 앞에 기호(`▸`)가 붙어 와도 같다.
 *
 * 그래서 꼬리말을 **없어도 되는 것**으로 두고 앞의 기호도 같이 뗀다. 남는 것이
 * 없으면 빈 글자다 — 부르는 쪽이 그걸 보고 줄을 통째로 뺀다.
 */
function 앞머리떼기(s, 자세히) {
  return s.replace(new RegExp(`^[^\\p{L}\\p{N}]*${자세히}\\s*(?:[—–-]\\s*)?`, 'u'), '').trim();
}
...
function 나누기(말) {
  ...
  for (const m of 묶음) {
    ...
    for (const 절이름 of 이름들) {
      ...
      const 보임 = 앞머리떼기(조각.map((x) => x.요약).join(' · '), 설정.자세히);
      넣을것.set(덩이들[0].시작, [
        보임 ? `> **${설정.자세히}** — ${보임}` : `> **${설정.자세히}**`,
        '>',
