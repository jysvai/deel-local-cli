...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/jysvai/deel-local-cli/main/docs/assets/hero-ko-dark.svg">
  <img alt="deel — 이 컴퓨터 안에서만" src="https://raw.githubusercontent.com/jysvai/deel-local-cli/main/docs/assets/hero-ko-light.svg" width="620">
</picture>
...
**[English](README.md)** · [사내 반입 안내](#사내-반입) · [문제 해결](#문제-해결) · [자세한 문서](docs/ko/)

</div>

---

### 켜자마자 제일 궁금한 것부터 답합니다

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/jysvai/deel-local-cli/main/docs/assets/shot-chat-ko-dark.svg">
  <img alt="deel 시작 화면과 시킨 일 한 바퀴 — 모델·보내는 곳·폴더·승인 방식, 그리고 Grep→Read→Edit" src="https://raw.githubusercontent.com/jysvai/deel-local-cli/main/docs/assets/shot-chat-ko-light.svg" width="900">
</picture>

**`보냄  이 컴퓨터 안 127.0.0.1 ← 여기 말고는 어디로도 안 갑니다`.**
문서에 적힌 약속이 아니라 켤 때마다 화면에 뜹니다. 앞에 붙은 `⌂` 는 대화 내내
상태줄에 남아 있고, 바깥으로 나갈 일이 생기면 그 자리에서 `↗` 로 바뀝니다.

그 아래는 시킨 일 한 바퀴입니다 — 찾고(`Grep`), 읽고(`Read`), 고치고(`Edit`),
무엇을 고쳤는지 그 자리에서 보여 줍니다.
...
| `/lang [ko|en|ja|zh] [시킬말]` | 화면 말 (한국어·English·日本語·中文 네 가지). 표에 든 것은 다 채워져 있다 — 다만 도구 결과 한 줄(`5줄`·`1군데`)과 생각 중 표시는 아직 표 밖이라 영어 화면에서도 한국어로 나온다. **시킬 말을 따로 줄 수 있다** — `/lang ko en` 이면 모델에게는 영어로 시키고 답은 한국어로 받는다(8k 창에서 고정 몫 23% 감소). 코드는 그대로 |
...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/jysvai/deel-local-cli/main/docs/assets/shot-reset-dark.svg">
  <img alt="deel reset — 지울 수 있는 것과 안 지우는 것을 세어서 보여 준다" src="https://raw.githubusercontent.com/jysvai/deel-local-cli/main/docs/assets/shot-reset-light.svg" width="760">
</picture>
...
심사서는 `deel audit` 으로 먼저 볼 수 있습니다. 손으로 적은 것이 아니라
**소스를 훑어서 그때그때 만듭니다** — 그래서 코드와 어긋날 수가 없습니다.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/jysvai/deel-local-cli/main/docs/assets/shot-audit-dark.svg">
  <img alt="deel audit — 의존성·네트워크 호출·포트를 소스에서 훑어 만든 심사서" src="https://raw.githubusercontent.com/jysvai/deel-local-cli/main/docs/assets/shot-audit-light.svg" width="820">
</picture>
