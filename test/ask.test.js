...
/*
 * ── 「ㅇㅇ」 「네.」 「no.」 가 계획을 다시 짜러 갔다 ──────────────────────
 *
 * 첫 낱말을 표와 **글자 그대로** 견줬다. 끝에 마침표·느낌표가 붙거나 자음을 두 번 친 답
 * (ㅇㅇ · ㄴㄴ)은 표에 없는 글이 되어 「그 말대로 다시 짜라」 로 갔다. 그러라고 했는데 계획이
 * 또 나오고, 그만하라고 했는데 계획이 또 나온다.
 *
 * 물음표는 안 뗀다 — 「네?」 「ok?」 는 되묻는 말이지 그러라는 말이 아니다.
 */
{
  const 재기 = (답) => 계획답풀기(답, 4);
  for (const 답 of ['ㅇㅇ', '응', 'yes!', 'ok.', '네.', 'ㅇㅇ 검사도 넣어줘']) {
    check(`★ 「${답}」 는 진행이다`, 재기(답).갈래 === '진행', JSON.stringify(재기(답)));
  }
  for (const 답 of ['no.', 'ㄴㄴ']) check(`★ 「${답}」 는 그만이다`, 재기(답).갈래 === '그만', JSON.stringify(재기(답)));
  check('「네?」 는 진행이 아니다 — 되묻는 말이다', 재기('네?').갈래 !== '진행', JSON.stringify(재기('네?')));
  check('자음 두 번 뒤의 한마디도 그대로 얹힌다', 재기('ㅇㅇ 검사도 넣어줘').덧말 === '검사도 넣어줘', JSON.stringify(재기('ㅇㅇ 검사도 넣어줘')));
}

{
  /*
   * ── 「ok 말고」 「응 취소」 가 진행이었다 (Gemini 화면4) ───────────────────
   *
   * 첫 낱말만 표와 견줬다. 첫 낱말이 진행 쪽이면 뒤에 붙은 말은 「진행하되 한마디」 로 얹었다 —
   * 「ok 말고」 「진행 하지 마」 「응 취소」 「y 아니 멈춰」 가 전부 계획 실행으로 갔다. 멈추라는 말을
   * 진행으로 읽는 것이 이 상자에서 가장 비싼 틀림이다(안 시킨 일을 한다).
   * 「그만」 「멈춰」 「stop」 은 표에 없어서 「그 말대로 다시 짜라」 로 갔다.
   */
  const 재기 = (답) => 계획답풀기(답, 4);
  for (const 답 of ['ok 말고', 'ok 하지마', '진행 하지 마', '진행 말고 다시', 'y 아니 멈춰', '네 아니요', '응 취소', 'go no',
    'ok 안 돼', 'ok 안돼', 'y 하지 말고 1번만', '네 잠깐만', 'yes but stop', 'ok wait']) {
    check(`★★ 「${답}」 는 진행이 아니다`, 재기(답).갈래 !== '진행', JSON.stringify(재기(답)));
  }
  for (const 답 of ['그만', '그만해', '멈춰', '중단', 'stop', 'cancel', '아뇨', 'nope', '그만.']) {
    check(`★ 「${답}」 는 그만이다`, 재기(답).갈래 === '그만', JSON.stringify(재기(답)));
  }
  // 뒤에 붙은 말이 멈춤이 아니면 여태 그대로 진행이다 — 「note」 의 not, 「아니」 없는 「안내」 에 안 걸린다.
  for (const 답 of ['y 검사도 같이 넣어줘', 'ok 테스트 먼저', '응 3번은 꼼꼼히', 'y note 도 남겨줘', 'ok 안내문도 고쳐줘']) {
    check(`「${답}」 는 여전히 진행이다`, 재기(답).갈래 === '진행', JSON.stringify(재기(답)));
  }
  // Gemini 화면5 — 「진행 종료」 「y abort」 도 멈추라는 말이다. 「진행해」 「좋아」 「ㄱㄱ」 는 진행하라는 말이다.
  for (const 답 of ['진행 종료', '진행 포기', 'y abort', 'y quit', 'y 중지', 'ok 그만둬']) {
    check(`★★ 「${답}」 는 진행이 아니다`, 재기(답).갈래 !== '진행', JSON.stringify(재기(답)));
  }
  for (const 답 of ['진행해', '진행해줘', '좋아', '좋아요', '좋습니다', '그래', 'okay', 'sure', 'ㄱㄱ', '고고']) {
    check(`★ 「${답}」 는 진행이다`, 재기(답).갈래 === '진행', JSON.stringify(재기(답)));
  }
}

/*
 * ── 「2번」 「1번 2번」 「２」 가 고른 것이 아니었다 ──────────────────────
 *
 * 수 바로 뒤에 글자가 붙으면 수로 안 읽는 선(「3개만」)이 「번」 까지 막았다. 한국 사람은 번호를
 * 「2번」 이라고 친다. 일본어·중국어 입력기는 전각 숫자(２)를 낸다.
 */
{
  const 목록 = ['가', '나', '다'];
  check('★ 「2번」 은 2번을 고른 것이다', 고른것풀기('2번', 목록).번호들.join() === '2', JSON.stringify(고른것풀기('2번', 목록)));
  check('★ 「1번 2번」 은 둘 다다', 고른것풀기('1번 2번', 목록).번호들.join() === '1,2', JSON.stringify(고른것풀기('1번 2번', 목록)));
  check('★ 전각 숫자 「２」 도 2번이다', 고른것풀기(String.fromCodePoint(0xff12), 목록).번호들.join() === '2',
    JSON.stringify(고른것풀기(String.fromCodePoint(0xff12), 목록)));
  check('「2번 이건 나눠서」 는 고르고 말도 붙인다', 고른것풀기('2번 이건 나눠서', 목록).덧말 === '이건 나눠서',
    JSON.stringify(고른것풀기('2번 이건 나눠서', 목록)));
  check('「3개만 해줘」 선은 그대로다', 고른것풀기('3개만 해줘', 목록).번호들.length === 0, JSON.stringify(고른것풀기('3개만 해줘', 목록)));
  check('계획 상자도 「3번 이건 빼자」 를 3단계에 대한 말로 읽는다', 계획답풀기('3번 이건 빼자', 4).단계 === 3,
    JSON.stringify(계획답풀기('3번 이건 빼자', 4)));
}

/*
 * ── 「1 번」 · 「2- 나눠서」 · 「취소 말고」 (2.0.0 6회차 고르기6cg) ──────────
 *
 * 셋 다 사람이 흔히 치는 모양인데 세 가지로 어긋났다.
 *   · 「1 번」  — 「번」 을 수에 붙여 칠 때만 읽어서, 빈칸을 넣으면 「번」 이 하고 싶은 말로
 *                남아 모델에게 「가 — 번」 이 갔다.
 *   · 「2- 」   — 머리말이 적어 둔 모양 중 「2 - 나눠서」 는 읽는데 빈칸 없는 「2- 나눠서」 는
 *                아무것도 못 골랐다. 붙임표는 글자가 아니라 구분표다. 다만 「2-3」 은 범위로
 *                읽힐 수 있으니 뒤가 숫자면 여태처럼 안 읽는다.
 *   · 「취소 말고」 — 멈춤 낱말 뒤에 부정이 붙으면 멈추라는 말이 아니다. 진행 쪽은 이미
 *                「ok 말고」 를 다시로 보내는데(위 블록), 멈춤 쪽만 그대로 멈췄다.
 */
{
  const 목록 = ['가', '나', '다'];
  const 재기 = (답) => 고른것풀기(답, 목록);
  const 계획 = (답) => 계획답풀기(답, 4);

  check('★ 「1 번」 은 1번이고 「번」 이 말로 안 남는다',
    재기('1 번').번호들.join() === '1' && 재기('1 번').덧말 === '', JSON.stringify(재기('1 번')));
  check('「2 번 이건 나눠서」 도 고르고 말만 붙인다',
    재기('2 번 이건 나눠서').번호들.join() === '2' && 재기('2 번 이건 나눠서').덧말 === '이건 나눠서',
    JSON.stringify(재기('2 번 이건 나눠서')));
  check('「2번째로 해줘」 는 여태처럼 고른 것이 아니다', 재기('2번째로 해줘').번호들.length === 0,
    JSON.stringify(재기('2번째로 해줘')));

  check('★ 「2- 나눠서」 도 2번을 고른 것이다',
    재기('2- 나눠서').번호들.join() === '2' && 재기('2- 나눠서').덧말 === '나눠서', JSON.stringify(재기('2- 나눠서')));
  check('「2-3 해줘」 는 범위라 안 읽는다', 재기('2-3 해줘').번호들.length === 0, JSON.stringify(재기('2-3 해줘')));
  check('「1.5 는」 선도 그대로다', 재기('1.5 는').번호들.length === 0, JSON.stringify(재기('1.5 는')));

  for (const 답 of ['취소 말고', '그만 말고', '아니 말고', 'no 말고', 'stop 하지 말고', '중단 하지 마']) {
    check(`★★ 「${답}」 는 그만이 아니다`, 계획(답).갈래 !== '그만', JSON.stringify(계획(답)));
  }
  for (const 답 of ['no 그만해', 'n 중단', 'no 검사도 같이 넣어줘', '취소', '그만']) {
    check(`「${답}」 는 그대로 그만이다`, 계획(답).갈래 === '그만', JSON.stringify(계획(답)));
  }
}

/*
 * ── 「2번- 나눠서」 (2.0.0 8회차 uimisc PK4) ────────────────────────────────
 *
 * 붙임표를 구분표로 받은 것은 **수 바로 뒤**뿐이었다. 「번」 뒤 전방탐색에는 붙임표가 빠져 있어서
 * 「2번- 나눠서」 가 아무것도 못 골랐다 — 같은 정규식 안에서 「2- 나눠서」 는 읽는데 「번」 을
 * 하나 더 친 사람만 고른 것이 사라진다. 한국 사람이 제일 흔히 치는 두 모양을 섞은 꼴이다.
 *
 * 범위 선은 그대로다 — 「2번-3」 은 「2-3」 과 똑같이 안 읽는다.
 */
{
  const 목록 = ['가', '나', '다'];
  const 재기 = (답) => 고른것풀기(답, 목록);

  check('★ 「2번- 나눠서」 도 2번을 고른 것이다',
    재기('2번- 나눠서').번호들.join() === '2' && 재기('2번- 나눠서').덧말 === '나눠서', JSON.stringify(재기('2번- 나눠서')));
  check('「2번 - 나눠서」 도 같다',
    재기('2번 - 나눠서').번호들.join() === '2' && 재기('2번 - 나눠서').덧말 === '나눠서', JSON.stringify(재기('2번 - 나눠서')));
  check('긴 붙임표(–)도 구분표다',
    재기('2번– 나눠서').번호들.join() === '2' && 재기('2번– 나눠서').덧말 === '나눠서', JSON.stringify(재기('2번– 나눠서')));
  check('「2번-3 해줘」 는 범위라 안 읽는다', 재기('2번-3 해줘').번호들.length === 0, JSON.stringify(재기('2번-3 해줘')));
  check('계획 상자도 「3번- 이건 빼자」 를 3단계에 대한 말로 읽는다', 계획답풀기('3번- 이건 빼자', 4).단계 === 3,
    JSON.stringify(계획답풀기('3번- 이건 빼자', 4)));
}
