...
trace('5-몫과차례');
{
   ...
   */
  /*
   * 10차 리뷰. 주석은 「설정에서 온 값은 글자열이기 쉽다」 고 적어 놓고
   * 정작 무한대는 숫자로만 쟀다. 글자열 `'Infinity'` 갈래를 같이 넣는다.
   *
   * 그리고 **개수만** 재면 몫 나눔이 깨져도 초록이다 — 상한 3에서 내장이
   * 셋 다 차지해도 길이는 3이다. 갈래별로 몇이 실렸는지도 같이 잰다.
   */
  for (const [상한, 몇개, 내장몇] of [
    ['10', 10, 2], [10, 10, 2], ['3', 3, 0], [4, 4, 1],
    [Infinity, 16, 8], ['Infinity', 16, 8], ['1e2', 16, 8],
  ]) {
    const s = 세션만들기();
    s.maxSkillsListed = 상한;
    ...
    check(`★★★ 숫자꼴 글자열과 무한대는 상한으로 받는다 — ${String(상한)}`,
      실린것.length === 몇개, `${실린것.length}개 · 바람 ${몇개}개`);
    check(`★★★ 그 안에서 내장 몫은 상한의 1/4 이다 — ${String(상한)}`,
      내장 === 내장몇 && 실린것.length - 내장 === 몇개 - 내장몇,
      `내장 ${내장} · 바람 ${내장몇} · 전부 ${실린것.length}`);
  }
  /*
   * 11차 리뷰. 위 줄들은 내장이 여덟뿐이라 상한이 크면 1/4 이 아니라
   * **있는 것 전부**가 실린다 — 검사 이름이 재지도 않는 것을 주장했다.
   *
   * 12차 리뷰가 그 고침에 또 같은 탈이 남았다고 짚었고, 재 보니 맞았다.
   * 「몫은 상한의 1/4 에서 멈춘다」 고 이름 붙였는데 **내장만 있을 때는
   * 안 멈춘다.**
   *
   *     내장 40 · 사용자 40 · 상한 40   →  내장 10   (깎인다)
   *     내장 40 ·  사용자 0 · 상한 40   →  내장 40   (안 깎인다)
   *
   * 1/4 은 천장이 아니라 **자리를 다투는 쪽이 있을 때만 내장을 묶어 두는
   * 빗장**이다. 둘 다 재고, 이름도 하는 일에 맞춘다. 주석에 재현 조건을
   * 적을 때 사용자 40 을 빼먹은 것도 같이 고친다.
   */
  {
    const s = 세션만들기();
    s.maxSkillsListed = 40;
    s.skills = [...짓기(40, 'user'), ...짓기(40, 'builtin')];
    const 실린것 = s.listedSkills();
    const 내장 = 실린것.filter((x) => x.source === 'builtin').length;
    check('★★★ 다투는 쪽이 있으면 내장 몫은 상한의 1/4 에서 멈춘다',
      실린것.length === 40 && 내장 === 10,
      `전부 ${실린것.length} · 내장 ${내장}`);
  }
  {
    const s = 세션만들기();
    s.maxSkillsListed = 40;
    s.skills = 짓기(40, 'builtin');
    const 실린것 = s.listedSkills();
    check('★★★ 다투는 쪽이 없으면 내장이 상한까지 채운다',
      실린것.length === 40 && 실린것.every((x) => x.source === 'builtin'),
      `전부 ${실린것.length}`);
  }
  /*
