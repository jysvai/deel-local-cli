...
//   · 한 번 누르면 화면 고치기가 **한 번**이어야 한다.
{
   ...
   *
   * 그래서 재는 것을 바꾼다. 테두리는 쌓인 것에서 보고(한 번 그려진 뒤로 아무도
   * 안 지웠다), 정작 중요한 **지운 결과가 화면에 제대로 남았나**는 지금글() 로
   * 본다. 이쪽이 원래 잡으려던 것에 더 가깝다 — 세 번 지웠으면 세 글자가
   * 없어져 있어야 한다.
   */
  check('지운 뒤에도 상자가 안 깨진다',
    /╰/.test(r[2].쌓인것) && /│ ❯ /.test(r[2].쌓인것) && r[2].본것 === r[2].기대,
