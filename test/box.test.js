 ...
 * 아이에게 넘길 환경.
 *
 * NO_COLOR 를 뺀다. 이 검사는 「몇 번 다시 그렸나」 를 ANSI 로 센다 —
 * 색이 꺼진 아이는 그 글자를 아예 안 내보내서, 고칠 것이 하나도 없는데
 * 「0회」 로 빨개진다. 실제로 NO_COLOR 를 켜 둔 셸에서만 두 항목이 졌다.
 * FORCE_COLOR 를 줘도 소용없다 — NO_COLOR 가 이기는 것이 표준이고,
 * 이 프로그램도 그렇게 따른다.
 *
 * 재는 것은 「색이 있으면 몇 번 그리나」 이지 「이 셸에 색이 있나」 가 아니다.
 */
function 아이환경(더할것) {
  const { CI, NO_COLOR, ...나머지 } = process.env;
...
async function 키눌러보기(단계들) {
  ...
  // out 은 이번 단계에만 나간 것(깜빡임을 재려면 단계별이어야 한다),
  // 전체는 처음부터 쌓인 것(상자의 지금 모습을 보려면 이쪽이어야 한다).
  let out = '';
  let 전체 = '';
  kid.stdout.on('data', (b) => { out += b; 전체 += b; });
  kid.stderr.on('data', (b) => { out += b; 전체 += b; });
  let 끝남 = false;
  const 닫힘 = new Promise((r) => kid.on('close', () => { 끝남 = true; r(); }));
  const 자기 = (ms) => new Promise((r) => setTimeout(r, ms));
   ...
   * 마지막으로 그려진 상자 안쪽 글.
   *
   * **단계별(out)이 아니라 처음부터 쌓인 것(전체)에서 찾는다.** 상자는 바뀐
   * 줄만 보내므로, 어떤 단계에서 입력줄이 안 바뀌었으면 그 단계 출력에는
   * 입력줄이 아예 없다. 단계별로만 보면 '화면이 비었다' 로 잘못 읽힌다.
   *
   * 쌓인 것에서 **마지막으로 나간 입력줄**이 곧 지금 화면이다 — 바뀔 때마다
   * 반드시 나가고, 안 나갔다는 것은 그대로라는 뜻이기 때문이다.
   */
  const 지금글 = () => {
    const 줄들 = 전체.replace(/\x1b\[[0-9;?]*[a-zA-Z]/g, '').split('\n').filter((l) => /│ [❯…] /.test(l));
  ...
  const 결과 = [];
  for (const 단계 of 단계들) {
    // 뜸들이기: 키를 치기 전에 기다린다. 시계로 도는 것(자가치유)을 재려면
    // 가만히 있는 시간이 필요하다.
    if (단계.뜸들이기) await 자기(단계.뜸들이기);
    out = '';
    ...
    // 제어문자를 안 벗긴 것도 같이 돌려준다. 깜빡임은 **글이 아니라 제어문자
    // 순서**로 생기므로, 벗기고 나면 잴 수가 없다.
    // 쌓인것 = 이 단계까지 화면에 나간 것 전부. 바뀐 줄만 보내므로,
    // '테두리가 아직 있나' 같은 것은 단계별 출력이 아니라 이쪽으로 봐야 한다.
    // 쌓인것 = 이 단계까지 화면에 나간 것 전부, 제어문자를 벗긴 것. 바뀐 줄만
    // 보내므로 '테두리가 아직 있나' 같은 것은 단계별이 아니라 이쪽으로 본다.
    결과.push({
      ...단계,
      본것,
      찍힌것,
      날것: out,
      쌓인것: 전체.replace(/\x1b\[[0-9;?]*[a-zA-Z]/g, ''),
      맞나: 본것 === 단계.기대,
    });
  }
...
// 둘 다 실제로 겪은 것이고, 둘 다 값으로는 안 잡힌다.
{
  ...
  const r = await 키눌러보기([
    ...
    { 이름: '지우고 갈리는 것을 친다', 키: [지움, '/', 'm', 'o', 'd'], 기대: '/mod' },
    // /mode·/model 이 남아 있다. 하나를 골라 박으면 지우는 수고가 더 든다.
    // (`/mo` 까지만 치면 /motion 도 걸려서 채울 것이 없다 — 그래서 `/mod`.)
...
// ── 글자를 지울 때 화면이 꺼지는 것처럼 보이면 안 된다 ──────────────────
//
// 겪은 것: "백스페이스 누를 때마다 화면이 꺼지는 느낌".
//
// 원인은 그리는 차례였다. 옛 방식은 한 번 누를 때마다
//   1) 상자를 싹 지우고(`\x1b[K` + 위로 를 줄 수만큼)
//   2) 새로 그리고
//   3) 커서를 되돌린다
// 를 **따로따로** 내보냈다. 터미널은 받는 대로 그리므로 1) 과 2) 사이에
// **빈 상자가 실제로 한 번 화면에 나타난다.** 글자를 지울 때는 한 글자에
// 한 번씩 반복되니, 상자가 깜빡거리는 것으로 보인다.
//
// 여기서 재는 것은 '느낌' 이 아니라 나간 제어문자다.
//   · 지우고 다시 그린 흔적(지우기 버스트)이 없어야 하고
//   · 한 번 누르면 화면 고치기가 **한 번**이어야 한다.
{
  const BS = '\x7f';
  const r = await 키눌러보기([
    { 이름: '글을 쳐 둔다', 키: ['집', '계', ' ', '함', '수'], 기대: '집계 함수' },
    { 이름: '한 글자 지운다', 키: [BS], 기대: '집계 함' },
    { 이름: '이어서 세 번 더 지운다', 키: [BS, BS, BS], 기대: '집' },
    // 가만히 두면 4초에 한 번은 통째로 다시 그린다(자가치유). 그때는 안 바뀐
    // 테두리까지 다시 나가야 한다 — 그것을 이 단계 출력에서 직접 본다.
    { 이름: '가만히 두면 통째로 다시 그린다', 뜸들이기: 4600, 키: ['!'], 기대: '집!' },
  ]);

  for (const x of r) check(x.이름, x.맞나, x.맞나 ? '' : `본 것 ${JSON.stringify(x.본것)}`);

  // 지우기 버스트: 줄을 지우고(`\x1b[K`) 곧바로 위로 올라가는(`\x1b[1A`) 것이
  // 글자 하나 없이 이어지는 모양. 지우고 다시 그릴 때만 나온다.
  const 버스트 = (s) => (s.match(/\x1b\[K\x1b\[1A/g) ?? []).length;
  const 한번 = r[1].날것; const 여러번 = r[2].날것;
  check('지웠다가 다시 그리지 않는다 — 빈 상자가 안 스친다',
    버스트(한번) === 0 && 버스트(여러번) === 0,
    `버스트 ${버스트(한번)}회 · ${버스트(여러번)}회`);

  // 커서 숨김(`\x1b[?25l`) 한 번 = 화면 고치기 한 번. 한 번 누르면 한 번이다.
  const 고침수 = (s) => (s.match(/\x1b\[\?25l/g) ?? []).length;
  check('한 번 누르면 화면을 한 번만 고친다', 고침수(한번) === 1, `${고침수(한번)}회`);
  check('세 번 누르면 세 번', 고침수(여러번) === 3, `${고침수(여러번)}회`);

  /*
   * 지운 뒤에도 상자는 멀쩡해야 한다.
   *
   * 예전에는 '이번 단계 출력에 ╰ 와 │ ❯ 가 있나' 로 봤다. 상자가 매번
   * 통째로 다시 나가던 시절에는 그것으로 됐는데, 지금은 **바뀐 줄만** 나가므로
   * 안 바뀐 테두리는 그 출력에 없다 — 멀쩡한데도 없다고 잡힌다.
   *
   * 한동안 이렇게 재고 있었다 — 테두리는 **쌓인 것**에서 보고, 지운 결과만
   * 지금글() 로 본다. 그런데 쌓인 것은 프로세스가 켜진 순간부터의 전부라
   * `/╰/` 는 상자가 처음 그려진 그 순간부터 늘 참이다. 지금 화면에서 테두리가
   * 통째로 사라져도 통과한다 — 아무것도 안 지키는 절이었다.
   *
   * 그래서 두 가지로 나눠 잰다.
   *   · 지운 결과가 제대로 남았나  → 지금글() (이건 원래도 진짜였다)
   *   · 테두리가 살아 있나         → **자가치유가 도는 단계의 출력**에서 본다.
   *     4초를 가만히 두면 견주기를 접고 통째로 다시 그리므로, 그 단계에는
   *     안 바뀐 테두리까지 반드시 나간다. 안 나가면 자가치유가 죽은 것이다.
   */
  check('지운 결과가 화면에 그대로 남는다',
    r[2].본것 === r[2].기대,
    `본 것 "${r[2].본것}" · 바란 것 "${r[2].기대}"`);
  check('가만히 두면 테두리까지 통째로 다시 그린다',
    /╭/.test(r[3].찍힌것) && /╰/.test(r[3].찍힌것) && /│ ❯ /.test(r[3].찍힌것),
    r[3].찍힌것.includes('╰') ? '' : '그 단계 출력에 테두리가 없다');
}
...
// 파이프에서는 지워지는 대신 **한 줄에 같이 찍힌다.** 그걸 잡는다.
{
  ...
  // 답은 이제 마크다운으로 그려진다 — `**계획표**` 는 화면에 `계획표` 로 뜬다.
  // 그러니 원문 그대로가 아니라 **보이는 글자**로 재야 한다. 여기서 재려는 것은
  // 꾸밈이 아니라 "글자가 먹히지 않았는가" 이므로 꾸밈 기호만 벗기고 본다.
  const 보일것 = 답.filter((t) => t !== '\n').map((t) => t.replace(/\*\*/g, ''));

  // 답 글자가 하나도 안 없어졌는가.
  const 먹힌것 = 보일것.filter((t) => !r.글.includes(t));
  check('흘러나온 답이 한 토막도 안 없어진다', 먹힌것.length === 0, 먹힌것.join(' / '));

  // 그리고 실제로 그려졌는가 — 별표가 화면까지 따라 나오면 안 그린 것이다.
  check('마크다운 기호는 화면에 안 남는다', !/\*\*/.test(r.글),
    (r.글.match(/.{0,20}\*\*.{0,20}/) ?? [''])[0]);
...
// 일하는 표시는 점자 한 칸이 도는 것이었는데, 지금은 갈래별 작은 그림(점자 세 칸)이다.
// 그래서 돌림표 열 글자를 못 박는 대신 **점자면 된다**로 찾는다.
// 여기서 재려는 것은 '어떤 글자냐' 가 아니라 '일하는 상자가 서 있느냐' 다.
