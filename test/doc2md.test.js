...
trace('2-표');
{
  ...
  /*
   * ★★★ `|` **바로 앞**의 백슬래시가 우리가 씌운 벗김표를 먹었다 (20회차 #140).
   *
   * 칸에 `a\|b` 가 들어오면 파이프만 벗기던 옛 코드가 `a\\|b` 를 냈다. 마크다운은
   * 그것을 **「백슬래시 하나」 + 살아 있는 칸 가름표**로 읽는다 — 막으려던 바로
   * 그 일을 우리가 한 셈이다. 가름표 앞 백슬래시가 홀수 개면 언제나 그랬다.
   *
   * 재는 법: **살아 있는 가름표 수**를 센다. 날것 `|` 를 세면 벗겨진 것까지
   * 같이 세어 잣대가 거꾸로 선다 — 한 번 그렇게 짰다가 고쳤다. 마크다운이
   * 실제로 칸을 가르는 것은 앞에 백슬래시가 **짝수 개** 붙은 `|` 뿐이다.
   *
   * 끝엣것만 보는 위 두 줄은 옛 코드에서도 초록이었다. 그래서 이 줄이 따로 있다.
   */
  const 살아있는가름표 = (줄) => {
    let n = 0;
    for (let i = 0; i < 줄.length; i += 1) {
      if (줄[i] !== '|') continue;
      let 앞 = 0;
      for (let j = i - 1; j >= 0 && 줄[j] === '\\'; j -= 1) 앞 += 1;
      if (앞 % 2 === 0) n += 1;   // 짝수면 벗김표가 아니다 — 진짜 가름표다
    }
    return n;
  };
  const 가름표 = (값) => 살아있는가름표(한칸(값));
  check('★★★ 파이프 앞 백슬래시가 칸 가름표를 안 되살린다',
    가름표('a\\|b') === 가름표('ab'),
    `${한칸('a\\|b')} · 가름표 ${가름표('a\\|b')} vs ${가름표('ab')}`);
  check('★★ 백슬래시가 둘이어도 마찬가지다',
    가름표('a\\\\|b') === 가름표('ab'),
    `${한칸('a\\\\|b')} · 가름표 ${가름표('a\\\\|b')}`);
