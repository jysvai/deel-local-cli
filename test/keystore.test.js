...
if (!잠금장치.되나) {
  ...
  const t2 = Date.now();
  const 또 = 잠그기(열쇠);
  const 또시간 = Date.now() - t2;
  // 첫 잠그기는 이 러너에서 powershell.exe 가 처음 뜨는 값까지 낸다 — 윈도우 CI 에서
  // 보통 0.4초인데 가끔 4~6초로 튄다(2026-09-24 정기 판 6238ms, 코드는 그대로).
  // 우리 탓이 아닌 그 한 번을 빼고, 데워진 두 번째가 5초 안이면 참을 만하다고 본다.
  // 둘 다 느리면 여전히 빨갛다.
