...
s3.messages = 대화만들기(12);
/*
 * ★ 물러선 자리에서도 들고 있던 파일 내용을 버려야 한다.
 *
 * 한동안 성공한 길에만 이 처리가 있었다. 그래서 **정작 자주 지나는 길** —
 * 서버가 흔들려 요약을 못 받은 자리 — 에서만 조용히 어긋났다. 줄이기(trim)는
 * 옛 메시지를 진짜로 지우는데, 지운 뒤에도 「앞에서 읽은 그대로입니다」 를
 * 내밀면 모델은 대화에 없는 글을 가리키는 쪽지만 받는다.
 */
const 읽은것 = '     1\t첫 줄\n     2\t둘째 줄\n' + Array.from({ length: 90 }, (_, i) => `${String(i + 3).padStart(6)}\t줄 ${i + 3}`).join('\n');
s3.파일기억.실을것('/집/본것.txt', 읽은것);
const r3 = await compact(s3);
check('요약 실패해도 던지지 않음', typeof r3 === 'object');
check('실패하면 그냥 줄이기로 물러섬', r3.fallback === true && r3.after < r3.before,
  `${r3.before} → ${r3.after}`);
check('★ 그냥 줄인 뒤에도 파일은 통째로 다시 싣는다 — 가리킬 앞엣것이 없다',
...
// 때마다 이 값이 오르고, 작은 모델에서 먼저 티가 난다.
{
  ...
  //
  // 두 번째는 1.5.6 에서 도구 둘(Ask·Move)을 더할 때였다. 2,764 → 3,142 로
  // 378토큰이 늘었다. 8k 에서 재면 Ask 가 134, Move 가 193, 나머지는 규칙에
  // 붙은 세 줄이다(되묻기 · 시킨 것을 다시 묻지 마라 · 도구가 실패하면 말해라).
  //
  // 여기서도 먼저 줄였다. 설명은 **앞에 중요한 것부터** 놓았다(좁은 창에서는
  // 뒤에서부터 문장째로 잘리니까), question·from·to 는 이름만 봐도 아는
  // 인자라 뻔한인자 에 넣어 좁은 창에서 설명을 뺐다.
  //
  // 그러고도 남는 378 은 **울타리를 옮겨서** 받았다. 둘 다 없으면 8k 모델이
  // 못 하게 되는 일이 생긴다 — 갈림길에서 글로 묻고 턴을 끝내거나(그 창에서
  // 여태 읽은 것이 통째로 버려진다), 구조를 바꾸려고 Bash mv 를 파일마다
  // 부르거나(되돌리기에 안 잡힌다). 고정 몫 378 을 아끼려고 창 하나를
  // 통째로 잃는 셈이라, 작은 창일수록 오히려 밑지는 거래다.
  //
  // 울타리는 여전히 빡빡하게 둔다. 다음에 무엇을 더하든 여기서 한 번 더
  // 멈추게 하려고 그렇게 둔 것이다.
  //
  // ── 3,200 → 3,500 으로 옮겼다. **프롬프트가 는 것이 아니다.** ────────
  //
  // 재는 자(session.js 의 #원추정)가 시스템 글의 조각을 손으로 나열하고
  // 있었고, 거기서 셋이 빠져 있었다 — 셸 안내, 급말(작은 모델에 붙는
  // 문단), 그리고 사람이 못 박은 글. 그래서 이 울타리는 **늘 236토큰
  // 모자란 값**을 재고 있었다. 8k 에서 38% 라고 적혀 있던 것이 실제로는
  // 42% 였고, 남았다던 58토큰은 애초에 없었다.
  //
  // 이제 진짜 시스템 글을 통째로 재고 따로 줄을 갖는 것만 뺀다. 울타리는
  // 그 정직한 값 위에 다시 세운다 — 낮춰 두면 다음 사람이 「아직 여유가
  // 있다」 를 믿고 늘리는데, 그 여유는 처음부터 없던 것이다.
  check('고정 몫이 3,500토큰을 안 넘는다', 작은것.고정 < 3500,
   ...
   */
  {
    ...
    //
    // 400 → 430. 위 울타리와 같은 까닭이다 — 스킬 절의 **머리말**
    // (「쓸 수 있는 스킬 / 필요한 것이 있으면 Skill 도구로…」)이 여태 어느
    // 칸에도 안 세어지고 있었다. 스킬 줄만 세고 그 위 두 줄은 아무도 안
    // 셌다. 실제로 나가는 글이니 이제 세어진다.
    check('내장 스킬이 얹는 몫이 430토큰을 안 넘는다', 얹힌것 < 430,
