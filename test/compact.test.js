...
// 때마다 이 값이 오르고, 작은 모델에서 먼저 티가 난다.
{
  ...
  //
  // 두 번째는 1.5.6 에서 도구 둘(Ask·Move)을 더할 때였다. 2,764 → 3,107 로
  // 343토큰이 늘었다. 8k 에서 재면 Ask 가 134, Move 가 193, 거기에 되묻기
  // 규칙 한 줄이 더 붙는다.
  //
  ...
  //
  // 그러고도 남는 343 은 **울타리를 옮겨서** 받았다. 둘 다 없으면 8k 모델이
  // 못 하게 되는 일이 생긴다 — 갈림길에서 글로 묻고 턴을 끝내거나(그 창에서
  // 여태 읽은 것이 통째로 버려진다), 구조를 바꾸려고 Bash mv 를 파일마다
  // 부르거나(되돌리기에 안 잡힌다). 고정 몫 343 을 아끼려고 창 하나를
  // 통째로 잃는 셈이라, 작은 창일수록 오히려 밑지는 거래다.
  //
  // 울타리는 여전히 빡빡하게 둔다(43토큰 남음). 다음에 무엇을 더하든
  // 여기서 한 번 더 멈추게 하려고 그렇게 둔 것이다.
  check('고정 몫이 3,150토큰을 안 넘는다', 작은것.고정 < 3150,
