...
// ── 붙였다고 말한 자리가 실제로 붙었나 ──────────────────────────────────
{
  ...
  /*
   * ★★ 대화 이름도 **같은 세 갈래를 다** 지나간다.
   *
   * 이름이 빠지는 갈래가 하나라도 있으면 게이트웨이는 그 요청만 새 대화로
   * 잡는다. 하필 빠지기 쉬운 자리가 「열쇠를 새로 받은 뒤」 인데, 그 갈래는
   * 세 시간짜리 대화의 두 시간째에나 처음 도므로 손으로는 영영 안 걸린다.
   *
   * 값을 안 보고 **넘겼는지만** 본다 — 무엇이 실려 나가는지는 wire.test.js 가
   * 진짜로 보내 보고 잰다. 여기가 지키는 것은 「빠진 갈래가 없다」 하나다.
   */
  check('★★ 대화 이름도 어느 길로 나가도 같이 간다',
    돌려주는것.length >= 3 && 돌려주는것.every((x) => /더할머리\(conn\.kind,\s*이름\)/.test(x)),
    `${돌려주는것.filter((x) => !/더할머리\(conn\.kind,\s*이름\)/.test(x)).length}개 빠짐`);
}
