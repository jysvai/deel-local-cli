...
//
// ── 말뭉치에 든 열쇠는 하나도 진짜가 아니다 ─────────────────────────────
//
// `fixtures/secrets-corpus.txt` 의 열쇠들은 전부 자리표시자다. 구글 줄은
// `AIza` 뒤에 0‑9 를 적고 a‑v 를 순서대로 이어 붙인 것이고, 나머지도 같은
// 식이다. **모양이 진짜와 같아야 무늬를 잴 수 있어서** 그렇게 지었다.
//
// 그래서 깃허브 비밀 훑기가 이 파일을 잡는다 — 2.0.0 에 Google API Key 로
// 한 번 떴다. 잡힌 것을 하나씩 닫아도 말뭉치에 한 줄을 더하는 날 또 뜨고,
// 그러면 **진짜 경고가 가짜 사이에 묻힌다.** 그래서 `.github/secret_scanning.yml`
// 에서 `test/fixtures/**` 를 빼 두었다.
//
// 뒤집어 말하면 — **진짜 열쇠를 저 파일에 넣으면 아무도 안 잡아 준다.**
// 저기 들어가는 것은 언제나 지어낸 모양이어야 한다.
//
// 설명을 말뭉치 파일이 아니라 여기 적는 까닭: 저쪽은 **줄 수가 곧 잣대**라
// 주석 한 줄만 더해도 짝지은 masked 파일과 통째로 어긋난다. 실제로 그렇게
// 해 보고 되돌렸다(115줄 대 104줄).
