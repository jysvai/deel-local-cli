...
trace('1-설정펴기');
{
  ...
  /*
   * 세 이름(`때 · 자리 · event`) 을 다 보고 골라 놓고, 무엇이 틀렸는지 적을
   * 때는 `자리` 만 안 봤다. `{자리: '오타'}` 로 적어 둔 사람은 「모르는 자리:
   * (없음)」 을 받는다 — 제가 적은 글자가 화면에 안 나오니 어디를 고쳐야
   * 하는지 알 길이 없다.
   */
  for (const 이름 of ['때', '자리', 'event']) {
    const r = 훅펴기({ hooks: [{ [이름]: '그런자리없음', 명령: 'x' }] }, '검사');
    check(`★ 「${이름}」 에 적은 오타를 그대로 되짚어 준다`,
      r.버린것.some((x) => x.includes('그런자리없음')), JSON.stringify(r.버린것));
  }
