...
import { buildBody, 간자리 } from '../src/backend/adapter.js';
 ...
 * ── 이 요청이 **어느 자리로 갔나** ──────────────────────────────────────
 *
 * 위 검사들은 「우리가 보내는 앞머리가 바이트로 같은가」 를 잰다. 그런데 그것이
 * 전부 참인데도 캐시가 0% 가 되는 자리가 있다 — 게이트웨이가 같은 모델의 **다른
 * 자리**로 보냈을 때다. 캐시는 자리마다 따로라, 우리가 한 글자도 안 바꿔도
 * 앞머리를 통째로 다시 쓴다.
 *
 * 실제 청구서에서 그 줄을 봤다. 같은 122.4K 프리픽스가 3분 41초 만에 다시
 * 나갔고, 그 한 번이 그날 제일 비싼 요청이었다. 화면에는 아무 말도 안 났다.
 *
 * 그래서 서버가 말해 줄 때만 읽는다. 없으면 아무 말도 안 한다 — 모르는 것을
 * 「안 바뀌었다」 로 적지 않는다.
 */
{
  const 머리 = (표) => new Headers(표);

  check('★★ 서버가 알려 준 자리를 읽는다',
    간자리(머리({ 'x-litellm-model-id': 'abc-123' })) === 'abc-123', '');
  /*
   * ── 그럴듯한 일반형은 **일부러 안 읽는다** ──────────────────────────
   *
   * 세션 이름을 내보낼 때는 아는 이름을 다 단다 — 머리는 모르면 무시되니
   * 이름이 늘어도 잃을 것이 없다. 여기는 반대다. **받아서 읽는** 자리라,
   * 이름 하나를 잘못 고르면 없는 일을 있다고 말하게 된다.
   *
   *   x-deployment-id       일반 웹·컨테이너 인프라가 배포 해시로 쓴다.
   *                         모델과 상관없이 배포할 때마다 바뀌므로, 멀쩡한
   *                         캐시를 두고 「자리가 바뀌었다」 를 찍는다.
   *   x-ms-deployment-name  Azure 의 **논리** 이름이라 물리 자리가 바뀌어도
   *                         그대로다. 있는데 없다고 말하는 쪽이라 더 나쁘다.
   *
   * 틀린 경고는 진짜 경고까지 안 읽게 만든다. 그래서 안 읽는 것도 약속이고,
   * 약속이면 재야 한다 — 안 그러면 다음 사람이 「이것도 넣지」 하고 넣는다.
   */
  check('★★ 배포 해시처럼 딴 뜻으로 쓰이는 이름은 안 읽는다',
    간자리(머리({ 'x-deployment-id': 'v1234' })) === null, '');
  check('★★ 물리 자리가 바뀌어도 안 변하는 이름은 안 읽는다',
    간자리(머리({ 'x-ms-deployment-name': 'kr-1' })) === null, '');
  /*
   * 안 알려 주면 **없다**고 답해야 한다. 여기서 빈 글자나 'unknown' 같은 것을
   * 지어내면 루프가 그것을 자리 이름으로 알고, 다음 요청에서 「자리가 안
   * 바뀌었다」 고 판단한다 — 모르는 것이 아는 것으로 둔갑하는 자리다.
   */
  check('★★ 안 알려 주면 없다고 답한다', 간자리(머리({ 'content-type': 'application/json' })) === null, '');
  check('★ 머리가 아예 없어도 안 터진다', 간자리(null) === null && 간자리(undefined) === null, '');
  check('★ 빈 값은 없는 것으로 친다', 간자리(머리({ 'x-litellm-model-id': '   ' })) === null, '');

  /*
   * 같은 응답을 두 번 봐도 **같은 값**이 나와야 한다.
   *
   * 루프는 이 값을 지난 턴 것과 견줘서 「자리가 바뀌었다」 를 판단한다.
   * 읽을 때마다 값이 흔들리면 안 바뀐 자리를 바뀌었다고 적고, 그러면
   * 걸음마다 경고가 떠서 곧 아무도 안 읽는다.
   */
  const 한번 = 머리({ 'x-litellm-model-id': 'abc-123', 'x-request-id': 'r1' });
  check('★★ 같은 머리를 두 번 읽으면 같은 값이다', 간자리(한번) === 간자리(한번), '');
  check('★ 상관없는 머리는 안 섞인다', 간자리(한번) === 'abc-123', String(간자리(한번)));
}
