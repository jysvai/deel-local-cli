...

{
   ...
   * ── 여기 `!key.meta` 가 있었다 ─────────────────────────────────────
   *
   * 그래서 이 갈래는 **한 번도 안 돌았다.** node 의 readline 은 홀로 온
   * ESC 를 `{ name: "escape", meta: true }` 로 준다 — 다른 키의 앞머리인지
   * 아닌지를 시한(escapeCodeTimeout)으로 가린 뒤에도 meta 는 참으로 둔다.
   * 즉 맨 ESC 일수록 meta 가 참이다. `!key.meta` 는 맨 ESC 를 걸러내는
   * 조건이었던 셈이라, 사람이 아무리 눌러도 아무 일도 안 났다.
   *
   * 검사가 그걸 못 잡은 까닭이 여기다 — **틀린 조건을 그대로 베껴** 못
   * 박아 두었다. 소스 글자를 견주는 검사는 이렇게 틀린 것을 지키는 쪽으로
   * 돌아설 수 있다. 그래서 지금은 두 가지를 나눠 본다.
   *   · ctrl·shift 가 섞인 ESC 는 안 받는다 (여기서 재는 것)
   *   · meta 는 보면 안 된다 (그게 맨 ESC 다)
   * 진짜로 멈추는지는 ask.test.js 가 실제 ESC 바이트를 넣어서 잰다.
   */
  check('맨 ESC 일 때만 본다', /!key\.ctrl/.test(줄들[자리] ?? '') && /!key\.shift/.test(줄들[자리] ?? ''),
    줄들[자리]?.trim() ?? '');
  check('★ meta 는 안 본다 — 맨 ESC 가 바로 meta:true 다', !/key\.meta/.test(줄들[자리] ?? ''),
    줄들[자리]?.trim() ?? '');
}
