 ...
 */
{
   ...
   * ── 몸통은 **한 자리에서만** 지어진다 ────────────────────────────────
   *
   * 예전에는 chat 과 chatStream 이 각자 buildBody 를 불렀고, 이 검사는 그 둘이
   * 같은 것을 넘기는지 봤다. 그런데 「같은 것을 두 번 적는다」 는 언젠가 한
   * 쪽만 고쳐지는 모양이다 — 실제로 전선 카드를 붙이면서 그럴 뻔했다.
   *
   * 지금은 몸만들기() 하나가 짓고 두 길이 그것을 부른다. 그래서 재는 것도
   * 바뀐다. 「둘이 같은가」 가 아니라 **「지을 수 있는 자리가 하나뿐인가」** 다.
   * 이쪽이 더 센 조건이다 — 어긋날 자리 자체가 없다.
   */
  const 몸통자리 = [...ad.matchAll(/buildBody\(conn\.kind, \{([\s\S]*?)\n  \}\)/g)].map((m) => m[1]);
  check('★ 몸통을 짓는 자리가 하나뿐이다', 몸통자리.length === 1, `${몸통자리.length}자리`);
  check('★ 그 자리가 다듬은 목록을 쓴다',
    몸통자리.length === 1 && /tools: 맞춘것\.tools/.test(몸통자리[0]),
    몸통자리.length ? '있음' : '없음');
  check('★ 그 자리가 회사를 같이 넘긴다',
    몸통자리.length === 1 && /회사: 벤더\(conn\)/.test(몸통자리[0]),
    몸통자리.length ? (/회사:/.test(몸통자리[0]) ? 'o' : 'x') : '없음');
  /*
   * ★ 전선 카드와 세션 이름도 그 자리에서 같이 실린다.
   *
   * 부르는 자리마다 손으로 넘기게 두면 언젠가 한 곳이 빠지고, 그 한 곳만
   * 캐시가 안 걸린다 — 요약을 만드는 부름(agent/compact.js)이 딱 그런 자리다.
   */
  check('★ 그 자리가 전선 카드를 같이 넘긴다',
    몸통자리.length === 1 && /카드:/.test(몸통자리[0]) && /세션이름:/.test(몸통자리[0]),
    몸통자리.length ? '있음' : '없음');

  // 그리고 두 길이 실제로 그 자리를 지난다.
  // 정의 한 줄은 빼고 **부르는** 자리만 센다.
  const 부르는자리 = [...ad.matchAll(/= 몸만들기\(conn, opts, 맞춘것/g)];
  check('★ 보내는 길 둘이 다 그 자리를 지난다', 부르는자리.length === 2, `${부르는자리.length}자리`);
