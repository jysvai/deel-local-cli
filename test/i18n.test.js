...
// 그 말은 영영 화면에 안 나오는데, 파일에는 있으니 누군가 계속 손본다.
{
   ...
   */
  /*
   * ★★★ 뽑는 자리를 여기서 또 적으면 안 된다.
   *
   * 여기 적혀 있던 것은 `[^{}s:]` 였다 — 백슬래시 하나가 빠져서 **빈칸이
   * 아니라 글자 `s` 를 뺐다.** 그래서 `{turns}` · `{files}` 처럼 이름에 s 가
   * 든 자리를 통째로 못 읽었고, 네 말 다 「자리 0개」 라 완벽한 짝이 됐다.
   *
   * 옮기면서 `{files}` 를 빠뜨려도 이 검사는 초록이었다. 화면에는 숫자가
   * 소리 없이 사라진 문장이 나간다 — 「파일 個を元に戻しました」.
   *
   * 재는 쪽과 쓰는 쪽이 다른 무늬를 보면 검사는 무엇이든 통과시킨다.
   * 그래서 제품이 쓰는 그 함수를 그대로 부른다.
   */
  const 자리뽑기 = (글) => 자리이름들(글).join(',');
  for (const l of 언어들.filter((x) => x !== 'ko')) {
  ...
  for (const l of ['ja', 'zh']) {
    ...
    if (없는열쇠) {
      ...
      /*
       * 물러날 자리가 없으면 위 검사는 잴 것이 없다. 그렇다고 `check(…, true)`
       * 를 두면 통과 건수만 부풀리는 늘 초록인 줄이 된다 — 1.9.2 에서 그런 줄을
       * 걷어낸 이유가 그거다. 대신 그 말이 참인지를 잰다: 아무 열쇠나 골라도
       * 실제로 **그 언어 표**에서 나와야 한다.
       */
      const 표 = 표보기(l);
      const 아무거나 = Object.keys(ko).find((k) => 표[k] && 표[k] !== en[k] && 표[k] !== ko[k]);
      check(`${l}: 전부 옮겨져 있어 물러날 자리가 없다`, 말(아무거나) === 표[아무거나],
        `${아무거나} → ${말(아무거나)}`);
    }
  }
  언어정하기('ko');
 ...
 * ── 세는 말 ─────────────────────────────────────────────────────────────
 *
 * 도구 결과 한 줄(`└ 1개 파일 · 3건 · rg`)은 이 조각들을 이어 만든다.
 * 지키는 것이 둘이다.
 *
 *   1) **홑복수.** 한국어는 「1개 파일」 로 끝나지만 영어에서 `1 files` 는
 *      그 줄 전체를 대충 만든 것으로 보이게 한다. 한국어만 보고 검사하면
 *      이건 영영 안 잡힌다 — 한국어는 아무 문제가 없기 때문이다.
 *   2) **자릿점.** 부르는 쪽마다 toLocaleString() 을 하면 어디선가 빠져서
 *      `50700` 과 `50,700` 이 한 화면에 같이 뜬다. 한 자리에서만 한다.
 */
{
  언어정하기('ko');
  check('한국어: 하나여도 같은 말', 세말('files', 1) === '1개 파일', 세말('files', 1));
  check('한국어: 여럿', 세말('files', 3) === '3개 파일', 세말('files', 3));
  check('★ 자릿점을 여기서 붙인다', 세말('lines', 50700) === '50,700줄', 세말('lines', 50700));

  언어정하기('en');
  check('★ 영어: 하나면 홑', 세말('files', 1) === '1 file', 세말('files', 1));
  check('★ 영어: 여럿이면 복수', 세말('files', 3) === '3 files', 세말('files', 3));
  const 홑들 = ['files', 'hits', 'lines', 'spots', 'places', 'count'].map((g) => 세말(g, 1));
  check('★ 영어: 어느 갈래든 `1 …s` 가 안 나온다',
    !홑들.some((x) => /^1 \w+s$/.test(x)), 홑들.join(' / '));

  // 홑을 안 가르는 갈래는 열쇠를 늘리지 않는다. 늘렸다면 표에 짝이 있어야 한다.
  check('홑이 따로 없는 갈래도 안 깨진다', 세말('sec', 1) === '1s' && 세말('chars', 1) === '1 chars',
    `${세말('sec', 1)} / ${세말('chars', 1)}`);
  // 숫자가 아닌 것이 와도 안 죽는다. 화면 말 하나 때문에 대화가 끊기면 안 된다.
  check('숫자가 아니어도 안 죽는다', typeof 세말('files', '?') === 'string', 세말('files', '?'));
  언어정하기('ko');
}
...
// 안 세는 게 잘못이다. 숫자를 화면에 적어 두면 다음 사람이 어디를 도울지 안다.
{
  ...
  // 일본어·중국어도 같이 적어 둔다. 안 적으면 「영어는 100%」 한 줄만 보고
  // 다 된 줄 안다 — 실제로는 여기가 24개 비어 있었고 아무도 못 봤다.
  for (const l of ['ja', 'zh']) {
    const q = 옮긴만큼(l);
    적어둘것.push(`${l === 'ja' ? '일본어' : '중국어'}: ${q.옮김}/${q.전체} 마디 (${Math.round((q.옮김 / q.전체) * 100)}%)`
      + (q.남음 ? ` — 아직 ${q.남음}개: ${q.안옮긴열쇠.slice(0, 6).join(', ')}${q.남음 > 6 ? ' …' : ''}` : ''));
  }
