...
async function 귀막힘(일) {
  ...
  return { 틈: Math.max(0, 제일큰틈), 박동 };
}

/** 틈만 필요한 자리를 위한 얇은 껍데기. */
async function 귀막힌시간(일) { return (await 귀막힘(일)).틈; }
...
// 우리 도구는 그걸 쓰면 안 된다 — 오래 도는 동안에도 ESC 가 들려야 한다.
{
  ...
  /*
   * ── 시간이 아니라 **박동 수**를 잰다 (18회차) ─────────────────────────
   *
   * 전에는 「제일 큰 틈이 150ms 보다 작아야 한다」 였다. 세 번 재서 최솟값을
   * 쓰는 완충까지 달았는데도 조각으로 돌린 판에서 이 파일이 혼자 빨개졌다 —
   * **같은 파일을 단독으로 네 번 돌리면 네 번 다 초록이었다.** 재는 것이
   * 「이 PC 가 300ms 안에 우리를 몇 번 깨워 주나」 였기 때문이다. 메모리가
   * 모자란 기계에서는 OS 가 안 깨워 주고, 그러면 고쳐야 할 코드가 없는데도
   * 빨간불이 켜진다. **헛경고도 흠이다.**
   *
   * 아래 6번은 이미 이 파일 안에서 「시간이 아니라 횟수를 잰다」 로 고쳐 놨다.
   * 5번만 벽시계로 남아 있었다. 같은 잣대로 맞춘다.
   *
   * 그리고 **막는 쪽을 같은 판에서 같이 잰다.** 절대 숫자를 못 박으면 그 숫자가
   * 다시 기계를 재게 된다. 스위치를 가르는 것은 「막느냐 안 막느냐」 이지
   * 「몇 번 뛰었느냐」 가 아니다 — 막으면 0에 붙고, 안 막으면 자식이 사는
   * 300ms 내내 16ms 마다 뛴다. 두 수를 나란히 놓으면 느린 기계에서도 갈린다.
   *
   * ── 다리가 **둘**인 까닭 ──────────────────────────────────────────────
   *
   * 박동만 세면 **내내** 막는 것만 잡는다. spawn 앞에 300ms 를 심어 놓고 그
   * 뒤는 제대로 기다리면, 자식이 사는 동안 심장은 멀쩡히 뛰어서 박동이 안
   * 줄어든다 — 어긋 하나를 일부러 만들어 돌려 보니 정말 샜다. 그런데 이 파일
   * 머리말에 적힌 옛 사고가 바로 그 모양이었다(「spawn 앞에 300ms 를 심어 보니
   * 검사가 안 빨개졌다」). 박동으로 갈아타면서 그 구멍을 되살릴 뻔했다.
   *
   * 그래서 **틈도 같이 본다.** 다만 옛날처럼 150ms 라고 못 박지 않고 막는 쪽의
   * 틈에 견준다. 기계가 느려지면 두 수가 같이 커지므로 잣대가 안 흔들린다.
   */
  const { spawnSync: 막는부름 } = await import('node:child_process');
  const 막는쪽 = await 귀막힘(async () => {
    막는부름(process.execPath, [느린것], { encoding: 'utf8', windowsHide: true });
  });

  const 잰것들 = [];
  for (let i = 0; i < 3; i++) {
    잰것들.push(await 귀막힘(async () => {
      await runTool('Bash', { command: `"${process.execPath}" "${느린것}"`, timeout: 5000 }, 판(방));
    }));
  }
  const 제일많이뛴것 = Math.max(...잰것들.map((x) => x.박동));
  const 적은틈 = Math.min(...잰것들.map((x) => x.틈));
  const 잰말 = `비동기 박동 ${잰것들.map((x) => x.박동).join('·')} · 틈 ${잰것들.map((x) => x.틈).join('·')}ms`
    + ` / 동기 박동 ${막는쪽.박동} · 틈 ${막는쪽.틈}ms`;
  check('★ 오래 도는 도구가 도는 동안 심장이 뛴다 (내내 막으면 0에 붙는다)',
    제일많이뛴것 >= 막는쪽.박동 + 3, 잰말);
  check('★ 그리고 중간에 잠깐씩도 안 막는다 (막는 쪽 틈의 절반 아래)',
    적은틈 * 2 < 막는쪽.틈, 잰말);
