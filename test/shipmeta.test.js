...
/*
 * ── README 의 명령표가 지금 있는 명령을 다 적었나 ───────────────────────
 *
 * 세 개가 빠져 있었다 — `/review` · `/paste` · `/quit`.
 *
 * 셋 다 만들어 두고 화면에서 잘 도는 것들이다. `/review` 는 바꾼 것을 새
 * 창에서 다시 보는 자리고, `/paste` 는 캡처를 붙이는 자리다. 그런데 README
 * 어디에도 없으니, README 만 읽고 오는 사람에게는 **없는 기능**이다.
 * 만든 값이 통째로 0 이 되는 셈이다.
 *
 * 화면의 `/help` 는 cmdnames.js 에서 저절로 나오므로 늘 맞다. 그래서 어긋난
 * 것을 아무도 못 본다 — 쓰는 사람은 다 보이고, 안 쓰는 사람은 있는 줄도
 * 모른다. 어긋나는 방향이 언제나 한쪽이라 더 안 들킨다.
 *
 * 이름만 본다. 설명까지 맞추라고 하면 문구를 다듬을 때마다 검사가 빨개진다.
 */
{
  ...
  /*
   * 폴더 구조 설명에 적어 둔 **숫자**도 본다. 「명령 49종」 이라고 적혀 있는
   * 동안 실제로는 52종이었다. 세 개가 늘 때까지 아무도 안 틀렸다고 말해
   * 주지 않았다 — 검사 배지 때와 똑같은 자리다.
   */
  for (const [파일, 무늬] of [
    ['docs/ko/develop.md', /명령 (\d+)종/],
    ['docs/en/develop.md', /(\d+) slash commands/],
  ]) {
    const m = 읽기(파일).match(무늬);
    check(`★★ ${파일} 의 명령 수가 맞다`, m?.[1] === String(이름들.length),
      `문서 ${m?.[1] ?? '(못 찾음)'} vs 실제 ${이름들.length}`);
  }
}
