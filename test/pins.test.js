...
// ── 못 박고 빼기 ────────────────────────────────────────────────────────
{
  ...
  // 맥에서 친 한글은 자모로 풀린 꼴(NFD)로 오기도 한다 — 보기에는 같은 말이다 (2.0.2 · 429).
  const 풀린꼴 = '운영 DB 는 건드리지 마라'.normalize('NFD');
  const 또또 = p.더하기(풀린꼴);
  check('★ 자모로 풀린 꼴(NFD)도 같은 말로 본다', 풀린꼴 !== '운영 DB 는 건드리지 마라' && !또또.ok && p.목록().length === 2, JSON.stringify(또또));
