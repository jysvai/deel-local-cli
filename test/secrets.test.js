...
// 모델은 값이 어디서 오는지를 몰라 생각보다 쉽게 박아 넣는 쪽으로 간다.
{
  ...
  /*
   * ── 이름은 대문자만이 아니다 (1.20.0) ───────────────────────────────
   *
   * `.env` 는 대문자가 관례라 대문자 이름만 봐도 됐는데, 코드와 JSON·YAML
   * 설정은 아니다. 진짜 열쇠가 `apiKey: 'sk-…'` 로 적혀 평문으로 나갔다.
   *
   * 낱말 사이가 **끊겨 있어야** 한다 — `_key` 아니면 `Key`. 그냥 소문자
   * `key` 로 끝나는 것까지 받으면 `monkey` · `hotkey` 가 걸린다.
   */
  for (const [글, 사라져야] of [
    ["apiKey: 'sk-진짜열쇠-1234'", 'sk-진짜열쇠-1234'],
    ['api_key: "prod.auth.secret"', 'prod.auth.secret'],
    ["accessToken: 'ya29.abcdefgh'", 'ya29.abcdefgh'],
    ["clientSecret: 'abcdef123456'", 'abcdef123456'],
    ['db_password: "hunter2hunter2"', 'hunter2hunter2'],
  ]) {
    const r = 가리기(글);
    check(`★★★ 낙타등·밑줄 이름도 비밀 이름이다 — ${글}`,
      !r.글.includes(사라져야) && r.글.includes('«가림:환경변수»'), r.글);
  }
  // 낱말 사이가 안 끊긴 이름은 안 본다 — 안 그러면 원숭이가 걸린다.
  for (const 글 of ["monkey: 'brown-monkey'", "hotkey: 'ctrl+shift+s'", "turkey: 'roasted12'"]) {
    check(`★★★ 끊기지 않은 이름은 비밀 이름이 아니다 — ${글}`,
      가리기(글).글 === 글, 가리기(글).글);
  }
  /*
   * 치르는 값: 비밀이 아닌 낙타등 이름도 같이 가려진다. 이름만으로는
   * `cacheKey` 와 `apiKey` 가 안 갈린다. 줄이 뭉개지는 편이 낫다(1.19.0).
   * 이 줄이 빨개지면 무엇으로 갈랐는지 확인하고 위 검사를 같이 다시 봐야 한다.
   */
  for (const [글, 사라져야] of [["cacheKey: 'user:12345'", 'user:12345']]) {
    const r = 가리기(글);
    check(`★★ 비밀 아닌 낙타등 이름도 같이 가려진다 — ${글}`,
      !r.글.includes(사라져야) && r.글.includes('«가림:환경변수»'), r.글);
  }
  /*
   ...
   * 보면 `const API_KEY = 설정.비밀번호;` 가 가려진다 — 제 코드부터 뭉갰다.
   */
  for (const 글 of [
    'const API_KEY = 설정.비밀번호;',
    'const API_TOKEN = 설정.열쇠고리.토큰;',
    'API_KEY: 설정.열쇠;',
  ]) {
    check(`★★★ 한글 이름씨도 코드가 가리키는 이름이다 — ${글}`,
      가리기(글).글 === 글, 가리기(글).글);
  }
  // 그렇다고 설정 줄까지 풀리면 안 된다 — 코드의 표시가 없으면 가린다.
  {
    const r = 가리기('API_KEY: 설정.열쇠');
    check('★★★ 한글이어도 설정 줄이면 가린다 — API_KEY: 설정.열쇠',
      r.글.includes('«가림:환경변수»'), r.글);
  }
  /*
   * ── 따옴표 안의 `#` 은 주석이 아니다 (16차 리뷰) ────────────────────
   *
   * `#` 앞 빈칸을 요구하면 `API_KEY=값#(운영)` 이 새고, 안 요구하면 글자열
   * 안의 `#` 부터 잘려 뒤쪽 `;` 이 사라진다. 따옴표를 세면서 걷는다.
   */
  {
    const r = 가리기('API_KEY=prod.v1.secret#(운영)');
    check('★★★ 빈칸 없이 붙인 # 도 주석이다', !r.글.includes('prod.v1.secret'), r.글);
  }
  for (const 글 of [
    'msg = "a # b"; x.API_KEY = config.apiKey;',
    "msg = 'a # b'; API_KEY = config.apiKey;",
    'x.API_KEY = obj.#몰래칸;',
  ]) {
    check(`★★★ 따옴표 안의 # 은 안 뗀다 — ${글}`, 가리기(글).글 === 글, 가리기(글).글);
  }
  /*
   * ── `\r` 만 쓰는 글 (16차 리뷰) ──────────────────────────────────────
   *
   * 줄바꿈을 `\n` 으로만 찾으면 파일 통째가 한 줄이 되고, 저 아래 어딘가의
   * `;` 하나 때문에 이 줄이 코드로 뒤집힌다.
   */
  {
    const r = 가리기('API_KEY: tok.v1.abc\rconst x = 1;');
    check('★★★ \\r 만 써도 줄을 가른다', r.글.includes('«가림:환경변수»'), JSON.stringify(r.글));
  }
  /*
