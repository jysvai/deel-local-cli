...
import { 요청항목들, 빠진것, 빠졌다는말, 알맹이낱말, 조건줄인가 } from '../src/agent/asks.js';
...
// 해도 무엇이 남았는지 모른다.
{
  ...
  // (사냥5 B5-01) 잘라 놓고 머리말은 「원문 그대로 · 빠짐없이」 였다. 모델은 앞 1,200자를
  // 시킨 일 전부로 믿고 끝까지 했고, 배치는 0 으로 끝났다.
  check('★★ 잘랐으면 「원문 그대로 · 빠짐없이」 라고 하지 않는다', !/원문 그대로/.test(긴것) && !/빠짐없이/.test(긴것),
    긴것.split('\n')[0]);
  check('  잘렸다고 머리말에서 먼저 밝힌다', /잘/.test(긴것.split('\n')[0]), 긴것.split('\n')[0]);
}
...
// ══ 영문·숫자 뒤에 붙은 조사 ═══════════════════════════════════════════
//
// ★ (6회차 시킨것6au-a AS2) 「README를 수정해줘」 의 알맹이는 `readme를` 한 낱말이 되고, 줄여 보기는
// 한글로 시작할 때만 해서 붙은 조사를 못 뗐다. README.md 를 고치고 「README 와 CHANGELOG
// 고쳤습니다」 라고 보고해도 **둘 다 안 했다**고 되밀었다 — 머리말이 제일 나쁘다고 한 쪽이다.
{
  const 빠진글 = (요청, 자국) => 빠진것({ 요청, 자국 }).map((x) => x.글);
  let r = 빠진글('README를 수정해줘\nCHANGELOG를 정리해줘', ['README.md', 'CHANGELOG.md', 'README 와 CHANGELOG 고쳤습니다']);
  check('★ (6회차 AS2) README를 · CHANGELOG를 — 고친 것을 안 했다고 안 한다', r.length === 0, JSON.stringify(r));
  r = 빠진글('API를 붙여줘\n로그인 화면 고쳐줘', ['src/api.js', '로그인 화면 고침', 'API 붙임']);
  check('★ (6회차 AS2) API를 — 붙인 것을 안 했다고 안 한다', r.length === 0, JSON.stringify(r));
  r = 빠진글('3번을 고쳐줘\n5번을 지워줘', ['3번 고침 · 5번 지움']);
  check('★ (6회차 AS2) 3번을 · 5번을 — 한 것을 안 했다고 안 한다', r.length === 0, JSON.stringify(r));
  r = 빠진글('README를 수정해줘\nCHANGELOG를 정리해줘', ['README.md', 'README 고쳤습니다']);
  check('(AS2 짝) 정말 안 한 CHANGELOG 는 여전히 잡는다', r.length === 1 && /CHANGELOG/.test(r[0]), JSON.stringify(r));
  // 조사만 뗀다 — 영문 머리까지 줄이면 cli·deel 처럼 짧은 말이 아무 데나 걸린다(머리말 76).
  r = 빠진글('config를 고쳐줘\n로그인 고쳐줘', ['con 이라는 글 · 로그인 고침']);
  check('(AS2 짝) 영문 머리는 안 줄인다 — config를 이 con 에 안 닿는다', r.length === 1 && /config/.test(r[0]), JSON.stringify(r));

  // ★ (6회차 AS3) 이모지 꼴 가림표가 붙은 ▪️ 줄머리. 표에 ▪️ 를 넣어 둔 뜻과 달리 가림표가
  // 빈칸 앞에 끼어 안 맞았고, 항목이 0개라 대조가 통째로 안 돌았다.
  const 네모 = '▪' + String.fromCodePoint(0xfe0f);
  const 항목 = 요청항목들(`${네모} 작업 목록 만들기\n${네모} 배포 문서 쓰기`);
  check('★ (6회차 AS3) ▪️ 줄머리도 항목으로 잡는다', 항목.length === 2 && 항목[0]?.글 === '작업 목록 만들기', JSON.stringify(항목));
}

// ══ 줄 끝이 「…지 마」 인 금지 ═════════════════════════════════════════
//
// ★ (6회차 시킨것6au-b AS6) 금지 동사를 목록(하지·고치지·건드리지 …)으로만 봐서 「손대지 마」 ·
// 「건들지 마세요」 · 「지우지 말 것」 은 할 일로 잡혔다. 지킨 금지는 자국이 안 남으니
// 「시킨 것 중 1가지에 아직 손을 안 댔습니다: 기존 파일은 손대지 마. 지금 하세요」 가 나갔다 —
// 다 한 것을 안 했다고 하는 데다, 하지 말라는 것을 하라고 되민다.
{
  const 빠진글 = (요청, 자국) => 빠진것({ 요청, 자국 }).map((x) => x.글);
  for (const 금지 of ['기존 파일은 손대지 마', '설정 파일은 건들지 마세요', '옛 기록은 지우지 말 것', '배포 설정은 바꾸지 마라']) {
    const r = 빠진글(`1. 로그인 기능 만들어줘\n2. ${금지}`, ['로그인 기능 구현 완료']);
    check(`★ (6회차 AS6) 「${금지}」 를 안 했다고 되밀지 않는다`, r.length === 0, JSON.stringify(r));
  }
  // 줄 가운데의 「잊지 말고」 는 금지가 아니라 할 일이다 — 그대로 잡아야 한다.
  const r = 빠진글('로그인 기능 만들어줘\n잊지 말고 테스트도 돌려줘', ['로그인 기능 구현 완료']);
  check('(AS6 짝) 「잊지 말고 테스트도 돌려줘」 는 여전히 할 일로 잡는다', r.length === 1 && /테스트/.test(r[0]), JSON.stringify(r));
  check('(AS6 짝) 줄 가운데 「잊지 말고」 는 조건줄이 아니다', 조건줄인가('잊지 말고 테스트 돌려줘') === false);
}
...
//
// 「최대 5개까지만」 은 1번에 붙은 조건이지 따로 할 일이 아니다. 지킨 것을
// 안 했다고 말한 것이고, 이 검사 맨 위에 적어 둔 「다 한 것을 안 했다고 안
// 하나」 를 정면으로 어긴 자리다. 한 번 이러면 그 뒤로 아무도 안 믿는다.
trace('조건줄');
{
  const 진짜있었던말 = [
    '이 프로젝트의 공동작업 기능을 분석해서 동시 편집 시 데이터 손실이 날 부분을 찾아줘.',
    '각 문제마다 관련 파일과 함수, 발생 조건, 원인을 설명해줘.',
    '코드는 수정하지 마.',
    '최대 5개까지만 찾아줘.',
  ].join('\n');

  const 항목 = 요청항목들(진짜있었던말);
  check('★ 개수 제한은 할 일이 아니다', !항목.some((x) => /최대 5개/.test(x.글)),
    항목.map((x) => x.글.slice(0, 14)).join(' | '));
  check('★ 하지 말라는 말도 할 일이 아니다', !항목.some((x) => /수정하지 마/.test(x.글)),
    항목.map((x) => x.글.slice(0, 14)).join(' | '));
  check('진짜 시킨 둘은 그대로 남는다', 항목.length === 2, `${항목.length}개`);

  // ★ 그리고 그 말이 실제로 안 나간다 — 자국에 '분석·설명' 만 남겨도 조용해야 한다.
  const 빠진 = 빠진것({ 요청: 진짜있었던말, 자국: ['공동작업 분석 결과를 파일·함수와 함께 설명했습니다'] });
  check('★★ 다 한 판에서는 아무 말도 안 한다', 빠진.length === 0,
    빠진.map((x) => x.글.slice(0, 20)).join(' | '));

  // 반대쪽 — 진짜 빠뜨린 것은 여전히 잡아야 한다. 느슨해지기만 하면 검사가 죽는다.
  const 진짜빠진 = 빠진것({
    요청: '로그인 화면 고쳐줘.\n엑셀 내보내기도 만들어줘.',
    자국: ['로그인 화면을 고쳤습니다'],
  });
  check('★★ 진짜 빠진 것은 그대로 잡는다', 진짜빠진.length === 1 && /엑셀/.test(진짜빠진[0].글),
    진짜빠진.map((x) => x.글).join(' | '));

  // 수가 들어 있다고 다 조건인 것은 아니다. 가르는 자리는 **남는 알맹이**다.
  check('수가 붙은 진짜 할 일은 안 걸린다', !조건줄인가('테스트 케이스 최대 10개 만들어줘'));
  check('맨 동사와 수만 남으면 조건이다', 조건줄인가('3개만 골라줘'));
  check('금지는 조건이다', 조건줄인가('코드는 건드리지 마세요'));
  check('영어 금지도 조건이다', 조건줄인가('do not change the tests'));
  check('알맹이가 있으면 조건이 아니다', !조건줄인가('README 도 같이 고쳐줘'));
}

// ══ 8회차 판정 — without · 줄 가운데 금지 · 1글자 머리 ═══════════════════
trace('8회차');
{
  const 빠진글 = (요청, 자국) => 빠진것({ 요청, 자국 }).map((x) => x.글);

  /*
   * ★ `without` 은 금지가 아니라 **단서**다. 「Deploy the app without downtime」 은
   * 하라는 말인데 조건줄로 걸려 목록에서 빠졌고, 남은 항목이 하나뿐이라 대조가
   * 통째로 안 돌았다 — 두 번째 줄을 안 해도 아무 말이 안 나간다.
   */
  const 영문 = '1. Deploy the app without downtime.\n2. Update the README.';
  check('★ (8회차) without 은 금지가 아니다', 조건줄인가('Deploy the app without downtime.') === false);
  check('★ (8회차) without 한 줄이 목록을 통째로 비우지 않는다', 요청항목들(영문).length === 2,
    JSON.stringify(요청항목들(영문).map((x) => x.글)));
  check('★ (8회차) 그래서 안 한 것이 잡힌다', 빠진글(영문, ['Updated the README']).length === 1,
    JSON.stringify(빠진글(영문, ['Updated the README'])));
  check('  (8회차 짝) do not · never 는 여전히 금지다',
    조건줄인가('do not change the tests') && 조건줄인가('never touch the config'));

  /*
   * ★ 줄 가운데 「고치지 말고 …」 는 **그 앞까지만** 금지다. 뒤에 붙은 것은 할 일이다.
   * 줄 전체를 금지로 보면 「새 테마 파일을 만들어줘」 가 사라지고, 그 줄 하나가
   * 빠지면서 항목이 둘 미만이 되어 목록이 통째로 빈다.
   */
  const 말고 = '기존 스타일은 고치지 말고 새 테마 파일을 만들어줘.\n로그인 화면도 손봐줘.';
  check('★ (8회차) 줄 가운데 「고치지 말고」 뒤의 할 일은 남는다',
    조건줄인가('기존 스타일은 고치지 말고 새 테마 파일을 만들어줘.') === false);
  check('★ (8회차) 그래서 목록이 통째로 비지 않는다', 요청항목들(말고).length === 2,
    JSON.stringify(요청항목들(말고).map((x) => x.글)));
  check('★ (8회차) 안 만든 테마 파일이 잡힌다', 빠진글(말고, ['로그인 화면 손봄']).length === 1,
    JSON.stringify(빠진글(말고, ['로그인 화면 손봄'])));
  check('  (8회차 짝) 뒤에 할 일이 없으면 여전히 금지다', 조건줄인가('기존 코드는 고치지 말고.') === true);
  check('  (8회차 짝) 「…하지 마」 로 끝나면 여전히 금지다', 조건줄인가('코드는 수정하지 마.') === true);
  check('  (8회차 짝) 「아무것도 건드리지 말고 그냥 둬」 도 금지다', 조건줄인가('아무것도 건드리지 말고 그냥 둬') === true);

  /*
   * ★ 영문·숫자 머리가 **1글자**면 조사를 뗄 고리가 한 번도 안 돌았다 —
   * `Math.max(머리.length, 2)` 가 문턱이라 「7을」 은 n=1 에서 시작도 못 했다.
   * 「이슈 7을 닫아줘」 를 닫고 「이슈 7 닫았습니다」 라고 보고해도 안 했다고 되밀었다.
   */
  const 한글자머리 = '이슈 7을 닫아줘\n로그인 화면 고쳐줘';
  check('★ (8회차) 머리가 1글자여도 조사를 뗀다 — 한 것을 안 했다고 안 한다',
    빠진글(한글자머리, ['이슈 7 닫았습니다 · 로그인 화면 고침']).length === 0,
    JSON.stringify(빠진글(한글자머리, ['이슈 7 닫았습니다 · 로그인 화면 고침'])));
  check('  (8회차 짝) 정말 안 닫았으면 여전히 잡는다',
    빠진글(한글자머리, ['로그인 화면 고침']).length === 1,
    JSON.stringify(빠진글(한글자머리, ['로그인 화면 고침'])));
  check('  (8회차 짝) 영문 머리는 여전히 안 줄인다',
    빠진글('config를 고쳐줘\n로그인 고쳐줘', ['con 이라는 글 · 로그인 고침']).length === 1);
}
