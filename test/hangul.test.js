...
// 칸이 하나씩 밀린다 — 표가 통째로 어긋난다.
{
   ...
   * ── 그림글자 · 안 보이는 글자 ─────────────────────────────────────────
   *
   * 입력 상자와 접어쓰기가 이 값으로 줄 수를 센다. 모자라게 세면(✅ 를 한 칸) 줄이 터미널보다
   * 넓어져 터미널은 두 줄로 접고 상자는 한 줄로 센다 — 위쪽 대화가 갉여 나간다. 넘치게 세도
   * (ZWJ 가족 8칸) 커서가 엉뚱한 칸에 선다. 보이지 않는 글자는 코드값으로 만든다.
   */
  const 글자 = (...cps) => String.fromCodePoint(...cps);
  const 두칸 = [['✅', 글자(0x2705)], ['❌', 글자(0x274c)], ['⚡', 글자(0x26a1)], ['⭐', 글자(0x2b50)], ['☕', 글자(0x2615)],
    ['⛔', 글자(0x26d4)], ['U+1FAE0', 글자(0x1fae0)], ['한자 확장 B U+20000', 글자(0x20000)], ['웃는 얼굴', 글자(0x1f600)]];
  for (const [이름, s] of 두칸) check(`★ ${이름} 는 두 칸이다`, width(s) === 2, String(width(s)));
  const 영칸 = [['ZWSP', 글자(0x200b)], ['ZWJ', 글자(0x200d)], ['BOM', 글자(0xfeff)], ['결합 악센트', 글자(0x301)], ['변형 선택자', 글자(0xfe0f)]];
  for (const [이름, s] of 영칸) check(`★ ${이름} 는 칸을 안 먹는다`, width(s) === 0, String(width(s)));
  check('★ ZWJ 로 이은 가족 그림은 두 칸이다', width(글자(0x1f468, 0x200d, 0x1f469, 0x200d, 0x1f467)) === 2,
    String(width(글자(0x1f468, 0x200d, 0x1f469, 0x200d, 0x1f467))));
  check('★ 풀어 쓴 한글(NFD 「한」)도 두 칸이다', width('한'.normalize('NFD')) === 2, String(width('한'.normalize('NFD'))));
  check('e + 결합 악센트는 한 칸이다', width(`e${글자(0x301)}`) === 1, String(width(`e${글자(0x301)}`)));
  // 우리가 그리는 표시는 한 칸 그대로여야 한다 — 두 칸으로 세면 상태줄·표가 한 칸씩 밀린다.
  for (const 표 of ['✓', '✗', '⚠', '●', '›', '☀', '✔', '★', '☆', '→', '⏵', '◎', '⌂', '↗', '▰', '▱', '⠋', '─', '│', '╭', '…']) {
    check(`우리 표시 ${표} 는 한 칸 그대로다`, width(표) === 1, String(width(표)));
  }

  /*
   * ── 이형 선택자·키캡·살색·국기 (Gemini 화면4) ─────────────────────────────
   *
   * U+FE0F 는 「그림으로 그려라」 는 표시다. 윈도 터미널·iTerm2·kitty 는 하트·경고·체크에 이것이
   * 붙으면 두 칸에 그리는데, 우리는 바탕 글자 한 칸 + 선택자 0칸 = 한 칸으로 셌다. 모델 답에 흔한
   * 모양이라 한 줄에 하나만 있어도 입력 상자가 줄 수를 틀린다. 키캡(1+FE0F+20E3)·한 칸 그림에
   * 붙은 살색도 그랬다. 국기(지역 표시 둘)는 거꾸로 넷으로 셌다 — 둘이 두 칸 하나로 겹친다.
   * 선택자 없는 경고·체크는 여전히 한 칸이다(바로 위 우리 표시).
   */
  const VS16 = 글자(0xfe0f);
  const 그림꼴 = [['하트+VS16', 글자(0x2764) + VS16], ['경고+VS16', 글자(0x26a0) + VS16], ['체크+VS16', 글자(0x2714) + VS16],
    ['재생+VS16', 글자(0x25b6) + VS16], ['저작권+VS16', 글자(0xa9) + VS16], ['키캡 1', '1' + VS16 + 글자(0x20e3)], ['키캡 #', '#' + VS16 + 글자(0x20e3)],
    ['검지+살색', 글자(0x261d, 0x1f3fd)], ['국기 KR', 글자(0x1f1f0, 0x1f1f7)], ['무지개 깃발', 글자(0x1f3f3) + VS16 + 글자(0x200d, 0x1f308)],
    ['불타는 하트', 글자(0x2764) + VS16 + 글자(0x200d, 0x1f525)], ['초록 체크+VS16', 글자(0x2705) + VS16], ['엄지+살색', 글자(0x1f44d, 0x1f3fd)]];
  for (const [이름, s] of 그림꼴) check(`★ ${이름} 는 두 칸이다`, width(s) === 2, String(width(s)));
  check('★ 국기 둘은 네 칸이다 (짝끼리만 겹친다)', width(글자(0x1f1fa, 0x1f1f8, 0x1f1f0, 0x1f1f7)) === 4, String(width(글자(0x1f1fa, 0x1f1f8, 0x1f1f0, 0x1f1f7))));
  check('로마자 뒤 선택자는 칸을 안 늘린다 (a + VS16)', width('a' + VS16) === 1, String(width('a' + VS16)));
  check('숫자 뒤 선택자만으로는 키캡이 아니다 (1 + VS16)', width('1' + VS16) === 1, String(width('1' + VS16)));
  check('글 모양 선택자(VS15)는 한 칸 그대로', width(글자(0x2764, 0xfe0e)) === 1, String(width(글자(0x2764, 0xfe0e))));
  check('★ 선택자 붙은 그림이 섞인 줄도 합이 맞다', width(`경고 ${글자(0x26a0) + VS16} 끝`) === 10, String(width(`경고 ${글자(0x26a0) + VS16} 끝`)));
  // 자를 때도 덩이째다 — 반쪽 국기 · 떨어진 선택자 · 끝에 남은 ZWJ 가 없어야 한다(clip 은 코드 포인트 하나씩 잘랐다).
  const 국기둘 = 글자(0x1f1f0, 0x1f1f7, 0x1f1fa, 0x1f1f8);
  check('★ 국기를 반쪽으로 안 자른다', clip(국기둘, 3) === 글자(0x1f1f0, 0x1f1f7) + '…', JSON.stringify(clip(국기둘, 3)));
  const 하트줄 = 'a' + 글자(0x2764) + VS16 + 'bcd';
  check('★ 선택자 붙은 그림을 잘라도 칸을 안 넘는다', width(clip(하트줄, 3)) <= 3, `${width(clip(하트줄, 3))}칸`);
  const 가족줄 = 글자(0x1f468, 0x200d, 0x1f469, 0x200d, 0x1f467) + 'xyz';
  check('★ 자른 끝에 ZWJ 가 안 남는다', !clip(가족줄, 3).includes(글자(0x200d) + '…'), JSON.stringify(clip(가족줄, 3)));
  // 꼬리보다 좁은 칸 — 남은 예산이 음수여도 꼬리를 붙여 max 를 넘겼다 (Gemini 화면5).
  for (const [글, max, 꼬리] of [['테스트', 0, '…'], ['abc', 1, '~~'], ['abc', -3, '…']]) {
    check(`★ clip 이 꼬리보다 좁은 칸(${max}, 꼬리 ${꼬리})도 안 넘긴다`, width(clip(글, max, 꼬리)) <= Math.max(0, max), JSON.stringify(clip(글, max, 꼬리)));
  }
  check('칸이 꼬리만큼이면 꼬리만 남긴다', clip('abc', 1, '…') === '…', JSON.stringify(clip('abc', 1, '…')));
}
