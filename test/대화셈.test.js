...
/*
 * ── 9. 「도구 정의」 칸의 캐시 열쇠가 다른 것을 같은 것으로 쳤다 (8회차 세션 4·5) ──
 *
 * 이 칸은 한 번 재면 열쇠에 넣어 두고 다시 안 잰다. 그래서 **열쇠가 못 가르는 차이는
 * 앞사람 값이 그대로 돌아온다** — 틀린 값이 화면에만 있는 것이 아니다. effort.js 가
 * 이 값으로 남은 자리를 셈해 출력 상한을 잡으므로 답이 이유 없이 짧아지거나, 다 찼는데
 * 안 찼다고 알고 그대로 나간다.
 *
 *   4. `도구제한` 의 `null`(제한 없음)과 `[]`(0개)이 둘 다 빈 글로 합쳐져 같은 열쇠였다.
 *      하위 작업이 도구를 다 잃은 판에서 부모의 전체 도구 값(4천 토큰대)을 그대로 집어 온다.
 *   5. 에이전트 목록을 **개수**로만 갈랐다. 이름과 설명은 Task 스키마에 통째로 실리는데
 *      (agents.js 의 고를말), 개수가 같으면 설명이 아무리 달라도 앞 값이 돌아온다.
 */
{
  const 도구칸 = (s) => s.breakdown().rows.find((r) => /도구 정의|Tool definitions/i.test(r.label))?.n;
  ...
  s.도구제한 = [];
  const 없앤뒤 = 도구칸(s);
  const 맨처음부터없앰 = (() => { const t = 새세션(); t.도구제한 = []; return 도구칸(t); })();
  check('★★ 도구 0개는 제한 없음과 다른 값이다 (8회차 세션 4)',
    없앤뒤 !== 제한없음 && 없앤뒤 === 맨처음부터없앰, `제한없음 ${제한없음} · 0개 ${없앤뒤} (맨처음부터 ${맨처음부터없앰})`);

  const t = 새세션();
  t.실린에이전트들 = [{ 이름: '리뷰어', 설명: '고친 데만 본다. '.repeat(9) }];
  const 긴설명 = 도구칸(t);
  t.실린에이전트들 = [{ 이름: '짧', 설명: 'ㄱ' }];
  const 짧은설명 = 도구칸(t);
  const 짧은것만 = (() => { const u = 새세션(); u.실린에이전트들 = [{ 이름: '짧', 설명: 'ㄱ' }]; return 도구칸(u); })();
  check('★★ 수가 같아도 에이전트 목록이 다르면 다시 잰다 (8회차 세션 5)',
    짧은설명 !== 긴설명 && 짧은설명 === 짧은것만, `긴설명 ${긴설명} · 짧은설명 ${짧은설명} (혼자 ${짧은것만})`);

  // 캐시 자체는 살아 있어야 한다 — 열쇠를 아예 못 쓰게 만들면 매 턴 통째로 다시 잰다.
  const u = 새세션();
  check('  (짝) 같은 자리에서 두 번 부르면 같은 값', 도구칸(u) === 도구칸(u), String(도구칸(u)));
}
