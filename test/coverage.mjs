...
if (JSON출력) {
   ...
   * 전체만 **소수 한 자리**로 적는다.
   *
   * 반올림해서 적었더니 화면이 이랬다 — 막대 옆에는 `90%` 라고 크게 적혀
   * 있는데 바로 아래 줄이 `커버리지 90.0% 가 문턱 90% 아래입니다` 였다.
   * 실제 값은 89.957% 였다. 사람이 보기에 앞뒤가 안 맞는 화면이고, 관문에
   * 걸린 사람은 무엇을 믿어야 할지 모른다.
   *
   * 파일별은 그대로 반올림해 둔다. 거기 걸린 판단이 없어서 자릿수만 늘고
   * 표가 읽기 어려워진다. 관문이 보는 수는 이 한 줄뿐이다.
   */
  console.log(`  ${b('전체')}${' '.repeat(폭 - 2)}  ${막대(총비율)} ${(총비율 >= 문턱 ? g : r)(총비율.toFixed(1).padStart(5) + '%')} ${d(`${총밟음}/${총전체}줄`)}`);
 ...
 * ── 문턱이 아무것도 안 막고 있었다 ──────────────────────────────────────
 *
 * `--min` 은 여태 **색칠과 개수 세기**에만 쓰였다. 종료코드는 검사 스위트
 * 것을 그대로 넘겼으니, `--min 95` 로 돌려도 초록으로 끝났다. 관문처럼
 * 생겼는데 아무것도 안 막는 자리다 — CI 에 그대로 걸었다면 「커버리지를
 * 지키고 있다」 는 줄이 하나 늘고 지키는 것은 없었을 것이다.
 *
 * 그래서 **전체 비율**에 건다. 파일별로 걸지 않는 까닭은 위 머리말에 적힌
 * 그대로다 — 일부러 낮게 둔 파일이 있고, 그것을 억지로 올리면 검사가
 * 거짓말을 시작한다. 전체는 그런 핑계가 없다.
 */
const 문턱미달 = 총비율 < 문턱;
if (문턱미달 && !인자.includes('--json')) {
  console.error(`커버리지 ${총비율.toFixed(1)}% 가 문턱 ${문턱}% 아래입니다.`);
}
process.exitCode = (code === 0 && !문턱미달) ? 0 : 1;
