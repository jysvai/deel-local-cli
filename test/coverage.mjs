...
function 재기(경로, 프로세스별) {
  ...
  // 한 번도 안 지나간 함수. 가장 손대기 쉬운 구멍이다.
  const 안밟은함수 = [];
  for (const [키, 불림] of 함수불림) {
    if (불림) continue;
    const [이름, off] = [키.slice(0, 키.lastIndexOf('@')), Number(키.slice(키.lastIndexOf('@') + 1))];
    안밟은함수.push({ 이름: 이름 || '(이름없음)', 줄: 줄번호[Math.min(len - 1, Math.max(0, off))] + 1 });
  }
