...
// 보는 화면이 갈라지고, 그때부터 검사는 아무것도 안 지켜 준다.
{
  ...
  const 가짜상자 = {
    지운횟수: 0, 그린것: null, 일감: null, 줄중간: false,
    지우기() { this.지운횟수++; },
    ...
    // 줄 중간에서는 아무것도 안 그린다 — 그리면 대화 줄을 덮는다.
    줄중간표시() { this.지운횟수++; this.줄중간 = true; },
    줄끝남() { this.줄중간 = false; },
    대기갱신(글, 수) { this.대기 = { 글, 수 }; },
  };
...
// ── ESC 를 누른 것이 화면에 곧바로 보이는가 ─────────────────────────────
//
// 멈추기는 즉시가 아니다. 돌던 자식 프로그램을 죽이고 대화를 성하게 닫는 데
// 잠깐이 걸린다. 그 잠깐 동안 화면이 아까 하던 말을 그대로 하고 있으면 사람은
// **안 먹었다고 판단하고 또 누른다** — 그러다 Ctrl+C 로 손이 가고, 두 번
// 누르면 대화가 통째로 닫힌다.
//
// 함정이 하나 있다. 45초가 넘으면 문구가 '느긋' 으로 갈리는데, 사람이 ESC 를
// 누르는 때가 하필 딱 그때다(오래 걸려서 누르는 것이다). 순서를 안 정해 두면
// 「멈추는 중」이 화면에 영영 안 뜬다. 그래서 여기서는 **오래 걸린 뒤**를 잰다.
{
  const { InputBox } = await import('../src/ui/inputbox.js');
  const { 문구: 문구2, 느긋해질때 } = await import('../src/ui/working.js');
  const 진짜쓰기 = process.stdout.write.bind(process.stdout);
  const 담기 = (fn) => {
    let 글 = '';
    process.stdout.write = (s) => { 글 += String(s); return true; };
    try { fn(); } finally { process.stdout.write = 진짜쓰기; }
    return 글.replace(/\x1b\[[0-9;?]*[A-Za-z]/g, '');
  };

  const b = new InputBox();
  담기(() => b.일시작(null, '읽기'));
  // 45초가 지난 것처럼 되돌린다. 검사가 진짜로 45초를 기다릴 수는 없다.
  //
  // 둘 다 되돌린다. 느긋으로 넘어가는 셈은 **갈래가 바뀐 뒤**로 재고(그래야
  // 45초가 지난 다음에도 하는 일이 바뀌면 그 일을 말한다), 걸린 시간은
  // 턴이 시작된 때부터 잰다. 여기서 만들려는 상황은 「45초 동안 아무것도
  // 안 바뀌었다」 이므로 두 값이 같이 뒤로 가야 한다.
  b.일감.시작 = Date.now() - (느긋해질때 + 5000);
  b.일감.바뀐때 = b.일감.시작;

  const 느긋할때 = 담기(() => b.일그리기());
  check('오래 걸리면 느긋 문구로 바뀐다',
    문구2.느긋.some((m) => 느긋할때.includes(m)), 느긋할때.slice(0, 60));
  // 이 한 줄이 없으면, '멈추는 중일 때만 감추기' 를 '늘 감추기' 로 잘못
  // 적어도 아무것도 안 빨개진다. 그러면 멈추는 길을 아무도 못 배운다.
  check('평소에는 ESC 로 멈출 수 있다고 알려 준다',
    느긋할때.includes('ESC 중단'), 느긋할때.slice(0, 80));

  // 그린 것을 여기서 받아야 한다. 상자는 **달라진 줄만** 다시 쓰므로,
  // 한 번 그린 뒤에 또 그리면 아무것도 안 나온다 (빈 글자에는 무엇을 찾아도
  // 없어서, 검사가 통과한 척한다 — 실제로 그렇게 한 번 속았다).
  const 멈출때 = 담기(() => b.일바꿈('멈춤'));
  check('멈추라고 하면 화면을 실제로 다시 그린다', 멈출때.length > 0, `${멈출때.length}자`);
  check('★ ESC 를 누르면 오래 걸린 뒤에도 「멈추는 중」이 뜬다',
    멈출때.includes('멈추는 중'), 멈출때.slice(0, 80));
  check('멈추는 중에는 ESC 를 또 누르라고 안 한다',
    !멈출때.includes('ESC 중단'), 멈출때.slice(0, 80));
  check('멈춤 문구는 안 돌아간다 — 한 마디만 붙박이',
    문구2.멈춤.length === 1, JSON.stringify(문구2.멈춤));
...
/*
 * ── ESC 를 눌렀는데 화면이 가만히 있으면 ────────────────────────────────
 *
 * 멈추는 데는 잠깐이 걸린다 — 돌던 자식을 죽이고, 안 돈 도구 자리를 채우고,
 * 대화를 성하게 닫아야 한다. 상자 화면은 그 잠깐 동안 상자 안 문구를
 * '멈추는 중' 으로 바꿔 준다. 그런데 줄 화면(`--no-tui`·파이프)의 일바꿈()
 * 은 **아무것도 안 하는 빈 메서드**였다. 그래서 거기서는 ESC 를 눌러도
 * 화면이 아까 하던 말을 그대로 하고 있었다.
 *
 * 사람은 안 먹었다고 판단하고 또 누른다. 그러다 Ctrl+C 로 손이 가고,
 * 두 번 누르면 대화가 통째로 닫힌다 — 실제로 잃는 것은 그 대화다.
 */
{
  const s = new LineScreen();
  const 진짜쓰기 = process.stdout.write.bind(process.stdout);
  const 잡기 = (일) => {
    let v = '';
    process.stdout.write = (b) => { v += b; return true; };
    try { 일(); } finally { process.stdout.write = 진짜쓰기; }
    return 벗기기(v);
  };

  const 멈춤글 = 잡기(() => s.일바꿈('멈춤'));
  check('★ 줄 화면도 멈춘다고 말한다', /멈추는 중/.test(멈춤글), JSON.stringify(멈춤글));

  /*
   * ★ 한 번만 낸다. ESC 를 세 번 누르면 같은 줄이 세 번 찍히는데,
   * 그건 신호가 아니라 잡음이다.
   */
  const 두번째 = 잡기(() => { s.일바꿈('멈춤'); s.일바꿈('멈춤'); });
  check('★ 두 번 눌러도 한 번만 적는다', 두번째 === '', JSON.stringify(두번째));

  // 턴이 새로 시작하면 다시 낼 수 있어야 한다 — 안 그러면 두 번째 턴에서 벙어리다.
  const 새턴 = 잡기(() => { s.일끝(); s.일바꿈('멈춤'); });
  check('★ 다음 턴에는 다시 적는다', /멈추는 중/.test(새턴), JSON.stringify(새턴));

  /*
   * 나머지 갈래는 여전히 조용해야 한다. 줄 화면에는 '한 자리에서 도는
   * 표시' 를 둘 데가 없어서, 갈래가 바뀔 때마다 줄을 찍으면 대화가
   * 도구 이름으로 뒤덮인다.
   */
  s.일끝();
  const 딴갈래 = 잡기(() => { s.일바꿈('읽기'); s.일바꿈('생각'); s.일바꿈('접기'); });
  check('다른 갈래에는 아무것도 안 찍는다', 딴갈래 === '', JSON.stringify(딴갈래));
}
...
// ── 일하는 도중에 미리 쳐 두기 ──────────────────────────────────────────
//
// readline 은 일하는 동안에도 글을 받아 두고 있었다. 다만 화면에 아무것도
// 안 보여서, 사람은 "작업 중에는 못 친다" 고 생각하고 끝나기를 지켜봤다.
// 로컬 모델은 한 턴이 몇 분이라, 그 시간이 통째로 버려지고 있었다.
{
  const 일감 = { 돌림: '⠋', 말: '코드 짜는 중', 곁: '12초 · Ctrl+C 중단' };
  const 그냥 = 프레임({ 폭: 80, 일감 });
  const 치는중 = 프레임({ 폭: 80, 일감: { ...일감, 대기: '집계도 같이 줄여줘' } });

  check('평소 일하는 상자는 한 줄짜리', 그냥.줄들.filter((l) => /│/.test(벗기기(l))).length === 1);
  check('미리 치면 한 줄이 는다', 치는중.줄들.filter((l) => /│/.test(벗기기(l))).length === 2,
    String(치는중.줄들.filter((l) => /│/.test(벗기기(l))).length));
  check('치던 글이 그대로 보인다', /❯ 집계도 같이 줄여줘/.test(벗기기(치는중.줄들.join('\n'))),
    벗기기(치는중.줄들[2] ?? ''));
  // 언제 나가는지 말해 주지 않으면, 몇 분 뒤에 저절로 나가는 것처럼 보인다.
  check('언제 나가는지 알려 준다', /끝난 뒤에 보냅니다/.test(벗기기(치는중.줄들.at(-1))),
    벗기기(치는중.줄들.at(-1)));
  check('돌아가는 표시는 그대로 있다', /⠋ 코드 짜는 중/.test(벗기기(치는중.줄들[1])), 벗기기(치는중.줄들[1]));

  // 커서가 치는 자리에 있어야 한다. 아니면 백스페이스가 엉뚱해 보인다.
  // 앞머리 5칸(' │ ❯ ') 다음이 첫 글자 자리다. 한글은 두 칸으로 세야 한다.
  check('커서가 치던 자리에 간다', 치는중.커서.열 === 6 + width('집계도 같이 줄여줘'),
    `${치는중.커서.열}칸 (${6 + width('집계도 같이 줄여줘')} 이어야 한다)`);
  check('평소에는 커서를 안 보여 준다', 그냥.커서.열 === 1, String(그냥.커서.열));

  // 이미 Enter 를 친 것은 개수로 센다. 안 세면 몇 분 뒤에 엉뚱한 말이
  // 저절로 나가는 것처럼 보인다.
  const 예약 = 프레임({ 폭: 100, 일감: { ...일감, 곁: '12초 · 2건 예약됨 · Ctrl+C 중단' } });
  check('예약해 둔 개수를 센다', /2건 예약됨/.test(벗기기(예약.줄들[1])), 벗기기(예약.줄들[1]));

  // 아무리 길게 쳐도 상자가 화면을 다 먹으면 안 된다.
  const 길게 = 프레임({ 폭: 60, 일감: { ...일감, 대기: '가'.repeat(400) } });
  check('길게 쳐도 세 줄까지만', 길게.줄들.filter((l) => /❯/.test(벗기기(l))).length <= 1, '');
  check('길게 쳐도 상자가 안 넘친다', 길게.줄들.every((l) => width(벗기기(l)) <= 60),
    String(Math.max(...길게.줄들.map((l) => width(벗기기(l))))));
}

trace('9.6b-오래-걸릴때-무슨-말을-하나');

// ── '아직 하는 중' 은 언제 나와야 하나 ──────────────────────────────────
//
// 45초가 넘으면 문구를 '느긋' 으로 갈아 준다. 넣은 뜻은 「아까 하던 말을
// 계속 돌리면 붙박이처럼 보인다 — 아직 살아 있다는 신호를 줘라」 였다.
//
// 그런데 그 45초를 **턴이 시작된 때**부터 쟀다. 로컬 모델은 한 턴이 몇
// 분이라, 45초가 지난 다음부터는 파일을 읽든 코드를 짜든 명령을 돌리든
// 화면이 계속 '아직 하는 중' 만 돌았다 — 이 문구가 스스로 붙박이가 됐다.
// 그래서 **갈래가 바뀐 때**부터 잰다.
{
  const { InputBox } = await import('../src/ui/inputbox.js');
  const 진짜now = Date.now;
  const 원래write = process.stdout.write;
  const 벗 = (x) => String(x).replace(/\x1b\[[0-9;]*m/g, '')
    .replace(/\x1b\[[0-9;?]*[A-Za-z]/g, '');
  let 찍힌것 = '';
  let 밀기 = 0;
  Date.now = () => 진짜now() + 밀기;
  process.stdout.write = (x) => { 찍힌것 += String(x); return true; };
  const 상자 = new InputBox();
  // 상자는 세션에서 상태줄을 읽는다. 여기서 재는 것은 본문 한 줄뿐이다.
  const 쓸세션 = {
    root: process.cwd(), mode: 'auto', think: 'medium', effort: 'save', level: '보통',
    conn: { kind: 'openai', base: 'http://127.0.0.1:1/v1', model: '스텁', ctx: 32768 },
    usage: { input: 100, output: 10, 못잰것: 0 },
    breakdown: () => ({ used: 110, total: 32768, 총잰것: true }), edits: [],
  };
  const 본문 = () => {
    찍힌것 = '';
    상자.앞줄들 = null;              // 통째로 다시 그리게 해서 본문을 읽는다
    상자.일그리기();
    const 줄 = 벗(찍힌것).split('\n')
      .find((l) => l.includes('│') && !l.includes('╭') && !l.includes('╰'));
    return (줄 ?? '').replace(/│/g, '').trim();
  };
  ...
  try {
    ...
    표.push(['50초 · 그대로', 본문()]);
    상자.일바꿈('쓰기');
    표.push(['50초 · 쓰기로 바뀜', 본문()]);
    밀기 = 70000;
    표.push(['70초 · 쓰는 중', 본문()]);
    밀기 = 130000;
    표.push(['130초 · 쓴 지 80초', 본문()]);
  } finally {
    상자.일끝();
    Date.now = 진짜now;
    process.stdout.write = 원래write;
  }

  const 느긋인가 = (x) => /아직 하는 중|조금만 더|오래 걸리는 중|그래도 하는 중/.test(x);
  check('가만히 있으면 45초 뒤에 아직 하는 중이라고 말한다', 느긋인가(표[1][1]), 표[1][1]);
  check('★ 하는 일이 바뀌면 그 일을 말한다 — 45초가 지났어도', !느긋인가(표[2][1]), 표[2][1]);
  check('★ 바뀐 뒤로도 계속 그 일을 말한다', !느긋인가(표[3][1]), 표[3][1]);
  check('★ 바뀐 뒤 다시 45초가 지나면 그때 느긋으로 간다', 느긋인가(표[4][1]), 표[4][1]);
  // 걸린 시간은 늘 턴이 시작된 때부터다 — 갈래가 바뀌어도 안 되감는다.
  check('걸린 시간은 턴 시작부터 센다', /2분 10초/.test(표[4][1]), 표[4][1]);
}
