...
const srv = createServer((req, res) => {
  ...
  req.on('end', () => {
     ...
     * 계획을 **산문으로만** 내는 턴.
     *
     * TodoWrite 를 안 쓰는 모델이 흔하다. 그때 ctx.todos 에는 지난 턴의
     * 남은 할 일이 그대로 들어 있고, 그것이 「방금 낸 계획」 으로 상자에
     * 뜨면 안 된다.
     */
    if (/산문으로만/.test(사람말)) {
      return 답({ role: 'assistant', content: '설계는 이렇습니다. 이대로 진행할까요?' });
    }
    /*
     * 계획을 내다 **출력 상한에서 잘린** 턴 (finish_reason: length).
     *
     * 반쪽짜리 계획을 두고 「이대로 진행할까요?」 를 물으면 안 된다 —
     * ⏎ 한 번에 그 반쪽이 「사람이 승인한 계획」 으로 실려 나간다.
     */
    if (/잘리는설계/.test(사람말)) {
      return 답({ role: 'assistant', content: '1단계는 이렇게 하고 2단계는' }, 'length');
    }
    // 계획을 고쳐 달라고 한 턴.
...
trace('6-지난턴-할일을-이번-계획인-척');

// ── `ctx.todos` 는 세션 내내 산다. 그런데 비우는 자리가 없다 ─────────────
//
// 그래서 이런 판이 난다.
//   턴 A  계획을 세우고 실행까지 한다. 할 일 셋은 done 표시가 안 된 채 남는다
//   턴 B  「…설계하고 만들어줘」 — 겹침이라 계획 승인이 켜진다
//         모델은 TodoWrite 없이 산문으로만 설계를 낸다
//   화면  ┌ 계획 / 1. 비전 선언문 한 장 쓰기 / … ← **턴 A 의 목록**
//
// ⏎ 를 누르면 그 옛 목록이 「방금 낸 계획」 이라는 이름표를 달고 모델에게
// 실려 나간다. 이 파일은 「TodoWrite 를 안 쓴 계획」 을 이미 대비해 뒀지만
// (할일.length === 0 갈래), 목록이 **비지 않고 옛것으로 차 있는** 경우가
// 그 그물을 통과했다.
{
  const r = await 띄우기([
    'ax비전 선포 내용들 정리해서 만들어줘', '',
    'ax산문으로만 설계하고 만들어줘',
  ]);
  // 두 번째 계획 상자부터가 이 절이 보는 자리다.
  const 뒤쪽 = r.out.slice(r.out.indexOf('산문으로만'));
  check('★★ 지난 턴 할 일을 이번 계획으로 안 그린다',
    !/비전 선언문 한 장 쓰기/.test(뒤쪽),
    뒤쪽.split('\n').filter((l) => /비전 선언문/.test(l)).join(' | ').slice(0, 100));
  check('★ 단계가 안 적혔다고 말한다', !/┌ 계획[\s\S]{0,200}3개년 로드맵/.test(뒤쪽),
    뒤쪽.split('\n').filter((l) => /로드맵/.test(l)).join(' | ').slice(0, 100));
}

trace('7-반쪽-계획에는-안-묻는다');

// ── 잘린 계획에 「이대로 진행할까요?」 를 물으면 안 된다 ────────────────
//
// repl.js 는 `계획승인받나 && !턴탈났나` 로 이 창을 건다. 그 규칙을 적어 둔
// 주석은 「계획이 반만 나온 것을 두고 이대로 진행할까요? 를 묻는 꼴이 된다」
// 였는데, 정작 **계획이 반만 나오는 대표 사건들**(capped·cutoff·limit·
// refusal)이 그 깃발을 안 세우고 있었다. 넷 다 for-await 는 예외 없이
// 끝나므로 깃발은 거짓인 채 남는다.
{
  const r = await 띄우기(['ax잘리는설계 정리하고 만들어줘']);
  check('잘렸다고는 말한다', /잘렸습니다/.test(r.out),
    r.out.split('\n').filter((l) => /잘렸/.test(l)).join(' | ').slice(0, 90));
  check('★★ 반쪽 계획에는 승인 상자를 안 띄운다', !/┌ 계획/.test(r.out),
    r.out.split('\n').filter((l) => /계획/.test(l)).slice(-2).join(' | ').slice(0, 100));
  check('★★ 반쪽 계획에 이대로 진행할지 안 묻는다', !/이대로 진행할까요\?/.test(r.out), '');
}
