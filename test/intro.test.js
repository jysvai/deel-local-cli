...
// 터미널마다 폭이 달라지면 지워야 할 자리가 어긋나 앞 그림이 조각으로 남는다.
{
   ...
   * 첫 줄 폭은 틀마다 같아야 한다.
   *
   * 다시 그리기는 커서를 올려 같은 자리에 덮어쓰는 것이라, 앞 틀이 더 넓으면
   * 넓은 만큼이 안 지워지고 조각으로 남는다. 커서(▏)는 이름 자리 밖에 한 칸을
   * 미리 잡아 두고 그 안에서 그린다 — 그래야 이름이 다 자란 틀에서도 한 칸이
   * 더 밀리지 않는다.
   */
  const 첫줄폭 = Array.from({ length: 틀수 }, (_, i) => width(틀(i)[0]));
  check('첫 줄 폭이 틀마다 같다', new Set(첫줄폭).size === 1, JSON.stringify(첫줄폭));
  check('첫 줄 폭은 들여 둘 + 이름 + 커서 한 칸', 첫줄폭[0] === 이름너비() + 3,
    `${첫줄폭[0]} / ${이름너비() + 3}`);
}
...
// ── 돌릴 때는 제자리에서 다시 그린다 ────────────────────────────────────
{
  ...
  // 이름이 「숨겼다 다시 보인다」 인데 재는 것은 **둘 다 없다** 였다. 재는 것에 맞춰 부른다.
  check('커서 제어는 쓰기(곁말 흐름)에 안 섞인다',
