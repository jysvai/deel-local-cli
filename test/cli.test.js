...
let 헛잘림셈 = 0;
// 그때마다 받은 출력 상한. 다시 부르는데 이 값이 안 오르면 그것이 낭비다.
let 헛잘림상한들 = [];
// 생각 블록이 지워진 채 잘린 답을 몇 번 부탁받았나.
let 숨은생각셈 = 0;
const 대본초기화 = () => { 도구한번 = 잘림한번 = 할일한번 = 쓰기한번 = 여럿한번 = false; 도구번호 = 1; 헛잘림셈 = 0; 헛잘림상한들 = []; 숨은생각셈 = 0; 받은요청 = []; };
  ...
  req.on('end', () => {
    ...
    if (url === '/v1/chat/completions') {
      ...
      if (/일부러_맨잘림/.test(사람말)) {
         ...
         * **상한도 같이 적는다** (16회차).
         *
         * 「다시 불렀나」 만 세면 낭비와 제 일을 못 가른다 — 상한을 올려 다시
         * 부르는 것은 약속대로 도는 것이다(윗 갈래가 그것을 잰다). 낭비는
         * **같은 상한으로** 또 부르는 것이고, 그 차이는 이 값에만 있다.
         */
        헛잘림상한들.push(Number(json?.max_tokens ?? json?.max_completion_tokens ?? 0));
...

{
   ...
   */
  /*
   * ── 이 검사는 두 번 틀렸다. 무엇을 재야 하는지가 문제였다 (16회차) ────
   *
   * (가) 머리말은 「부른 횟수를 센다」 인데 코드는 **화면 글**을 재고 있었다.
   *      그 글은 「생각 중…」 줄에 잠깐 실렸다 `clearThinking()` 이 지우는
   *      임시 줄이라, 기계가 빠르면 그 프레임을 아예 안 그린다. 리눅스에서
   *      빨개진 첫 까닭이 이것이다.
   *
   * (나) 그래서 횟수로 바꿨더니 **또** 빨개졌다 — 이번엔 약속이 지켜지는데도.
   *      재 보니 거기서는 다시 부를 때 **상한이 올라갔다.** 그건 낭비가
   *      아니라 제 일이다(바로 윗 갈래가 그것을 잰다).
   *
   *          창 2,048 · 4,096   남은 자리 0 → 아껴 잡은 값도 푼 값도 바닥 512
   *                             → 한 번만 부른다          ← 여기서 잴 것
   *          창 8,192           1,124 → 2,249 로 올려 다시 부른다
   *          창 16,384          4,095 → 8,191
   *
   *      우리 PC 는 살림 프롬프트가 커서 4,096 에서도 자리가 0이었고, CI 는
   *      그것이 작아 95% 라 자리가 남았다. **같은 깃발이 기계마다 다른 갈래를
   *      밟은 것이다.** 깃발로 갈래를 고르면 재는 자리가 기계 따라 바뀐다.
   *
   * 그래서 깃발이 아니라 **상한**을 본다. 낭비는 「다시 불렀다」 가 아니라
   * 「**같은 상한으로** 다시 불렀다」 이고, 그 차이는 요청의 max_tokens 에만
   * 있다. 두 방향을 다 잰다 — 한쪽만 재면 「아무 때나 안 부른다」 도 초록이 된다.
   */
  대본초기화();
  const r = await 띄우기(['--ctx', '2048'], { 입력: ['일부러_맨잘림 답을 줘', '/exit'], 제한: 60000 });
  check('★ 재려는 자리가 실제로 섰다 — 상한을 못 올리는 판이다',
    헛잘림상한들[0] === 512, `첫 상한 ${헛잘림상한들[0]} (바닥 512 여야 이 갈래다)`);
  check('★★★ 생각이 0이고 상한도 못 올리면 다시 안 부른다', 헛잘림셈 === 1,
    `스텁이 ${헛잘림셈}번 불렸습니다 · 상한 ${헛잘림상한들.join(' → ')}`);
  // 그래도 사람에게는 말해 줘야 한다 — 잘린 채로 조용히 지나가면 안 된다.
  check('대신 잘렸다는 것과 손댈 자리를 알려 준다', /잘렸습니다/.test(r.out) && /\/out/.test(r.out),
    r.out.slice(-300));

  // 반대쪽 — 올릴 자리가 있으면 **올려서** 다시 부른다.
  대본초기화();
  await 띄우기(['--ctx', '16384'], { 입력: ['일부러_맨잘림 답을 줘', '/exit'], 제한: 60000 });
  check('★★ 올릴 자리가 있으면 상한을 올려 다시 부른다',
    헛잘림셈 === 2 && 헛잘림상한들[1] > 헛잘림상한들[0],
    `${헛잘림셈}번 · 상한 ${헛잘림상한들.join(' → ')}`);
  check('★★★ 어느 쪽이든 같은 상한으로 두 번 부르는 일은 없다',
    new Set(헛잘림상한들).size === 헛잘림상한들.length, 헛잘림상한들.join(' → '));
}
...

{
  ...
  /*
   * (2.1.0) 무엇이 바뀌는지를 **물음 앞에** 그린다. 물음 뒤에 그리면 사람은 이미 답한 뒤에 본다.
   * 물음 줄 앞 토막에서 새 파일 표시와 들어갈 내용 줄을 찾는다.
   */
  const 물음앞 = r.out.slice(0, Math.max(0, r.out.indexOf('실행할까요')));
  check('★★★ (2.1.0) 묻기 전에 바뀔 내용(새 파일 · 들어갈 줄)을 그린다',
    /새로쓴것\.txt.*\(새 파일\)/.test(물음앞) && /\+ +1 스텁이 쓴 내용/.test(물음앞), 물음앞.slice(-300));
