...
// ── 채팅창 밖의 화면도 센다 ─────────────────────────────────────────────
//
// README 는 이 검사가 「일곱 화면을 훑는다」 고 적어 두었는데, 위 대본 여섯
// 개는 **전부 채팅 REPL 안**이다(`--no-tui` 로 들어가서 슬래시 명령을 친다).
// argv 로만 닿는 화면 — `--help` · `status` · `sessions` · `reset` — 은 한
// 번도 재어진 적이 없다. 그런데 영어권 사람이 처음 보는 화면이 바로 그
// `--help` 다: `npx deel-local-cli --help` 가 91줄 중 70줄을 한국어로 낸다.
//
// ── 왜 0 이 아니라 래칫인가 ─────────────────────────────────────────────
//
// 0 으로 걸면 이 검사는 오늘 **빨갛게** 되고, `prepublishOnly` 가
// `npm run check && npm test` 이므로 번역이 다 끝날 때까지 배포가 막힌다.
// 번역은 다섯 파일에 걸친 300개 넘는 리터럴이라 한 판짜리 일이 아니다.
// 그동안 CI 가 빨간 채로 있으면, 새로 생긴 실패와 「알고 있는 그 실패」 를
// 가릴 수가 없다 — 관문을 깨 놓고 관문이라 부르는 셈이다.
//
// 그래서 **지금 수를 못박고, 늘어나면 실패한다.** 줄어들면 상수를 내린다.
// 번역을 사지 않고도 값이 얼마인지는 매 판 인쇄된다. 1.15 는 이 수를 보고
// 결정한다 — 논쟁이 아니라 숫자로.
const 밖대본 = [
  ['--help', ['--help']],
  ['status', ['status']],
...
// 오늘 잰 값. 줄이는 것만 허용한다 — 늘리려면 여기에 까닭을 적어야 한다.
//
// 92 → 93 (1.14.0): `deel run /이름` 을 넣으면서 --help 에 예시 한 줄이 늘었다.
// 화면이 한 줄 더 새는 것은 맞지만, 그 자리는 이미 91줄 중 71줄이 한국어다.
// 기능 설명을 빼서 수를 맞추는 것은 검사를 위해 화면을 나쁘게 만드는 일이라
// 안 한다. 갚을 곳은 --help 번역 한 판이고, 그건 1.15 의 몫이다.
//
// 93 → 94 (1.17.0): `--output-schema` 깃발 한 줄. 이 깃발은 파이프 뒤에서
// 쓰라고 만든 것이라 **깃발 목록에 없으면 없는 기능**이다 — 보기만 적어 두고
// 목록에서 빼면 사람은 `deel --help | grep schema` 로 못 찾는다. 대신 같이
// 넣은 보기 줄은 스키마 파일 이름과 jq 경로를 영문으로 적어 한 줄도 안 늘렸다
// (`out.schema.json` · `.term`). 그건 화면을 나쁘게 만드는 일이 아니다 —
// 셸에 그대로 붙여 넣는 줄이라 오히려 영문이 낫다.
//
// 94 → 95 (1.17.0): `--no-hooks` 깃발 한 줄. 이것도 목록에 없으면 없는
// 기능이다 — 훅이 망가졌을 때 물어볼 데가 `deel --help` 뿐인데, 거기 없으면
// 남는 길은 훅 파일을 지우는 것이다. 끄는 길이 안 보이는 안전 장치는 안전
// 장치가 아니다. 갚을 곳은 여전히 --help 번역 한 판이다.
// 95 → 96 (2.0.0): `deel reset all` 이 **따로 고를 수 없는 갈래 하나**를 더 지운다는
// 줄(`증거·내보낸 것·임시 — .deel/증거 · export · tmp · 붙인그림`). 이 판을 재던 중에
// `deel reset --hard all --yes` 한 줄이 저장소의 `.deel/tmp` 를 통째로 지웠고, 도움말만
// 읽은 사람은 그 폴더가 지워진다는 것을 알 길이 없었다 — `all` 줄은 「위 전부」 라고만
// 적고 그 갈래는 위에 없었다. 앞 줄(`deel reset all`)에 접어 넣어 줄 수를 지킬 수도
// 있었지만 그러면 한 줄이 130칸을 넘겨 접힌다. 지우는 자리를 미리 볼 데가 도움말
// 말고는 없는 갈래라, 줄 수보다 이쪽이 세다. 갚을 곳은 여전히 --help 번역 한 판이다.
const 래칫 = 96;

let 밖에샌것 = 0;
const 밖내역 = [];
for (const [이름, 인자] of 밖대본) {
  trace(`영어-밖-${이름}`);
  const 글 = await 띄우기(인자, [], { DEEL_LANG: 'en' });
  const 줄들 = [...new Set(글.split('\n').map((l) => l.trim()).filter(한글있나))];
  밖에샌것 += 줄들.length;
  밖내역.push(`${이름} ${줄들.length}`);
}
check(`★★ 채팅창 밖 화면의 한국어 줄이 ${래칫}줄을 안 넘는다`,
  밖에샌것 <= 래칫, `${밖에샌것}줄 (${밖내역.join(' · ')})`);
check('★ 그 수가 실제로 세어진다 (0 이면 세는 자리가 고장 난 것)',
  밖에샌것 > 0 || 래칫 === 0, String(밖에샌것));
