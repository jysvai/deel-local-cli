...
trace('2-카드');
{
  ...
  // 겪은 셈은 그대로 0 이다 — 문서로 안 것은 걸음으로 세지 않는다.
  // 다만 이미 바꿔 놓은 것이 있으니 「아직 판단하지 않습니다」 는 아니다.
  // 그렇게 두면 /card 가 바꿔 놓고 안 바꿨다고 말한다 (card.js 의 아직모름).
  check('겪은 걸음 셈은 그대로 0', 장.걸음 === 0, String(장.걸음));
  check('바꿔 놓고 「아직 모름」 이라고 하지 않는다', 장.아직모름 === false, String(장.아직모름));
