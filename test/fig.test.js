...

{
  ...
  /*
   * 도형이 상한을 넘으면 뒤쪽 쪽은 한 줄도 안 나온다. 그것을 「이 쪽은 비어
   * 있습니다」 라고 적으면, 시안에 멀쩡히 들어 있는 화면을 사람도 모델도
   * 없는 것으로 읽는다 — 그러고는 없는 화면을 새로 그린다.
   */
  const 잘린것 = readFig(시안, { 최대노드: 3 });
  check('★★ 상한에 걸려 못 편 쪽을 「비어 있다」 고 안 적는다',
    잘린것.ok === true && !/비어 있습니다\.\)/.test(잘린것.덩이들[2].문단들.join('')),
    JSON.stringify(잘린것.덩이들?.[2]?.문단들));
  check('★ 못 폈다고 말한다',
    /못 폈습니다/.test(잘린것.덩이들?.[2]?.문단들.join('') ?? ''),
    JSON.stringify(잘린것.덩이들?.[2]?.문단들));
