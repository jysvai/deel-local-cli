...
trace('3-JPEG표시');
{
  ...
  /*
   * ★ (6회차 눈6ak-b V1) 표시 앞의 채움 바이트.
   *
   * JPEG 은 표시 앞에 0xFF 를 여럿 둘 수 있다. 읽는 쪽은 그 FF 를 표시로 읽어 뒤 두
   * 바이트(FF C0 …)를 길이 0xC000 으로 먹고 튕겨 null 을 냈다. null 이면 픽셀 한도가
   * 안 걸려 긴 캡처가 그대로 실린다 — 다음 턴부터 400 이다.
   */
  const 채워넣기 = (buf, 몇) => {
    const 자리 = buf.findIndex((b, i) => b === 0xff && buf[i + 1] === 0xc0);
    return Buffer.concat([buf.subarray(0, 자리), Buffer.alloc(몇, 0xff), buf.subarray(자리)]);
  };
  const 하나 = 치수읽기(채워넣기(jpeg(200, 9000, [0xe0]), 1));
  check('★★★ (6회차 V1) SOF 앞에 채움 바이트(FF)가 있어도 치수를 읽는다',
    하나?.가로 === 200 && 하나?.세로 === 9000, JSON.stringify(하나));
  const 여럿 = 치수읽기(채워넣기(jpeg(200, 9000), 3));
  check('★★ (6회차 V1) 채움 바이트가 여럿이어도 읽는다', 여럿?.가로 === 200 && 여럿?.세로 === 9000, JSON.stringify(여럿));
}
