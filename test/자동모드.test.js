...
trace('7.6-계획거리');
{
   ...
   * ★★★ 그 예외의 양쪽 끝.
   *
   * 영어에는 「계획거리가 목적어면 시킴말이 아니다」 가 처음부터 있었는데
   * 한국어에는 없었다. 넣고 나면 반대로 너무 많이 빼는 것이 무섭다 —
   * 계획을 **실행**하라는 말과, 이름이 계획인 **파일**을 고치라는 말은
   * 그대로 시킴말이어야 한다. 여기가 새면 고치라는 말이 파일을 못 고치는
   * 모드로 간다.
   */
  for (const 글 of ['계획 정리해 줘', '계획을 수정해 줘', '로드맵 정리해 줘']) {
    check(`★★★ 계획거리를 손질하라는 말은 시킴말이 아니다 — "${글}"`,
      손대라했나(글) === false, `손대라=${손대라했나(글)}`);
  }
  for (const 글 of ['계획 구현해 줘', '계획서 정리해 줘', 'plan.md 정리해 줘',
    '계획 파일 수정해 줘', '코드 정리해 줘']) {
    check(`★★★ 그래도 이건 시킴말이다 — "${글}"`,
...
trace('9-길고르기6');
{
  ...
  for (const 말 of ['리뷰 결과대로 고쳐', '점검 결과대로 바꿔', '설계대로 만들어', '검토한 대로 지워.', '보안 점검 결과대로 수정해', '로그 수정해\n테스트도 돌려', '이 부분 리팩터링해!']) {
    check(`★ 반말 시킴 「${말.replace(/\n/g, ' ')}」 은 손대라는 말`, 손대라했나(말) === true && !읽기만하는모드.has(route(말).mode), `${손대라했나(말)} · ${route(말).mode}`);
  }
