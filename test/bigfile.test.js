...
// 빠르든 느리든 같은 뜻이 된다.
{
   ...
   *
   * 통째 쪽 잣대는 **한 배**다. 「우리 붙이기 한 판이 파일을 통째로 한 번
   * 읽는 것보다 싸다」 — 그것이 재려는 말의 전부다. 판마다 통째로 읽는
   * 구현으로 돌아가면 큰값이 통째값만큼 늘어나므로 반드시 이 선을 넘는다.
   *
   * 세 배를 요구해 봤다가 되돌렸다. 잣대를 제일 빠른 통째 읽기로 조이고
   * 나니 우리 것(46ms)과 그것(108ms)의 사이가 2.3배라, 세 배는 **코드가
   * 아니라 여유를 재는 수**가 된다 — 19회차에 버린 그 병이다. 지키려는
   * 말에서 곧장 나오는 수를 쓴다.
   */
  check('★★ 30MB 파일에 이어 붙이는 값이 크기를 안 탄다',
    배수 <= 4 && 큰값 < 통째값,
    `작은 파일 ${작은값.toFixed(0)}ms · 30MB ${큰값.toFixed(0)}ms · ${배수.toFixed(1)}배`
    + ` · 통째로 읽으면 ${통째값.toFixed(0)}ms`);
