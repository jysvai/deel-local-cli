...
// 빠르든 느리든 같은 뜻이 된다.
{
  ...
  // 1MB. 「크기를 타는가」 를 가르는 잣대다 — 30MB 와 **같은 길**을 지나면서
  // 크기만 다른 것이라야 한다(아래 머리말). 5줄짜리로 견주던 자리였는데,
  // 그것은 64KB 아래라 인코딩을 통째로 재는 다른 길로 간다.
  writeFileSync(join(뿌리, '한덩이.txt'), 한줄.repeat(Math.round(1024 * 1024 / Buffer.byteLength(한줄, 'utf8'))), 'utf8');

  /*
   * ── 값을 재기 **전에 결과부터 본다** ────────────────────────────────────
   *
   * 여기는 오래 시간만 쟀다. 그런데 30MB 쪽 Append 는 한동안 **통째로
   * 거절되고 있었다** — 인코딩을 앞 64KB 만 보고 재게 바꿨는데 그 자리가
   * 글자 한가운데라, UTF-8 한글 파일이 CP936 으로 판정됐다. 거절은 빠르니
   * 시간은 오히려 좋아 보였고, 검사는 내내 초록이었다.
   *
   * 「빨랐다」 는 「했다」 가 아니다. 그래서 붙일 때마다 답을 본다.
   */
  const 답보기 = [];
  const 붙이고보기 = (이름, 글) => {
    const r = 붙이기(이름, 글);
    if (r?.error) 답보기.push(`${이름}: ${String(r.error).split('\n')[0]}`);
    return r;
  };
  const 재기 = (이름) => {
    ...
    for (let n = 0; n < 5; n++) 붙이고보기(이름, `줄 ${n}\n`);
    return Number(process.hrtime.bigint() - 시작) / 1e6;
  };
  /*
   * ── 여러 번 재고 **제일 작은 것**을 쓴다 ────────────────────────────────
   *
   * 부하가 섞이면 잰 값은 **늘기만 한다.** 평균은 옆 검사 하나가 튀면 같이
   * 튀지만, 최솟값은 안 튄다. 한 번만 재고 울타리를 느슨하게 푸는 것보다
   * 세 번 재고 조이는 쪽이 훨씬 낫다 — 실제로 느슨하게 풀어 뒀더니
   * 「파일을 통째로 다시 세게 만드는」 어긋내기가 그 틈으로 샜다.
   *
   * 번갈아 재는 것도 같은 까닭이다. 한쪽만 부하를 맞으면 배수가 거짓말을 한다.
   */
  /*
   * ── 막는 쪽도 **같은 판에서** 잰다 (19회차) ─────────────────────────────
   *
   * 아래 절대 울타리(60ms)가 느린 CI 에서 빨개졌다 — 작은 파일 7ms · 30MB
   * 61ms · **3.0배**. 배수는 멀쩡한데 절대값만 걸렸다. 즉 그 60ms 는 코드가
   * 아니라 **그 기계의 디스크 속도**를 재고 있었다. 작은 파일은 작아서 안
   * 느려지고 30MB 만 느려지니, 「작은값 + 50」 으로 늘려 준 것도 안 먹는다.
   *
   * 그래서 못 박은 수를 버리고, **막으려던 구현을 같은 판에서 재서** 견준다.
   * 기계가 느려지면 두 수가 같이 커지므로 잣대가 안 흔들린다 — 박동·틈
   * 잣대를 고칠 때 쓴 것과 같은 방법이다.
   *
   * 재는 것은 **그중 제일 빠른 구현**이라야 한다 (2차 눈).
   *
   * 처음에는 고치기 전 그대로 `readFileSync(…, 'utf8').split('\n')` 을 쟀다.
   * 그런데 그것은 통째로 읽는 방법 중 제일 느린 축이다 — 이 기계에서 915ms.
   * 잣대가 그렇게 헐거우면, 같은 일을 더 빨리 하는 회귀(바이트로 읽어 0x0A만
   * 세기 · 105ms)가 그 틈으로 빠져나간다. **막으려는 것은 「느린 구현」 이
   * 아니라 「판마다 통째로 읽는 것」 이다.**
   *
   * 그래서 제일 빠른 통째 읽기로 잰다.
   */
  const 통째로재기 = () => {
    const 시작 = process.hrtime.bigint();
    // 판마다 30MB 를 통째로 읽고 줄을 센다 — 통째로 읽는 방법 중 제일 빠른 꼴이다.
    for (let n = 0; n < 5; n++) {
      const b = readFileSync(join(뿌리, '큰것.txt'));
      let c = 0; let i = -1;
      while ((i = b.indexOf(0x0a, i + 1)) !== -1) c += 1;
    }
    return Number(process.hrtime.bigint() - 시작) / 1e6;
  };
  ...
  let 통째값 = Infinity;
  for (let i = 0; i < 3; i++) {
    한덩이값 = Math.min(한덩이값, 재기('한덩이.txt'));
    큰값 = Math.min(큰값, 재기('큰것.txt'));
    통째값 = Math.min(통째값, 통째로재기());
  }
  /*
   * 나누는 값에 **바닥**을 깐다 (16회차).
   *
   * 바닥이 1ms 였다. 빠른 디스크(CI 의 tmpfs)에서 작은 파일 다섯 번이 1ms
   * 로 나오면 30MB 쪽 44ms 가 **35배**가 된다 — 그런데 그 44ms 는 아래 절대
   * 울타리(60ms) 안이고, 막으려던 것(판마다 통째로 다시 읽기)은 같은 표에서
   * 160ms 다. 즉 그 35배는 고장이 아니라 **눈금이 모자란 것**이었다.
   * 잣대보다 작은 값으로 나누면 배수는 잡음을 재는 수가 된다.
   *
   * 바닥은 이 잣대가 믿을 만해지는 자리 — 다섯 번 붙이기에 20ms 다. 이래도
   * 「통째로 읽기」 는 160/20 = 8배라 아래 울타리(4배)에 그대로 걸린다.
   */
  /*
   * ── 배수도 **같은 길을 지나는 것**끼리 잰다 (20회차) ────────────────────
   *
   * 5줄짜리에 견주던 배수가 덮개(coverage) 아래 CI 에서 29.7배로 빨개졌다.
   *
   *   작은 파일(5줄)  2ms
   *   1MB           596ms
   *   30MB          595ms
   *
   * 1MB 와 30MB 는 같다. 튀는 것은 **5줄짜리 하나**다. 64KB 를 넘으면 인코딩을
   * 앞 표본으로만 재고, 그보다 작으면 통째로 잰다 — 길이 갈린다. 그 갈림을
   * 덮개가 300배로 부풀리니, 그 배수는 크기가 아니라 **어느 길로 갔나**를
   * 재는 수가 된다.
   *
   * 재려는 말은 「파일이 커져도 값이 안 는다」 다. 그러면 잣대도 **같은 길을
   * 지나는, 크기만 다른 것**이라야 한다. 그래서 1MB 에 견준다.
   * 5줄짜리는 이제 재지 않는다 — 견줄 수 없는 것을 견주던 자리였다.
   */
  const 잴만한바닥 = 20;
  const 배수 = 큰값 / Math.max(한덩이값, 잴만한바닥);

  /*
   * ── 울타리를 어디에 세웠나 ──────────────────────────────────────────────
   *
   * 이 기계에서 실제로 잰 값이다(다섯 번 붙이기 기준).
   *
   *   작은 파일         28ms
   *   30MB · 고친 것    46ms   ← 붙인 만큼만 센다. 크기와 상관이 없다
   *   30MB · 통째로    108ms   ← 판마다 30MB 를 다시 읽는다(제일 빠른 꼴)
   *
   * 못 박은 ms 는 하나도 안 쓴다 — 그것이 19회차에 빨개진 까닭이었다.
   * 배수는 작은 파일에, 통째는 막으려던 구현에 견준다. 양쪽 다 같은 판에서
   * 잰 수라, 기계가 느려지면 견주는 쪽도 같이 느려진다.
   *
   * 처음에는 250ms 로 뒀는데 237ms 가 13ms 차이로 빠져나갔다. 울타리는
   * **지금 값**이 아니라 **막고 싶은 값**을 같이 보고 세워야 한다 —
   * 그 「막고 싶은 값」 을 이제 못 박는 대신 그 자리에서 잰다.
   */
  check('★★ 30MB UTF-8 한글 파일에 붙이는 것이 되기는 한다', 답보기.length === 0,
    답보기[0] ?? '거절 없음');
  const 마지막 = 붙이기('큰것.txt', '끝\n');
  check('★ 붙인 뒤 줄 수가 파일과 맞는다',
    Number(/전체 (\d+)줄/.exec(String(마지막.content))?.[1] ?? -1)
      === readFileSync(join(뿌리, '큰것.txt'), 'utf8').split('\n').length - 1,
    String(마지막.content ?? 마지막.error).slice(0, 70));

  /*
   * 다리를 둘로 놓는다. 둘 다 **같은 판에서 잰 값**에만 견준다.
   *
   *   배수   작은 파일에 견준다 — 붙인 만큼만 세는가
   *   통째   막으려던 구현에 견준다 — 판마다 통째로 다시 읽는가
   *
   * 통째 쪽 잣대는 **차이**로 본다. 「큰값 < 통째값」 으로 세웠다가 리눅스
   * CI 에서 빨개졌고, 그것이 또 **기계를 재고 있었다** (20회차).
   *
   *              우리 붙이기(5판)   30MB 통째 읽기(5판)
   *   윈도우          46ms              108ms      ← 초록
   *   리눅스 CI       48ms               28ms      ← 빨강
   *
   * 우리 값은 두 판에서 같다. 달라진 것은 **읽기 속도**뿐이다 — CI 의
   * tmpfs 는 30MB 를 5.6ms 에 읽는다. 그러니 「붙이기 한 판이 통째 읽기
   * 한 판보다 싸다」 는 참이 될 수도 거짓이 될 수도 있는 말이었다.
   * 우리 붙이기는 O(1) 이고 통째 읽기는 O(n) 이라, 둘을 맞대면 **n 이
   * 얼마나 커야 하는가**, 즉 그 기계의 디스크 속도를 묻게 된다.
   *
   * 재려는 말은 그게 아니다. **「파일이 커져도 값이 안 는다」** 다. 그러면
   * 잣대도 크기가 다른 같은 코드끼리여야 한다 — 그래서 1MB 를 같이 잰다.
   *
   *   지금 것     1MB 61ms · 30MB 60ms  → 더 든 것 -1ms
   *   되돌린 것   1MB 61ms · 30MB 169ms → 더 든 것 108ms (= 통째값 그대로)
   *
   * 판마다 통째로 읽는 구현으로 돌아가면 30MB 쪽만 통째값만큼 는다. 그래서
   * 선은 **통째값의 반**이다 — 안 읽는 것(0)과 한 번 다 읽는 것(통째값)의
   * 한가운데. 한 판에 반 벌이라도 읽고 있으면 그것이 이미 그 병이다.
   * 두 수 다 같은 판에서 잰 것이라 기계가 느려지면 선도 같이 내려간다.
   *
   * 1MB 를 잣대로 쓰는 까닭: 5줄짜리와 견주면 「작은 파일에만 있는 길」
   * (인코딩을 통째로 재는 자리)이 차이에 섞인다. 1MB 와 30MB 는 같은 길을
   * 지나므로, 남는 차이는 **크기 때문에 든 것**뿐이다.
   */
  check('★★ 30MB 파일에 이어 붙이는 값이 크기를 안 탄다',
    배수 <= 4 && (큰값 - 한덩이값) < 통째값 / 2,
    `1MB ${한덩이값.toFixed(0)}ms · 30MB ${큰값.toFixed(0)}ms · ${배수.toFixed(1)}배`
    + ` → 크기로 더 든 것 ${(큰값 - 한덩이값).toFixed(0)}ms`
    + ` · 통째로 읽으면 ${통째값.toFixed(0)}ms (반 ${(통째값 / 2).toFixed(0)}ms)`);
