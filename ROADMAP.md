...
|---|---|---|---|---|---|
| **0** | T0-1 | Corporate proxy that actually works (`HTTPS_PROXY` / config) — **done (da990ed·b177fbe)** | Docs promise `HTTPS_PROXY`; measured on Node 24.19: `fetch` with `HTTP_PROXY` set went **direct** (proxy saw 0 requests). On a no-direct-egress network the gateway is unreachable with no hint. | L | network |
| **0** | T0-2 | Retry with backoff on 429 / 5xx / reset — **done (5860276·0f29481)** | Shared gateways rate-limit per user; `Task` fans out calls; today one 429 throws in `adapter.js` and ends the turn. | S | — |
| **1** | T1-1 | Windows shell awareness (Git Bash → cmd, model is told) — **done (2d57e1e)** | `Bash` tool runs `cmd.exe` on Windows and the prompt never says so; models emit `ls`/`cat`/`grep`/`rm` → each failure is a 20–40 s round trip on a local model. | M | command exec |
