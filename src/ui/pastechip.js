...
// 붙여넣기를 다루는 쪽은 이 수를 같이 봐야 한다 — 화면에 몇 줄까지 펴서
// 남길지가 상자에 몇 줄이 들어가는지와 같은 값이라야 어긋나지 않는다.
export { 안쪽최대 };
 ...
 * 이 덩이가 상자에 안 들어가나.
 *
 * 개행이 하나라도 있으면 줄 수를 안 따지고 접는다 (아래 까닭). 개행이 없는
 * 한 덩이는 들어가면 접지 않는다 — 상자에 보이는 편이 낫다. 그 들어가는지를
 * 줄 수만 세서 보면 안 된다. 한 줄이어도 폭보다 길면 상자 안에서 여러 줄로
