...
export function 틀(n, { 바깥 = false, 곁말 = '' } = {}) {
  ...
  /*
   * 커서 한 칸은 이름 자리 **밖에** 미리 잡아 둔다.
   *
   * 남은 칸 안에서 그리면 이름이 다 자란 틀에서는 남은 칸이 없어 커서가 한 칸
   * 밖으로 밀리고, 첫 줄 폭이 틀마다 12·13 으로 달라진다. 다시 그리기는 커서를
   * 올려 같은 자리에 덮어쓰는 것이라, 넓었던 한 칸이 안 지워지고 조각으로 남는다.
   * 미리 한 칸을 잡아 두면 글자는 안 밀리고 폭은 어느 틀에서나 같다.
   */
  const 커서글 = f.커서 ? c.gray('▏') : ' ';
  // 다 자란 뒤에는 굵기를 뺀다. 굵은 채로 굳으면 '아직 찍히는 중' 으로 보인다.
  const 이름글 = f.커서 ? `${앞선글}${방금글}` : 색(자란글);
  const 첫줄 = `${들여}${c.bold(c.white(앞))}${이름글}${커서글}${남은칸}`;
