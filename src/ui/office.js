...
const C = {
  ...
  종이: 255, 종이어둠: 250, 쪽지: 227, 쪽지끝: 120, 초록: 46,
};
...
export function 사무실줄들(상태, 폭) {
  ...
  // 서버 불빛 14×5 — 모델 호출. 오른쪽 벽 끝.
  {
    ...
    for (let r = 0; r < 3; r++) {
      let 줄 = 'R';
      for (let k = 0; k < 12; k++) 줄 += k < Math.min(12, 부른횟수 * 2) ? 'L' : 'l';
      그림.push(줄 + 'R');
    }
    그림.push('RRRRRRRRRRRRRR');
    p.올리기(가로 - 15, 0, 그림, { R: C.검정, L: C.초록, l: C.쇠진 });
  }
...
export function 상태만들기(session, 일감, 틱 = 0, 할일들 = [], 도는하위 = 0) {
  ...
  if (일감) {
    ...
    const 수 = 1 + Math.max(0, 도는하위);
    for (let i = 0; i < 수; i++) 일꾼들.push({ 갈래 });
  }
