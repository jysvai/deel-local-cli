...
export class LineScreen {
  constructor() {
     ...
     */
    this.깊이 = 0;
    // 이번 턴에 '멈추는 중' 을 이미 적었나. 같은 줄을 되풀이해 찍지 않는다.
    this.멈춘다고했나 = false;
  }
  ...
  입력자리(session) {
     ...
     * 곧 지워질 글부터 걷는다.
     *
     * 이 줄이 없었다. 상자화면 쪽 같은 이름 메서드는 임시지움() 을 먼저
     * 부르는데 여기만 안 불렀다 — 그런데 아래 say('') 는 이 화면의 줄() 이
     * 아니라 ansi.js 의 say 라, 임시글을 걷는 문이 통째로 안 열렸다.
     *
     * 기다림() 은 `\r` 로 커서만 앞으로 보내 놓는다. 그 상태에서 say('') 가
     * 줄바꿈을 내면 커서만 아래로 내려가고 **「생각 중…」 은 화면에 그대로
     * 남는다.** 그 아래 상태줄이 붙으므로, 끝난 일의 문구가 입력칸 위에
     * 붙박인 것처럼 보인다.
     */
    this.임시지움();
    say('');
   ...
   */
  일시작() { this.멈춘다고했나 = false; }
  /*
   * 다만 '멈춤' 은 예외다 — 여기만 한 줄을 낸다.
   *
   * ESC 를 눌러도 줄 화면(`--no-tui`·파이프)에서는 **아무것도 안 나왔다.**
   * 멈추는 데는 잠깐이 걸리는데(돌던 자식을 죽이고, 안 돈 도구 자리를 채우고,
   * 대화를 성하게 닫는다) 그 사이 화면이 아까 하던 말을 그대로 하고 있으면
   * 사람은 안 먹었다고 판단한다. 그러면 또 누르고, 그러다 Ctrl+C 로 손이
   * 가고, 두 번 누르면 대화가 통째로 닫힌다 — 상자 화면에서 문구를 붙박이로
   * 둔 것과 똑같은 이유다.
   *
   * 한 번만 낸다. ESC 를 세 번 누르면 같은 줄이 세 번 찍히는데, 그건
   * 신호가 아니라 잡음이다.
   */
  일바꿈(갈래) {
    if (갈래 !== '멈춤' || this.멈춘다고했나) return;
    this.멈춘다고했나 = true;
    this.줄(`  ${c.gray(말('run.stopping'))}`);
  }
  일끝() { this.멈춘다고했나 = false; }
...
export class BoxScreen extends LineScreen {
  ...
  생각(s) {
    ...
    // 「생각」 은 상자 왼쪽 문구가 이미 말한다 — 곁에 또 붙이면 한 줄에 두 번 나온다.
    this.상자.일바꿈('생각', m ? `${m[1]}자` : null);
  }
