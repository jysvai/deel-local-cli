...
export const SEGMENTS = {
  ...
  verify: {
     ...
     * ── 못 확인한 것은 확인한 것이 아니다 ────────────────────────────
     *
     * 여태 갈래가 둘뿐이었다 — 탈이 있으면 빨강, 아니면 초록. 그래서
     * 「확인 0 · 못 확인 5」 가 초록 ✓0 으로 떴다. 바로 위에 적어 둔
     * 「초록 ✓ 가 없으면 아직 아무도 안 돌려 본 것이다」 를 뒤집어 읽으면,
     * 초록이 떠 있으니 돌려 봤고 탈이 없다는 뜻이 된다 — 다섯 가지를
     * 확인조차 못 한 판에서.
     *
     * 노랑을 하나 더 둔다. 빨강(탈이 났다) 과 초록(다 확인했다) 사이에
     * 「아직 모른다」 가 있어야 이 표시가 거짓말을 안 한다.
     */
    make: (s) => {
      const v = s.검증;
      if (!v?.돈횟수) return null;
      if (v.탈) return c.hred(`✓${v.확인} ✗${v.탈}`);
      return v.못확인 ? c.hyellow(`✓${v.확인} ?${v.못확인}`) : c.hgreen(`✓${v.확인}`);
    },
    short: (s) => {
      const v = s.검증;
      if (!v?.돈횟수) return null;
      if (v.탈) return c.hred(`✗${v.탈}`);
      return v.못확인 ? c.hyellow(`?${v.못확인}`) : c.hgreen(`✓${v.확인}`);
    },
  },
