// 사용자 수준. 화면에 무엇을 내놓을지만 정한다.
//
// 무엇을 바꾸는가:  보이는 명령의 개수, 설명의 말투, 첫 실행에서 물어보는 것
// 무엇을 안 바꾸는가: 안전 장치. 하나도 안 바꾼다.
//
// 초보라고 승인을 덜 받거나 작업 범위를 넓히지 않는다. 그건 배려가 아니라
// 위험을 떠넘기는 것이다. 초보일수록 되돌릴 수 있어야 한다.
//
// 감추는 것도 '못 쓰게' 가 아니라 '안 보이게' 다. 초보 수준에서도 /think 를
// 치면 그대로 먹는다. 목록에 안 띄울 뿐이다 — 처음 켠 사람에게 명령 쉰
// 개를 들이밀면 아무것도 못 고른다.
//
// 「열여덟 개」 라고 적혀 있던 자리다. 그 줄을 쓸 때는 명령이 통틀어 열여덟
// 개였는데(초보 목록은 열두 개) 그 뒤 쉰 개가 넘게 늘도록 숫자만 그대로여서,
// 마침 열여덟 개가 된 아래 목록이 제 머리말을 어기는 꼴이 됐다. 숫자만 고치면
// 또 낡으므로 검사가 실제 명령 수와 대 본다 (8회차 LV2).

import { 말 } from '../i18n/index.js';

export const LEVELS = {
  쉬움: {
    id: '쉬움',
    en: 'beginner',
    get name() { return 말('level.beginner.name'); },
    get hint() { return 말('level.beginner.hint'); },
    // 목록에 띄울 것. 나머지는 쳐도 먹지만 안 보인다.
    show: [
      'help', 'work', 'auto', 'code', 'plan', 'ask',
      // ctx 를 초보 목록에 넣는다. 컨텍스트가 작게 잡히면 "왜 파일을 조금만 읽지"
      // 가 되는데, 초보일수록 그 원인을 못 찾는다. 명령 하나로 끝나는 문제다.
      // diff 는 초보에게 특히 필요하다. auto 모드는 안 물어보고 고치니,
      // '무엇이 바뀌었나' 를 볼 통로가 없으면 되돌릴지 말지도 못 정한다.
      // bell 도 초보 목록에 둔다. 로컬 모델은 한 턴이 몇 분씩 걸려서 다들 다른 창으로
      // 가는데, 알림이 있는 줄 모르면 그 몇 분을 화면만 보고 앉아 있게 된다.
      // lang 은 반드시 초보 목록에 있어야 한다. 화면이 온통 못 읽는 말인 사람에게
      // '전부 보려면 /level developer' 라고 적어 봐야 그 줄도 못 읽는다.
      'model', 'ctx', 'scan', 'diff', 'undo', 'bell', 'lang', 'clear', 'sessions', 'cost', 'level', 'exit',
    ],
    // 첫 실행에서 훑어 추천까지 해 준다
    autoScan: true,
    // 오류를 무엇을 하라는 말로 바꿔 준다
    plainErrors: true,
  },

  개발자: {
    id: '개발자',
    en: 'developer',
    get name() { return 말('level.developer.name'); },
    get hint() { return 말('level.developer.hint'); },
    show: null,        // null = 전부
    autoScan: false,
    plainErrors: false,
  },
};

export const DEFAULT = '쉬움';
export const ORDER = ['쉬움', '개발자'];

export function normalize(v) {
  const s = String(v ?? '').trim().toLowerCase();
  if (!s) return null;
  const 별명 = {
    '쉬움': '쉬움', '초보': '쉬움', '초보자': '쉬움', 'beginner': '쉬움', 'easy': '쉬움', 'b': '쉬움',
    '개발자': '개발자', '고급': '개발자', 'developer': '개발자', 'dev': '개발자', 'advanced': '개발자', 'd': '개발자',
  };
  /*
   * 표에 **제 것으로 있는** 열쇠만 본다.
   *
   * `별명[s]` 는 물려받은 열쇠에도 값을 준다. `/level constructor` 면 함수
   * 하나가 수준 이름으로 통과해서, 그 뒤 shows() 가 `lv.show` 를 읽다가
   * 터진다 — 명령 목록을 그리는 자리라 대화가 통째로 끝난다.
   */
  if (Object.hasOwn(별명, s)) return 별명[s];
  return Object.hasOwn(LEVELS, v) ? v : null;
}

export function get(id) {
  return LEVELS[normalize(id) ?? DEFAULT];
}

/** 이 수준에서 목록에 띄울 명령인가. 안 띄운다고 못 쓰는 것은 아니다. */
export function shows(levelId, cmd) {
  const lv = get(levelId);
  return lv.show === null || lv.show.includes(cmd);
}

/**
 * 오류를 초보에게 맞는 말로 바꾼다.
 *
 * 원인을 숨기지 않는다 — 무엇을 하면 되는지를 앞에 놓고, 원래 문구는 뒤에 남긴다.
 * 원인을 지우면 물어볼 수도 없게 된다.
 *
 * 순서가 뜻을 정한다. 먼저 걸린 것이 나가므로 **좁은 것을 넓은 것 위에** 둔다.
 */
const 풀이 = [
  /*
   * 시간 초과가 맨 앞이다. Node 가 내는 이름은 `ETIMEDOUT` 인데, 그 안에는 timeout 이라는
   * 글자가 없다(TIMEDOUT). 그래서 `read ETIMEDOUT` 은 어느 풀이에도 안 걸려 날것으로 나갔고,
   * `connect ETIMEDOUT` 은 아래 넓은 `connect` 에 먼저 걸려 「모델이 안 켜져 있다」 는 틀린
   * 풀이가 나갔다 — 모델은 떠 있고 느린 것이라 그 말대로 해도 안 고쳐진다 (8회차 LV1).
   */
  {
    when: /timeout|ETIMEDOUT|시간 초과/i,
    say: '모델이 제때 답하지 않았습니다.\n  큰 모델이면 원래 느립니다. 잠시 뒤 다시 하거나 더 작은 모델을 골라 보세요.',
  },
  /*
   * 우리가 우리 말로 적은 풀이 셋이 `connect` **위**에 선다 (10회차 LV3).
   *
   * LV1 은 ETIMEDOUT 만 위로 올리고 이 셋은 그대로 뒀는데, 넓은 것은 ETIMEDOUT 만이
   * 아니었다. `connect` 는 글자 넉 자라 **경로와 주소에 그대로 들어간다** — `connect-api/`,
   * `connector.js`, 사내 게이트웨이 `connect.…`. 이 셋의 문구에는 그 경로·주소가 같이
   * 실리므로, 아래에 있는 동안 「범위 밖입니다」 가 「모델이 안 켜져 있습니다 — LM Studio 를
   * 켜세요」 로 나갔다. 모델은 켜져 있고, 시킨 대로 해도 안 고쳐진다.
   *
   * 반대 방향은 샐 일이 없다. 이 셋은 우리가 그 자리에서 지어 내는 문구라, 모델 서버가
   * 보내온 연결 오류에 「작업 범위 밖」 이 섞여 올 길이 없다.
   */
  {
    when: /허용되지 않은 주소/,
    say: '지금 연결된 곳이 아닌 데로 나가려 했습니다. 막힌 게 정상입니다.\n  다른 모델을 쓰시려면 /model 로 고르세요.',
  },
  {
    when: /작업 범위 밖/,
    say: '시작한 폴더 바깥의 파일은 건드리지 않습니다.\n  그 파일이 꼭 필요하면, 그 폴더에서 deel 을 다시 켜세요.',
  },
  {
    when: /먼저 Read 로 읽어야/,
    say: '고치기 전에 파일을 먼저 읽게 되어 있습니다. 잠시 후 다시 시도합니다.',
  },
  {
    when: /ECONNREFUSED|연결할 수 없|connect|fetch failed/i,
    say: '모델이 안 켜져 있는 것 같습니다.\n  LM Studio 나 Ollama 를 켜고 모델을 하나 올린 다음 다시 해보세요.\n  어떤 것이 떠 있는지 보려면 /scan',
  },
  {
    when: /401|403|인증|unauthorized/i,
    say: '열쇠(API 키)가 없거나 맞지 않습니다.\n  deel setup 으로 다시 넣어 주세요.',
  },
];

export function explain(levelId, message) {
  const lv = get(levelId);
  const raw = String(message ?? '');
  if (!lv.plainErrors) return { text: raw, plain: false };
  const hit = 풀이.find((r) => r.when.test(raw));
  if (!hit) return { text: raw, plain: false };
  return { text: hit.say, detail: raw, plain: true };
}
