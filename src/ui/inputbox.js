 ...
 */
import { c, width, cursor, clip, 판시작, 판끝, 화면글거르기 } from './ansi.js';
...
export function 프레임({ 글 = '', 커서: 커서자리 = null, 폭 = 80, 상태 = '', 경고 = '', 곁말 = '', 일감 = null, 추천 = [], 바깥 = false, 사무실 = null } = {}) {
   ...
   * 칸은 창보다 넓게 잡지 않는다.
   *
   * `Math.max(20, 폭)` 이었다 — 10칸 창에 20칸짜리 줄을 냈다. 터미널은 그 줄을 두 줄로 접고
   * 상자는 한 줄로 세니, 좁은 창(분할 창 · 옆에 붙인 창)에서 키를 칠 때마다 대화가 지워졌다.
   * 아주 좁으면 상자가 제 모양을 못 갖추지만, 모양이 깨지는 것이 대화가 지워지는 것보다 낫다.
   * 그래서 끝에서 한 번 더 폭 안으로 자른다(폭안).
   */
  const 칸 = Math.max(1, 폭);
  const 폭안 = (줄들) => 줄들.map((l) => (width(l) > 칸 ? clip(l, 칸) : l));
  /*
   * 사람이 친 글을 상자 안 줄로 접는다.
   *
   * 탭은 빈칸 넷으로 그린다. 날것으로 내보내면 터미널이 다음 탭 자리까지 밀어 그려서 몇 칸인지
   * 아무도 모른다 — 엑셀에서 한 줄을 붙이면 탭이 아홉 개 온다. 끝의 빈칸뿐인 줄도 버리지 않는다
   * (커서가 거기 선다 — wrap.js 의 빈칸줄남김).
   */
  const 탭 = String.fromCharCode(9);
  const 접기 = (s, w) => 접어쓰기(String(s ?? '').split(탭).join('    '), w, { 빈칸줄남김: true });
  /*
   * ── 테두리가 곧 경계선이다 ───────────────────────────────────────────
   *
  ...
  const 안쪽 = Math.max(2, 칸 - 앞머리 - 2);   // 오른쪽 ' │' 두 칸
  const 줄들 = [];
  ...
  const 가로 = '─'.repeat(Math.max(0, 칸 - 3));
  줄들.push(테(` ╭${가로}╮`));
  ...
  if (일감) {
    ...
    const 일안쪽 = 안쪽 + 2;
    /*
     * 여기 실리는 글은 **우리 것만이 아니다** — 들어온 제어 순서를 뗀다.
     *
     * 곁정보에는 모델이 적은 글이 온다(repl.js 가 `일바꿈('하위', clip(ev.목적, 24))` 로
     * 넣는 그 값은 모델이 Task 를 부를 때 적은 목적이다). 같은 글이 say() 로 나갈 때는
     * 걸러지는데(ansi.js 화면글거르기), 상자는 say 를 안 지나고 stdout 에 곧장 쓴다 —
     * 한쪽만 막혀 있었다. 클립보드를 바꾸거나(OSC 52) 윗줄을 덮어써서 「✗ 실패」 를
     * 「✓ 통과」 로 만드는 자리라, 폭이 어긋나는 것보다 무겁다.
     *
     * 우리 색은 남긴다 — 바로 아래 줄들이 그 색으로 그려진다(상태줄도 같은 길이다).
     */
    const 걸러 = (s) => 화면글거르기(s, { 색남김: true });
    /*
     * 줄바꿈과 탭은 거르개가 **일부러 남기는** 글자다(대화 글에는 있어야 한다). 그런데 여기는
     * 한 줄이다. \n 하나가 들어오면 터미널은 두 줄에 그리는데 상자는 `줄들.length` 로 한 줄만
     * 세고 그만큼만 커서를 올린다 — 그 뒤 판마다 한 줄씩 엇나간 자리에 덮어써서 위쪽 대화가
     * 갉여 나간다. 탭은 터미널이 다음 탭 자리까지 밀어 그려 폭 셈이 통째로 틀어진다(사람이 친
     * 글은 이미 위 접기 에서 빈칸 넷으로 편다). 한 줄 요약이니 빈칸 하나로 눕힌다 — 글자 수가
     * 안 바뀌어서 부르는 쪽이 이미 잰 폭(clip 24)도 그대로다.
     */
    const 한줄로 = (s) => 걸러(s).replace(/[\n\t]/g, ' ');
    const 왼쪽 = `${c.hcyan(일감.돌림 ?? '⠋')} ${c.white(한줄로(일감.말 ?? ''))}${c.gray('…')}`;
    const 오른쪽 = c.gray(한줄로(일감.곁 ?? ''));
     ...
     */
    const 대기줄들 = 일감.대기 ? 접기(일감.대기, 안쪽) : [];
    const 대기보일 = 대기줄들.slice(-대기최대);
    const 대기넘침 = 대기줄들.length > 대기최대;
    for (const [i, 한줄] of 대기보일.entries()) {
      const 표 = i === 0 ? (대기넘침 ? c.gray('…') : c.hcyan('❯')) : ' ';
      줄들.push(` ${테('│')} ${표} ${한줄}${' '.repeat(Math.max(0, 안쪽 - width(한줄)))} ${테('│')}`);
    }

    줄들.push(테(` ╰${가로}╯`));
    // 이 아래 줄도 폭 안에 넣는다 — 안내 문구는 43칸이라 좁은 창에서 접힌다.
    if (일감.대기) 줄들.push(clip(` ${c.gray(말('box.queuedHint'))}`, 칸));
    else if (곁말) 줄들.push(clip(` ${c.gray(곁말)}`, 칸));

    // 커서는 미리 치는 중일 때만 보여 준다. 아니면 칠 자리가 아니다.
    if (일감.대기) {
      const 마지막 = 대기보일.at(-1) ?? '';
      return { 줄들: 폭안(줄들), 커서: { 위: 2, 열: Math.max(1, Math.min(앞머리 + 1 + width(마지막), 칸 - 1)) } };
    }
    return { 줄들: 폭안(줄들), 커서: { 위: (곁말 ? 1 : 0), 열: 1 } };
  }
  ...
  const 커서앞줄수 = (커서자리 == null ? 글 : String(글).slice(0, 커서자리))
    ? 접기(커서자리 == null ? 글 : String(글).slice(0, 커서자리), 안쪽).length
    : 1;
  const 첫줄 = 넘침
  ...
  if (추천?.length) {
    ...
    if (추천.length > 보일것.length) {
      줄들.push(`     ${c.gray(말('box.more', { n: 추천.length - 보일것.length }))}`);
      추천줄수 += 1;
    }
  }

  // 곁말도 폭 안에 넣는다 — 위 머리글의 그 셈(줄 수)이 어긋난다.
  ...
  const 앞줄들 = 앞글 ? 접기(앞글, 안쪽) : [''];

  // 위로 접어 둔 줄만큼 줄 번호가 밀린다 (보일 자리는 커서를 따라 옮긴다).
  ...
  const 열 = Math.max(1, Math.min(앞머리 + 1 + width(앞줄들.at(-1) ?? ''), 칸 - 1));

  return { 줄들: 폭안(줄들), 커서: { 위, 열 } };
}
...
export class InputBox {
  ...
  일그리기() {
    ...
    this.그리기(this.session, '', 0, {
      ...
      곁: [
        ...
        this.일감.예약수 ? 말('box.queued', { n: this.일감.예약수 }) : '',
  ...
  그리기(session, 글 = '', 커서자리 = null, 일감 = null, 추천 = []) {
     ...
     *   폭이 바뀌었다   — 옛 수는 다 틀어졌다. 대신 상자 맨 윗줄로 올라가
     *                    거기서 아래를 통째로 쓸어낸다. 상자는 늘 맨 아래에
     *                    있어서 그 아래에는 아무것도 없고, 맨 윗줄보다 위로는
