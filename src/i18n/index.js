 ...
 * 자리를 찾는 무늬는 **한 벌**이다.
 *
 * 아래 말() 이 쓰는 것과, 「네 말의 자리 이름이 서로 같은가」 를 재는 검사가
 * 쓰는 것이 따로였다. 그런데 검사 쪽은 `[^{}s:]` 라고 적혀 있었다 —
 * 백슬래시 하나가 빠져서 **빈칸이 아니라 글자 `s` 를 뺐다.**
 *
 * 그래서 이름에 `s` 가 든 자리를 통째로 못 읽었다. 실제로 있는 말이다 —
 *
 *     "{turns}개 턴, 파일 {files}개를 되돌렸습니다."
 *
 * 검사는 이 글에서 자리를 **0개**로 읽었다. 네 말 다 0개니 완벽한 짝이다.
 * 누가 옮기면서 `{files}` 를 빠뜨려도 초록이고, 화면에는 「파일 個を元に
 * 戻しました」 — 숫자가 소리 없이 사라진 문장이 나간다.
 *
 * 재는 쪽과 쓰는 쪽이 다른 무늬를 보면 검사는 무엇이든 통과시킨다. 그래서
 * 무늬를 내보내고 양쪽이 같은 것을 쓴다. 이 저장소가 이미 밟은 함정이다 —
 * 백슬래시 하나가 규칙을 통째로 바꿔 놓고 아무 데서도 안 터진다.
 */
const 자리무늬 = () => /\{([^{}\s:]+)(?::([^{}\s]+))?\}/g;

/** 이 글에 든 자리 이름들. 조사 표시(`~`)는 떼고, 같은 이름은 한 번만. */
export function 자리이름들(글) {
  const 난것 = new Set();
  for (const m of String(글 ?? '').matchAll(자리무늬())) {
    난것.add(m[1].startsWith('~') ? m[1].slice(1) : m[1]);
  }
  return [...난것].sort();
}
...
export function 말(열쇠, 채움 = null) {
   ...
   * 틀린 숫자는 안 적힌 것보다 나쁘다.
   */
  return String(글).replace(자리무늬(), (통째, 이름, 갈래) => {
