...
export const en = {
  ...
  'undo.talkPartial': 'Of those, {n} turn(s) had already been folded into a summary, so their messages stay in the conversation.',
  'undo.notesDropped': 'The {n} pinned "what you asked" note(s) from that turn were removed, though, so the reverted work is not requested again.',
  ...
  'export.evidenceFailed': 'Evidence could not be collected ({왜}) — so this report cannot claim everything was proven.',
  ...
  'approve.newFile': 'new file',
  'approve.locked': 'A managed policy keeps approval at {바닥} or stricter — {곳}',
  'approve.lockedUnknown': 'The managed policy\'s approval value "{값}" is unknown, so it counts as strict, the narrowest — {곳}',
  'approve.noAlways': 'A managed policy sets approval, so "don\'t ask again" is not offered',
  'work.title': 'Work modes',
  'work.canEdit': 'edits files',
  'work.readOnly': 'read-only',
  'work.think': 'thinking',
  'work.steps': 'up to {n} steps',
  'work.tools': 'tools',
  'work.toolsWrite': 'read + change files',
  'work.toolsRead': 'read only — cannot change files',
  'work.howTo': 'To switch: {a} {b} {c} …  or {키} to cycle',
  'work.axis': 'A different axis from the approval policy ({명령}) — what you do / how much it asks.',
  'work.autoOn': 'From here it moves to the right mode by itself, message by message.',
  'work.pinned': 'You chose this, so it will not change by itself. To hand it back:',
  'trust.ignored': "This folder's project config was not read — the folder is not trusted yet",
  'trust.ignoredWhy': 'A config file ships with the repository. Reading a stranger\u2019s means it picks the endpoint and the command that fetches your key.',
  'trust.howTo': 'To read it:',
  'trust.filtered': 'Dropped the keys a project config may not set',
  'trust.trusted': "This folder's project config is being read",
  'trust.already': 'Already trusted',
  'trust.removed': 'Removed from the trusted list',
  'trust.notListed': 'That folder was not on the trusted list',
  'trust.parentStill': 'Still trusted, through a parent folder',
  'trust.stillTrustedEnv': "Still trusted, because of an environment setting on this machine",
  'stats.title': 'What happened in this folder',
  'stats.none': 'Nothing recorded yet — the log starts the first time deel does something here.',
  'stats.period': 'Period',
  ...
  'stats.tools': 'Tools',
  'stats.failed': '{n} failed',
  'stats.unknown': '{n} unknown',
  'stats.blocked': 'Blocked',
  'stats.undos': 'Undos',
  'stats.masked': 'Secrets masked',
  'stats.broken': '{n} lines could not be read — the numbers above are short by that much.',
  'stats.older': '{n} more lines fall outside the window (--all for everything).',
  'stats.noCost': 'Tokens and cost are not in this log. For the current conversation, use /cost.',
  'trust.listTitle': 'Trusted folders',
  'trust.listNone': 'No folder is trusted yet.',
  'trust.listIgnoredWide': 'ignored — too wide to trust (run deel trust --off in that folder to remove it)',
  'trust.blockedTitle': 'Keys a project config may never set, even when trusted',
  'trust.saveFail': 'Could not write the trusted list',
  'trust.refuseWide': 'Not trusting this folder',
  'trust.refuseWideWhy': 'deel trust covers this folder and everything below it. Trusting a home folder or a drive root would read the config of every repository cloned underneath',
  'trust.refuseWideHow': 'Change into the repository you mean and run deel trust there',
  'trust.why.allow': 'It widens the approval rules. A deny, which narrows them, is still read',
  'trust.why.apiKey': 'A key written into a repository is not a setting, it is a leak',
  'trust.why.authCmd': 'It runs before the first request — ahead of tool approval, so nothing catches it',
  'trust.why.connection': "Changing the address, API kind or auth of this PC's profile would send this PC's key where the repository chooses",
  'trust.why.online': 'Permission to go outside is only granted where a person chooses it',
  'trust.why.active': 'It would switch the connection to a profile the repository added. Picking a profile from this PC is still read',
  'trust.why.offline': 'It lifts the seal. Turning it on (true) is still read',
  'trust.why.proxy': 'Every request, key header included, would pass through that proxy',
  'trust.why.shellEnv': 'It hands secret environment variables back to shell commands',
  'trust.why.profilesNotArray': 'profiles is not a list ([ … ]), so none of it was read',
  ...
  'plan.timedOut': 'No answer in {초}s — going ahead with the plan.',
  'plan.autoGo': 'Auto mode — going ahead with the plan without asking.',
  ...
  'ask.autoWait': 'If no answer comes in {초}s, it carries on by itself',
  'ask.timedOut': 'No answer in {초}s — carrying on by itself.',
  'ask.autoSkip': 'Auto mode, so it carries on without asking — {물음}',
  ...
  'tier.nfc': 'ignoring Unicode normalization (NFC/NFD)',
  ...
  'box.more': '{n} more',
  'box.queuedHint': 'Press Enter to send it once the current task finishes',
  'box.queued': '{n} queued',
  ...
  'ev.phaseMove': '{왜} — from here on this runs as {새}, not {옛}',
  ...
  'ev.hookNote': 'hook: {말}',
  'ev.hookBlock': 'A hook blocked this',
  'ev.nudgeRead': 'It was about to finish having only read things — pushed back once',
  'check.start': 'Completion check ({판}/{최대}) — {명령}',
  'check.pass': 'Completion check passed — {명령}',
  'check.fail': 'Completion check failed ({판}/{최대}) — {요약}',
  'check.retry': 'Sending the failure back to fix',
  'check.gaveUp': 'Still failing after {판} runs — ending as failed',
  'check.skipped': 'Could not run the completion check — {까닭}',
  'cli.checkFlag': 'run this check before finishing; failures go back to be fixed, and still failing ends with 8',
  'cli.evalTitle': 'Golden set',
  'cli.evalWhy': '— run a set of tasks on the real model and measure how many it gets done',
  'cli.evalInit': 'create five example tasks in golden/',
  'cli.evalRun': 'run deel run on each task in a temp folder, then grade it with checks it never saw',
  ...
  'run.thoughtChars': 'thought {n} chars',
  'run.bye': 'Bye.',
  ...
  'cost.cacheLine': 'read {읽음} · written {씀} · {몫}% hit',
  ...
  'net.limit.oneTooBig': 'This single request is bigger than the per-minute limit — a new window will not help, because this one request still exceeds it. Shrink the prompt or attached files (/compact), lower the answer length cap, or raise the limit.',
