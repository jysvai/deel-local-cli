...
export async function chatLoop(opts = {}) {
  ...
  rl.on('line', (원래줄) => {
     ...
     * ── 접어 둔 붙여넣기를 여기서 편다 ──────────────────────────────────
     *
     * 화면에 남는 것과 모델에게 가는 것이 여기서만 일부러 다르다. 모델에게는
     * **언제나 원문**이 간다. 화면 쪽은 크기를 본다 — 상자에 들어갈 만한
     * 덩이면 그대로 펴서 남기고, 그보다 크면 `[붙여넣기 #1 · 47줄 · 2.1KB]`
     * 를 남긴다(ui/pastechip.js 의 남길글). 표에는 줄 수와 크기가 적혀 있어서
     * 나중에 스크롤을 올려도 무엇을 시켰는지 알아볼 수 있다.
     *
     * 상자에서 접는 기준과 화면에 남기는 기준이 다른 것은 일부러다. 상자는
     * `rl.line` 이라 여러 줄을 아예 못 그리고, 화면은 그냥 찍는 자리다.
     *
  ...
  if (process.stdin.isTTY) {
    ...
    process.stdin.on('keypress', (_ch, key) => {
      ...
      if (key?.sequence === '\x1b[201~') {
         ...
         * ── 일하는 도중에 붙여도 접는다 ──────────────────────────────────
         *
         * 여태 이 갈래는 `입력기다림` 일 때만 돌았다. 그런데 붙여넣기는
         * **일하는 도중이 더 흔하다** — 몇 분짜리 턴이 도는 동안 다음에
         * 시킬 것을 미리 붙여 두는 것이 그것이다.
         *
         * 그때 무슨 일이 났나. 붙인 마흔 줄 중 마지막 한 줄만 rl.line 에
         * 남고 나머지는 상자 밖 버퍼로 간다. 일하는 중 화면(대기갱신)은
         * rl.line 만 그리므로, 사람 눈에는 **한 줄만 들어간 것**으로 보인다.
         * 그래서 다시 붙인다. 이번엔 진짜로 두 번 들어간다 — 접기가 애초에
         * 막으려던 바로 그 사고가, 기다릴 때는 안 나고 일할 때만 났다.
         *
         * 펴는 자리는 그대로다(아래 붙임펼치기). 어느 길로 보내든 원문이
         * 간다 — 접는 자리만 넓혔지 되돌리는 길은 안 건드렸다.
         */
        if (붙인줄들 && 묻는중 === null && 붙임접을까(붙인것, { 폭 })) {
          const 번호 = ++붙임번호;
          붙인것들.set(번호, 붙인것);
          ...
          if (입력기다림) {
            화면.입력지움();
            say(`  ${c.gray(옮긴말('paste.folded', { 표 }))}`);
          } else {
            /*
             * 일하는 중에는 대화에 안 찍는다. 찍으면 이미 보낸 것처럼 보인다
             * (아래 미리 쳐 둔 것 갈래와 같은 까닭이다). 일감 상자에 지금
             * 들고 있는 것으로 그려 주면 그것으로 족하다.
             */
          ...
          // 알고 다시 붙인다.
          화면.입력지움();
          say(`  ${c.gray(옮긴말('paste.pasted', { n: 이어쓰기줄들.length + 1 }))}`);
        }
  ...
  const ctx = {
    ...
    lsp: { 켬: true },
    /*
     * 종을 울릴지. `/bell on|off` 가 **이 그릇**을 고친다.
     *
     * 앞서는 명령이 설정 파일만 고쳤다. 울릴지 말지는 켤 때 한 번 읽어
     * 둔 값(알림.켬)이 정하므로, `/bell off` 를 쳐도 그 세션 내내 계속
     * 울렸다 — 화면은 「이 세션에도 바로 먹습니다」 라고 적으면서.
     * `/bell on` 은 더 나빴다: 그 자리에서 한 번 울려 켜졌다는 증거까지
     * 보여 준 다음, 다음 턴부터 안 울렸다.
     */
    종알림: 알림,
    ask,
    ...
    ask물음: async (물음, 고를것 = [], 이해 = '') => {
       ...
       * 무엇을 고른 것인지 모른다.
       *
       * 그리고 **고르면서 한마디 덧붙일 수 있다.** 여태는 둘 중 하나였다 —
       * 숫자만 치면 고른 것이고, 그 밖의 글은 고른 것 없는 자유 답이었다.
       * 그래서 「2 파일은 나눠서」 라고 치면 2번을 골랐다는 사실이 사라졌다.
       * 가르는 규칙은 ui/pick.js 머리말에 적어 두었다.
       */
      const 푼것 = 고른것풀기(답, 고를것);
      const 고른것 = 푼것.글;
      /*
       * 되비추는 줄에도 **둘 다** 보인다. 고른 것만 보이면 덧붙인 말이
       * 갔는지 안 갔는지 알 길이 없어서, 사람은 다음 턴에 그 말을 또 친다.
       */
  ...
  for (;;) {
    ...
    if (이어갈모드) {
      ...
      if (골라진.mode) {
        ...
        /*
         * 골랐어야 할 만한 신호가 있었는데 **일부러 안 보낸** 자리다.
         *
         * 「설계 신호가 있었지만 고치라는 말이라 안 보냈다」 와 「그냥
         * 모르겠다」 는 사람에게 아주 다른 말이다. 앞엣것을 안 찍으면,
         * 설계를 기대한 사람은 종합으로 도는 화면을 보고 자동 모드가
         * 고장 났다고 여긴다. 뒤엣것은 안 찍는다 — 보통 한마디가 다
         * 그쪽이라 매번 뜨면 소음이다. 가르는 일은 route 가 한다.
         */
        say('');
        say(`  ${c.gray('◇ 종합 그대로 —')} ${c.gray(골라진.why)}`);
      }
    }
    ...
    try {
      ...
      for await (const ev of run(session, ctx, 보낼글, { signal: turn.signal, 그림들: 보낼그림, 끼어들기 })) {
        ...
        switch (ev.type) {
          ...
          /*
           * 상한에서 잘린 것이 아니라, 서버가 **끝난 까닭을 한 번도 안 주고**
           * 흘려보내기를 멈춘 것이다. 여태 이건 정상 종료와 구별이 안 돼서
           * 중간에서 끊긴 답이 온전한 답인 척 지나갔다.
          ...
          case 'refusal': {
            ...
            say('');
            /*
             * 흘려 나온 것이 없으면 거절 글을 여기서 적는다.
             *
             * 이 규격도 거절 글을 조각으로 보내기는 한다. 다만 그 조각을 답
             * 자리로 흘려보내지 않고 맨 끝에 한 번에 모아 준다(adapter.js 의
             * 흡수). 그래서 화면에는 아무것도 안 흘러나오고, 이 자리가 없으면
             * 「거절당했습니다」 한 줄만 남는다.
             */
            if (!흘렸나 && ev.text) { say(`  ${ev.text}`); say(''); }
    ...
    if (계획승인받나 && !턴탈났나) {
       ...
       * ── 골라진 것 + 하고 싶은 말 ────────────────────────────────────
       *
       * 여태는 셋 중 하나였다. ⏎ 면 진행, n 이면 그만, 그 밖의 글은 통째로
       * 「이걸 고쳐서 다시 내라」. 그래서 「그대로 하되 검사도 넣어줘」 라고
       * 치면 **계획을 다시 짜러 갔다** — 사람은 진행하라고 한 것인데.
       * 반대로 「3단계는 빼자」 도 그냥 글 한 줄로 갔다. 어느 단계 얘긴지
       * 모델이 다시 세어 봐야 한다.
       *
       * 이제 다섯이다. 가르는 규칙은 ui/pick.js 의 계획답풀기() 에 있다.
       *   ⏎ · y            → 그대로 진행
       *   y 검사도 넣어줘    → 진행하되 그 말을 얹어서
       *   n                → 그만
       *   3 이건 빼자        → 3단계에 대고 하는 말로 붙여 다시 짜라
       *   그 밖의 글         → 여태 그대로 (다시 짜라)
       */
      const 계획답 = 계획답풀기(답, 할일.length);

      if (계획답.갈래 === '그만') {
        say(`  ${c.gray(옮긴말('plan.cancelled'))}`);
      } else if (계획답.갈래 === '진행') {
        /*
        ...
        이어갈것 = (할일.length
          ...
          /*
           * 승인하며 덧붙인 말은 **맨 뒤에** 붙인다.
           *
           * 계획 다음에 오는 말이라 「계획은 그대로, 다만 이것」 으로 읽힌다.
           * 앞에 붙이면 계획보다 먼저 읽혀서, 작은 모델은 그 한마디만
           * 하고 계획은 안 한다.
           */
          + (계획답.덧말 ? `\n\n사람이 승인하며 덧붙인 말: ${계획답.덧말}` : '');
        이어갈모드 = 'code';
        say('');
        say(`  ${c.hgreen('▶')} ${c.gray(계획답.덧말
          ? 옮긴말('plan.goingWith', { 말: clip(계획답.덧말, 50) })
          : 옮긴말('plan.going'))}`);
      } else {
        // 'e' 를 따로 두지 않는다 — 고칠 점을 바로 치는 것이 한 걸음 짧다.
        // 단계 번호를 댔으면 그 단계에 대고 하는 말로 적어 준다. 모델이
        // 어느 단계 얘긴지 다시 세어 볼 까닭이 없다.
        const 고칠말 = 계획답.단계
          ? `${계획답.단계}단계에 대해: ${계획답.덧말}`
          : 계획답.덧말;
        이어갈것 = `계획에서 이걸 고쳐서 **계획만** 다시 내라 (아직 실행하지 마라): ${고칠말}`;
        이어갈모드 = 'plan';
        say('');
        say(`  ${c.hcyan('☰')} ${c.gray(계획답.단계
