...
export function 잠그기(글, { 프로필id = null } = {}) {
  ...
  // 여기는 해 보고 진 것이 아니라 **해 볼 것이 없는** 자리다 (해보고졌나 머리말).
...
/*
 * ── 그 까닭이 **해 보고 진 것**인가 (2.0.0 16회차) ──────────────────────
 *
 * 리눅스·그 밖의 갈래는 잠금장치가 아예 없어서 잠그기() 가 **아무것도 안 해
 * 보고** 돌아온다. 그런데 그 자리도 못잠근까닭 을 적었고, 보관방식() 은 그것을
 * 「잠그려다 실패했습니다」 로 옮겼다 — 안 한 일을 했다고 적은 것이다. 이 판
 * 내내 잡아 온 부류를 여기서 하나 더 만들고 있었다.
 *
 * 값은 두 겹으로 나갔다. 첫째, 리눅스 사람에게 「시도했다 실패」 라고 적으니
 * 무엇을 고치면 되는지 알 길이 없다 (쓸수있나() 는 같은 사실을 「이 운영체제에서는
 * 파일 권한(0600)으로만 둡니다」 로 바르게 적고 있었다). 둘째, 이 한 줄이
 * 「DEEL_KEYSTORE=off 로 꺼 두었습니다」 까지 **덮어썼다** — 스스로 끈 사람에게
 * 「잠그려다 실패했습니다」 라고 답했다.
 *
 * 까닭 자체는 안 지운다. config.js 가 「몇 개를 왜 못 잠갔나」 에 그대로 쓴다.
 * 대신 **해 봤는지**를 같이 적어서, 말을 고르는 자(보관방식)가 가려 쓰게 한다.
 */
let 해보고졌나 = false;
...
function 껐나() { return process.env.DEEL_KEYSTORE === 'off'; }
...
export function 보관방식(값 = null, { lang = 'ko' } = {}) {
  ...
  if (값) {
     ...
     *
     * 다만 **해 본 것만** 그렇다 (해보고졌나 머리말). 잠금장치가 아예 없는
     * 판에서 적힌 까닭은 시도가 아니라 사실이고, 그 사실은 쓸수있나() 가 이미
     * 더 바른 말로 적고 있다. 그리고 **끈 것은 묵은 실패보다 세다** — 스스로
     * 껐으면 이 판에서는 아무것도 안 해 봤으니, 앞 판의 실패 기록이 남아 있어도
     * 이번 답은 「껐습니다」 다.
     */
    const 실패 = 해보고졌나 && !껐나() ? 잠그다실패한까닭() : null;
    if (실패) {
