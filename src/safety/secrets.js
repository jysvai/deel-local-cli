 ...
 *
 * `$` 는 글 끝에만 붙고 `.` 은 `\r` 을 안 먹는다. 그래서 줄을 떼어 낼 때
 * **줄 끝 `\r` 을 먼저 떼야** 이 무늬가 CRLF 글에서도 붙는다. 안 떼면 윈도우
 * 줄바꿈으로 적힌 파일에서 이 고침이 통째로 헛돈다(15차 리뷰).
 */
const 주석 = /(?:^|[ \t])#.*$/;
...
const 점값을가릴까 = (전체, 자리, 값) => {
  ...
  const 뒤 = 전체.indexOf('\n', 자리);
  const 줄 = 전체.slice(앞, 뒤 < 0 ? 전체.length : 뒤).replace(/\r$/, '').replace(주석, '');
  return !코드표시.test(줄);
};
