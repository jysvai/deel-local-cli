...
export const 갈래 = [
  ...
  },
  {
     ...
     * 대신 **어디까지가 타입인가**만 정한다 — 등호 앞까지고, 쉼표에서 끊긴다.
     *
     * 쉼표가 여기서 유일하게 중요한 글자다. 쉼표는 **홑화살괄호 안에서만**
     * 타입의 일부고(`Record<string, string>`), 밖에서는 칸을 가른다.
     * 앞 판은 쉼표를 타입 글자에 그냥 넣어 둬서 `Record<A, B<C, D>>` 의
     * 첫 쉼표에서 어긋났다.
     *
     * 쉼표로 가르고 나면 **첫 글자가 대문자냐 아니냐는 더 볼 것이 없다.**
     * 위에서 대문자를 요구한 까닭이 `{ API_KEY: value, B: y = 1 }` 의
     * `value` 를 안 삼키려던 것인데, 그건 쉼표가 이미 막는다. 그래서 소문자도
     * 받고, 그 덕에 구조 분해의 `{ API_KEY: key = "기본값" }` 도 같이 걸린다.
     *
     * ── 달아난 따옴표에서 끊겼다 ─────────────────────────────────────
     *
