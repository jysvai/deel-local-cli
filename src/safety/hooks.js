...
export function 훅펴기(raw, 출처) {
  ...
  if (Array.isArray(것)) {
    for (const x of 것) {
      ...
      // 바로 위에서 `때 · 자리 · event` 셋을 다 보고 골랐는데, 무엇이 틀렸는지
      // 적을 때는 `자리` 만 빠져 있었다. `{자리: '오타'}` 로 적어 둔 사람은
      // 「모르는 자리: (없음)」 을 받는다 — 제가 적은 글자가 화면에 안 나온다.
      if (!자리) { 버린것.push(`모르는 자리: ${x?.때 ?? x?.자리 ?? x?.event ?? '(없음)'}`); continue; }
