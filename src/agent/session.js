...
export class Session {
   ...
   * ── 급은 **턴마다 한 번**만 매긴다 ──────────────────────────────────────
   *
   * 이 값은 시스템 글의 **굳은 부분**에 들어간다(아래 시스템조각 의 급말).
   * 그런데 매김() 은 살아 있는 사고 카운터를 본다 — 인자 잘림 한 번, 편집
   * 실패 한 번이면 **걸음 사이에** '보통' → '작음' 으로 뒤집힌다.
   *
   * 뒤집히는 자리가 프롬프트 맨 앞이라 그 뒤 전부가 캐시에서 빠진다 — 도구
   * 정의 + 시스템 글 + 대화 전체. 그것도 턴 사이가 아니라 **한 턴의 걸음
   * 사이**에서다. 대화가 꼬리만 자라서 앞머리가 가장 안정적이어야 할 바로
   * 그 자리다. 게이트웨이면 캐시 읽기(0.1배)로 나갔을 것이 정가로 다시 나가고,
   * 로컬이면 그 한 걸음에 프리필을 통째로 다시 돈다. 화면에는 왜 갑자기
   * 느려졌는지 한 줄도 안 뜬다.
   *
   * 그래서 빗장을 건다. 새로 배운 것을 버리는 것이 아니라 **적용을 다음 턴으로
   * 미루는 것**이다 — 턴시작() 이 빗장을 푼다. 사람이 /grade 로 정하거나
   * /model 로 갈아타면 급다시재기() 로 그 자리에서 다시 맨다. 그때는 사람이
   * 프롬프트가 바뀐 것을 알고 한 일이다.
   */
  #급빗장 = null;

  /** 지금 매겨진 모델 급. 화면과 프롬프트가 같은 것을 봐야 한다. */
  급() {
    if (!this.#급빗장) this.#급빗장 = 매김(this.conn, this.본것, this.급정한것);
    return this.#급빗장;
  }

  /**
   * 빗장을 풀어 다음에 물을 때 다시 매기게 한다.
   *
   * 부르는 자리는 셋이다 — 턴 시작(아래 턴시작), `/grade`, `/model`
   * (commands.js 의 연결적용). 앞의 둘은 사람이 그러라고 한 것이고, 마지막은
   * 모델 자체가 바뀐 것이라 앞의 매김이 뜻을 잃는다.
   */
  급다시재기() { this.#급빗장 = null; }
