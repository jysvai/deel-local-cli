...
/*
 * 「이걸 사실로 말해도 되나」 는 **여기서 안 정한다** (agent/신뢰도.js).
 *
 * 이 파일은 무엇을 겪었나를 센다. 그 셈을 얼마나 믿을지는 다른 일이고,
 * 섞어 두면 셈을 고칠 때마다 믿음의 문턱이 같이 흔들린다 — 그리고 그건
 * 화면에 안 나타난다.
 */
import { 삭힘, 며칠됐나, 뭐라할까, 하루할인된다 } from './신뢰도.js';
// 파일에서 읽은 것이 **우리가 적은 모양이 맞나** (agent/성한기록.js).
// 통계와 문지기는 다른 일이라 따로 둔다.
import { 거른표 } from './성한기록.js';
...
/*
 * 파일에서 읽은 것을 **그대로 믿지 않는다** (agent/성한기록.js 의 거른표).
 *
...
function 읽기(파일) {
  try {
    ...
    const 것 = { ...빈것(), ...(j && typeof j === 'object' ? j : {}) };
    것.명령 = 거른표(것.명령);
    return 것;
  } catch { return 빈것(); }
}
...
export class 배움 {
  ...
  명령본것(명령, 됐나, 이유 = '') {
    ...
    /*
     * 지난 셈을 **오늘 기준으로 삭혀서** 이어 센다 (agent/신뢰도.js 의 삭힘).
     *
  ...
  요약(모델 = '', 상한 = 최대토큰) {
     ...
     *
     * 문턱은 **여기서 안 정한다** (agent/신뢰도.js 의 뭐라할까). 여태 이 자리에
  ...
  현황(모델 = '') {
     ...
     * 것은 프롬프트에 안 실린다(agent/신뢰도.js). 화면과 실제가 갈리면
