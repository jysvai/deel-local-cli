 ...
 *
 * 눈금 한 칸을 내리면 생각 예산은 대략 네 할이 준다(agent/effort.js 의 배분).
 * 그렇게 되찾은 자리가 **1,024토큰은 돼야** 답이 한 문단이라도 더 나온다.
 * 그 아래에서 다시 부르면 같은 자리에서 또 잘리는데, 그 한 번이 프리픽스
 * 전체와 출력 상한 전체를 다시 낸다.
 *
 * 상한에 견준 비율(예전의 15%)로 안 재는 까닭: 같은 15% 가 16k 상한에서는
 * 2,400토큰이고 2k 상한에서는 300토큰이다. 같은 잣대인 척하면서 실제로는
 * 상한이 클수록 헐거워진다.
 */
export const 되찾을몫 = 0.4;
export const 쓸모있는자리 = 1024;
...
export async function* run(session, ctx, userText, { signal = null, 깊이 = 0, 그림들 = null, 끼어들기 = null, 고쳐쓰기 = false } = {}) {
  ...
  const 부른것 = new Map();
  /*
   * 그 부름을 마지막으로 봤을 때, 이 턴이 파일을 몇 개나 손댔었나.
   *
   * 되풀이를 셀 때 이 값과 견준다. 그 사이에 파일이 늘었으면 일이 되고
  ...
  while (steps < maxSteps) {
    ...
    try {
      ...
      /*
       * ── 게이트웨이가 이번 요청을 **다른 자리로** 보냈나 ─────────────────
       *
       * 같은 모델이 여러 자리에 올라가 있으면 프리픽스 캐시는 **자리마다**
       * 따로다. 다음 요청이 다른 자리로 가는 순간, 우리가 보낸 것이 한 글자도
       * 안 달라져도 캐시는 0% 가 되고 그 턴은 전액 다시 나간다.
       *
       * 여태 이건 화면에 안 나왔다. 사람이 볼 수 있는 것은 며칠 뒤 청구서의
       * 튄 줄 하나뿐이고, 그 줄만 보면 원인이 우리 쪽인지 게이트웨이 쪽인지
       * 가를 방법이 없다. 서버가 말해 줄 때는 그 자리에서 말한다.
       *
       * 서버가 안 알려 주면 **아무 말도 안 한다.** 모르는 것을 「안 바뀌었다」
       * 로 적지 않는다 (backend/adapter.js 의 간자리).
       */
      if (msg?.간자리) {
        if (session.간자리 && session.간자리 !== msg.간자리) {
          /*
           * **한 대화에 한 번만** 말한다.
           *
           * 세션 고정 없이 돌아가며 나눠 주는 게이트웨이에서는 이게 걸음마다
           * 바뀐다 — 그리고 그건 사실이다. 다만 사실이라고 걸음마다 찍으면
           * 그 줄은 곧 아무도 안 읽고, 그러면 정작 한 번만 튀는 자리도 같이
           * 안 읽힌다. 한 번 말하면 사람이 알아야 할 것은 다 말한 셈이다.
           */
          if (!session.자리바뀐것을말했나) {
            session.자리바뀐것을말했나 = true;
            yield { type: '자리바뀜', 몇번째: steps };
          }
        }
        session.간자리 = msg.간자리;
      }
       ...
       */
      const 낮춘생각 = level === 'off' ? level : shiftLevel(level, -1);
      let 마지막상한 = cap;
      /*
       * ── 생각을 줄이는 것이 **자리를 실제로 내는가** ──────────────────────
       *
       * 위 글이 「생각을 줄이면 예산이 실제로 남는다」 고 적어 뒀다. 맞는
       * 말인데, **생각을 안 한 턴에는 안 맞는다.** 상한을 못 올리는 자리에서
       * 이 조건은 `낮춘생각 !== level` 하나로 걸리므로, 생각이 0토큰이었어도
       * 눈금이 medium 이기만 하면 다시 부른다. 그러면 같은 상한으로 같은 답을
       * 다시 받고, 같은 자리에서 또 잘린다 — 바로 위 글이 「그건 그냥 낭비다」
       * 라고 적어 둔 그 일이다.
       *
       * 실제로 그 값을 봤다. 16,384 에서 잘린 답을 같은 16,384 로 다시 받아
       * 또 잘렸고, 그 한 번이 프리픽스 122K 와 출력 16K 를 통째로 다시 냈다.
       *
       * 그래서 **잰다.** 생각이 이번 답에서 자리를 얼마나 먹었는지 보고,
       * 눈금 한 칸으로 되찾을 만한 몫일 때만 다시 부른다. 서버가 추론 토큰을
       * 세어 주면 그 값을 쓰고, 안 세어 주면 흘러온 생각 글로 어림한다.
       * 둘 다 없으면 생각은 자리를 안 먹은 것이다 — 없는 것을 있다고 치고
       * 한 번 더 부르지 않는다.
       */
      /*
       * 생각이 얼마나 먹었나를 **세 갈래로** 잰다. 하나만 보면 못 잰다.
       *
       *   1. 서버가 세어 준 추론 토큰      — 있으면 제일 정확하다
       *   2. 흘러온 생각 글               — 세어 주지 않는 창구에서 쓴다
       *   3. **낸 토큰에서 보이는 글을 뺀 것** — 위 둘이 다 없을 때
       *
       * 3번이 중요하다. 게이트웨이가 생각 블록을 지워서 넘기면 1도 2도 0이
       * 되는데, 그때 「생각을 안 했다」 고 읽으면 정말로 생각에 예산을 다 쓴
       * 턴에서 다시 부르기를 그만둔다. 관측이 없는 것과 사건이 없는 것은
       ...
       *
       * 상한에 견준 비율로 재면 안 된다 — 같은 15% 가 16k 상한에서는 2,400
       * 토큰이고 2k 상한에서는 300토큰이라, 같은 잣대인 척하면서 다르게 잰다.
       * 되찾은 자리가 한 문단이라도 더 나올 만큼인지가 재야 할 것이고,
       * 그건 절대량이다.
       */
      ...
      /*
       * 탈이 나도 **여기까지 흘러온 말은 남긴다.**
       *
       * 중단(위)에서는 이미 남기고 있었는데 오류에서는 안 남기고 있었다.
       * 그런데 사람이 겪는 것은 오히려 이쪽이 잦다 — 200자를 받아 화면에
       * 읽고 있는데 연결이 끊기면, 화면에는 그 200자가 그대로 보이는데
       * 대화에는 한 글자도 안 들어간다. 그 상태에서 "이어서 해줘" 라고
       * 하면 모델은 방금 제가 한 말을 모른다. 화면과 대화가 어긋나 있으면
       * 사람은 그 뒤로 무엇을 믿어야 할지 알 수 없다.
       *
       * 반쪽인 것은 중단과 같은 표를 붙여 남긴다 — 온전한 답으로 읽히면
    ...
    for (const 덩어리 of 묶기(msg.toolCalls)) {
      ...
      for (const { call, result, ms } of 결과들) {
         ...
         * 옮긴 것도 손댄 것이다.
         *
         * Move 는 파일을 고치는 게 아니라 자리를 바꾸므로 changed·여럿 이
         * 아니라 바뀐것들 로 온다. 이걸 안 세면 스물두 개를 옮겨 놓고도
         * 이 턴은 "아무것도 안 했다" 고 알고 있게 된다.
         */
         ...
         * ── 읽은 파일은 **바뀐 만큼만** 싣는다 ────────────────────────────
         *
         * 아래 되풀이 막기는 「인자도 결과도 똑같을 때」만 걸리고, 그 표는
         * 이 턴 하나짜리다. 그래서 제일 흔한 모양이 그물을 빠져나갔다 —
         * 읽고, 한 줄 고치고, 확인하러 다시 읽는 것. 결과가 달라졌으니
         * 1,200줄이 통째로 한 벌 더 실렸다. 새로운 것은 한 줄뿐인데.
         *
         * 여기서는 앞서 실은 글과 견줘서, 그대로면 그렇다고만 적고 달라졌으면
         * 바뀐 자리만 적는다. 창이 밀리지 않으니 앞선 대화가 그만큼 오래 산다.
         */
        /*
         * ── 할 일 목록은 세션에도 들고 있는다 ────────────────────────────
         *
         * 여태 목록이 사는 자리는 도구 결과 하나뿐이었다. 그런데 도구 결과는
         * 55%에서 한 줄로 접히고 80%에서 요약에 뭉개진다. 접힘 문구는
         * 「필요하면 다시 읽으세요」인데 **할 일 목록은 다시 읽을 파일이 없다.**
         * 그래서 긴 작업일수록 남은 항목이 통째로 사라졌다 — 「대화를 잘
         * 기억 못 한다」로 제보받은 것의 큰 몫이 여기다.
         *
         * ctx.todos 는 우리 코드만 본다. 모델이 보는 자리에 남기려면 접거나
         * 줄일 때 다시 실어 줘야 하고, 그러려면 session 이 들고 있어야 한다.
         */
        if (!result.error && Array.isArray(result.todos)) session.할일 = result.todos;

        if (!result.error && result.읽은것 && session.파일기억) {
          const 정한것 = session.파일기억.실을것(result.읽은것, 실을것, { 부분인가: !!result.부분인가 });
          if (정한것.어떻게 !== 'full') {
            session.본것?.본것('아낀읽기');
            실을것 = 정한것.글;
          }
        ...
        if (!result.error) {
           ...
           *
           * 견주는 것은 **바꿔치기 전** 글이다. 위에서 파일 기억이 실을것 을
           * 「앞에서 읽은 그대로입니다」 로 갈아 끼우는데, 그것까지 표에 넣으면
           * 같은 파일을 두 번째 읽을 때가 첫 번째와 달라 보인다. 그러면 되풀이가
           * 한 바퀴 늦게 걸린다 — 헛도는 것을 한 걸음 더 돌고 나서야 잡는 셈이다.
           * 도구가 실제로 무엇을 돌려줬나로 세는 것이 맞다.
           */
          const 같은꼴 = 날것.normalize('NFC');
          const 앞것 = 부른것.get(서명);
          if (앞것 !== undefined && 앞것 === 같은꼴) {
            session.본것?.본것('되풀이');
            /*
             * ── 되풀이와 「일하면서 장부를 다시 쓴 것」 은 다르다 ──────────
             *
             * 「폴더 정리 해줘」 에서 이 자리가 사람의 턴을 죽였다. 화면은
             * 이랬다.
             *
             *   Move 7개 옮김 → TodoWrite → Move 22개 옮김 → TodoWrite
             *   → ⊘ 같은 자리에서 헛돌고 있어 멈췄습니다
             *
             * 파일은 스물아홉 개가 실제로 움직이고 있었다. 헛돈 것은 할 일
             * 목록을 같은 값으로 다시 쓴 것뿐이다. 그런데 세는 자리는 그
             * 사이에 무슨 일이 있었는지를 안 봤다 — 같은 부름이 세 번이면
             * 그것으로 끝이었다.
             *
             * 이 파일 머리말에 스스로 적어 둔 원칙이 있다: 헛도는 것은
             * '잘 되고 있는 긴 작업' 과 구분해야 한다. 그 구분을 여기에
             * 넣는다 — **그 사이에 파일이 진짜로 바뀌었으면 처음부터 다시
             * 센다.** 아무것도 안 바뀐 채로 되풀이하는 것만 헛도는 것이다.
             *
             * 이러면 끝없이 돌 수 있지 않나: 아니다. 손댄파일 은 서로 다른
             * 파일만 세므로, 같은 자리를 맴돌면 안 늘어난다. 늘어난다는 것은
             * 매번 새 파일이 움직였다는 뜻이고, 그건 일이 되고 있는 것이다.
             * 그래도 걸음 수 상한이 마지막 울타리로 남아 있다.
             */
            const 그새한일 = 손댄파일.size > (반복진척.get(서명) ?? 0);
            const n = 그새한일 ? 1 : (막힘.get(`반복|${서명}`) ?? 0) + 1;
            막힘.set(`반복|${서명}`, n);
