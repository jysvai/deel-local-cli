...
// 그래서 걸음 하한(최소걸음)과 비율 문턱을 둘 다 넘어야 움직인다. 많이 겪을수록
// 더 잘 맞추는 쪽이지, 많이 겪으면 멈추는 쪽이 아니다.
...
export function 카드(모델, 본것 = null, 겪은것 = null) {
  ...
  return {
    ...
    // '아직 모름' 은 다른 말이다.
    //
    // **이미 바꾼 것이 있으면 미루고 있는 것이 아니다.** 내장 카드(preset.js)는
    // 걸음 관문 앞에 서서 한 걸음도 안 걸은 모델의 하네스를 이미 바꾼다. 그런데
    // /card 화면은 아직모름 이면 왜 를 통째로 감추고 「아직 판단하지 않습니다」
    // 만 찍는다(commands/work.js) — 바꿔 놓고 안 바꿨다고 말하는 화면이 된다.
    아직모름: 걸음 < 최소걸음 && 왜.length === 0,
  };
}
