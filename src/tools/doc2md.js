 ...
 *
 * 백슬래시는 **끝엣것만** 짝을 맞춘다. 칸이 백슬래시로 끝나면 그것이 뒤따르는
 * 칸 가름표(`|`)를 먹어 버려 표가 그 줄부터 통째로 어긋난다 — 윈도우 경로가
 * 든 표에서 바로 나온다.
 *
 * 모든 백슬래시를 벗기려다 한 번 되돌렸다(2차 눈). 그러면 칸 안의
 * `` `\d+` `` 같은 글이 `` `\\d+` `` 로 **불어난다.** 가운데 백슬래시는
 * 표를 안 부수니 건드릴 까닭이 없다. 부수는 것은 끝엣것뿐이다.
 *
 * ── `|` 바로 앞의 백슬래시도 짝을 맞춘다 (20회차 CodeQL #140) ────────────
 *
 * `.replace(/\|/g, '\\|')` 하나만으로는 모자랐다. 칸에 `a\|b` 가 들어오면
 * 우리가 백슬래시를 하나 더해 `a\\|b` 가 되는데, 마크다운은 그것을
 * **「백슬래시 하나」 + 살아 있는 칸 가름표**로 읽는다. 막으려던 바로 그
 * 일이 우리 손으로 일어난다 — 가름표 앞에 백슬래시가 홀수 개면 언제나 그렇다.
 *
 * 그래서 `|` 앞에 붙은 백슬래시 묶음을 먼저 두 배로 늘리고 나서 `|` 를
 * 벗긴다. 그러면 가름표 앞 백슬래시 수가 늘 짝수라 벗김표가 살아남는다.
 * `|` 가 없는 자리의 백슬래시는 그대로 두므로 위의 `` `\d+` `` 도 안 는다.
 */
function 칸(v) {
  return String(v ?? '')
    .replace(/(\\*)\|/g, (m, 앞) => 앞 + 앞 + '\\|')
