 ...
 * ── 깨진 바이트를 만나면 (8회차 · 바깥) ────────────────────────────────
 *
 * 규약은 하나다 — **어디까지 읽었는지 말하고 멈춘다.**
 *
 * 한 파일 안에 이것과 반대인 두 버릇이 같이 있었다. 스키마 갈래 바이트가
 * 0·1·2 가 아니면 조용히 ENUM 으로 **바꿔서** 계속 읽었고(그 뒤는 전부 다른
 * 값이 되는데 아무 말도 안 나온다), 알맹이에서 모르는 칸 번호를 만나면
 * **이미 읽은 것까지** 말없이 버리고 던졌다. 지어내는 쪽과 버리는 쪽이
 * 한 파일에 같이 있으면 읽는 쪽은 무엇을 받았는지 알 길이 없다.
 *
 * 그래서 아래 여기까지() 하나로 묶는다. 멈추는 것은 같고, 오류에 **바이트
 * 자리**와 **여태 읽은 것**을 달아 보낸다. 그걸 쓸지는 부르는 쪽이 정한다 —
 * fig.js 는 안 쓴다(「반만 맞는 시안은 틀린 시안보다 나쁘다」). 버리는 것과
 * 버릴지를 고르게 하는 것은 다르다.
 */
...
/**
 * 깨진 바이트를 만났다 — **어디까지 읽었는지 말하고 멈춘다.**
 *
 * @param {string} 말      무엇이 깨졌나
 * @param {number} 자리    그때까지 읽은 바이트 수
 * @param {*} [읽은것]     여태 읽어 놓은 것. 말없이 버리지 않는다.
 */
function 여기까지(말, 자리, 읽은것 = null) {
  return Object.assign(new Error(`${말} — 바이트 ${자리} 까지 읽었습니다`), { 자리, 읽은것 });
}
...
export function 스키마읽기(buf) {
  const r = new 읽개(buf);
  // 개수는 스키마 쪽에서도 **남은 바이트와 대조하고** 들어간다. 여기만 날 varuint 라,
  // 「정의 40억 개」 라고 적힌 조각이 그 대조 없이 루프에 들어가 엉뚱한 말로 터졌다
  // (`문자열이 끝나지 않았습니다`). 정의 하나도 칸 하나도 최소 한 바이트는 먹는다.
  const 개수 = r.개수();
  ...
  for (let i = 0; i < 개수; i += 1) {
    const 이름 = r.문자열();
    /*
     * 모르는 갈래 바이트를 ENUM 으로 **바꾸지 않는다.**
     *
     * 갈래는 0=ENUM · 1=STRUCT · 2=MESSAGE 셋뿐이고, 셋이 서로 다르게 읽힌다.
     * 모르는 값을 ENUM 으로 치면 그 정의부터 뒤가 전부 다른 값으로 읽히는데
     * 오류는 안 난다 — 갈래 바이트 7 하나가 `{"갈래":"ENUM","칸들":[]}` 로
     * 조용히 둔갑했다. 지어내느니 여기까지 읽었다고 말하고 멈춘다.
     */
    const 갈래바이트 = r.바이트();
    const 갈래 = 갈래이름[갈래바이트];
    if (!갈래) throw 여기까지(`${이름} 의 정의 갈래가 ${갈래바이트} — 0·1·2 만 있습니다`, r.i, 정의들);
    const 칸수 = r.개수();
...
function 정의읽기(스키마, r, 번호, 깊이, 담나, 남길까) {
  ...
  const 한칸 = (f) => {
    ...
    if (f.배열) {
       ...
       * 한 글자도 안 달라진다.
       */
      // 개수 대조는 바로 위 `r.개수()` 가 이미 했다. 여기 한 번 더 있던 같은 대조는
      // **어떤 입력에도 안 걸리는** 죽은 단언이었다 — 지킨다고 적혀 있을 뿐 아무것도
      // 안 지키는 줄은, 다음 사람이 그 줄을 믿고 위쪽 대조를 빼게 만든다.
      if (!이칸담나 && f.형 === -2) {
        r.i += n;
        return;
      }
  ...
  //
  // 그 「말한다」 가 없었다. 그냥 던져서 **이미 읽은 것까지** 같이 사라졌고,
  // 부르는 쪽은 한 글자도 못 읽은 것과 구별할 수 없었다. 오류에 바이트 자리와
  // 여기까지 읽은 것을 달아 보낸다 — 쓸지는 부르는 쪽이 정한다.
  const 칸표 = new Map(d.칸들.map((f) => [f.값, f]));
  for (;;) {
    const id = r.varuint();
    if (id === 0) break;
    const f = 칸표.get(id);
    if (!f) throw 여기까지(`${d.이름} 에 모르는 칸 번호 ${id} — 여기서 더 못 읽습니다`, r.i, 것);
    한칸(f);
  }
  return 것;
}
