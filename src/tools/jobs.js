 ...
 *
 * ── 상한에 걸려도 taskkill 을 **죽이지 않는다** (2.0.0 6회차) ─────────────────
 *
 * 위처럼 execFileSync 의 timeout 으로 부르면, 상한에 걸리는 순간 Node 가 taskkill 을 죽인다.
 * 한가할 때는 그 순간 taskkill 이 아직 아무것도 안 죽인 채라 괜찮았다. 그런데 컴퓨터가
 * 바쁘면 taskkill 이 **나무를 반쯤 죽인 채** 죽는다 — Git Bash 나무(`bin/bash.exe →
 * usr/bin/bash.exe → node`)에서 뿌리와 가운데 셸만 죽고 손자가 부모 없이 남았다. 그 뒤로는
 * 손자를 가리킬 길이 없어(남은무리끊기 머리말) 두 번째 부름도 「프로세스 없음」 으로 끝났다.
 * 코어를 다 쓰는 판에서 끝내기 4번 중 1번 · 모두끝내기 4번 중 1번이 dev 서버를 남겼고,
 * 검사를 돌린 하루 동안 jobs 검사의 아이 74개가 그렇게 살아 있었다.
 *
 * 그래서 떼어서(detached) 띄우고 **상한까지만 동기로 지켜본다.** 그 안에 끝나면 다 훑은 것,
 * 못 끝나면 「잘렸다」 로 돌려주되 taskkill 은 제 할 일을 끝까지 한다 — deel 이 끝나는
 * 길이어도 떼어 띄운 것은 살아남는다. 돌려주는 뜻(잘렸으면 뿌리를 살려 두고 다시 온다)은 같다.
 * 잘렸나() 는 execFileSync 오류를 가르는 판별식으로 그대로 둔다 (검사가 값을 못박아 둔다).
 */
export function 나무끊기(pid, 상한 = 1500) {
  if (process.platform !== 'win32' || !pid) return true;
  let 끊개;
  try {
    끊개 = spawn('taskkill', ['/pid', String(pid), '/t', '/f'], { windowsHide: true, stdio: 'ignore', detached: true });
  } catch {
    return true;   // taskkill 을 못 띄운 판 — 예전에도 「잘림」 이 아닌 실패는 다 참이었다
  }
  끊개.once('error', () => { /* 못 띄웠다 — 위와 같다 */ });
  끊개.unref();
  if (!끊개.pid) return true;
  const 끝 = Date.now() + 상한;
  for (;;) {
    // 신호 0 은 「살아 있나」 만 묻는다. 끝났으면 던진다 — 없는 pid 로 끝난 것(이미 죽음)도 여기로 온다.
    try { process.kill(끊개.pid, 0); } catch { return true; }
 ...
 *
 * 무리로 못 물어보면 **그 하나에게** 물어본다 — 아래 신호주기() 와 같은 까닭이다.
 * 무리가 없는 판에서 「무리가 없다 = 다 죽었다」 로 읽으면, 살아 있는 자식을
 * 죽은 것으로 세고 그냥 지나간다.
 */
export function 무리살아있나(pid) {
  /*
   * EPERM 은 「없다」 가 아니라 **「있는데 못 건드린다」** 이다.
   *
   * 여기가 아무 탈이나 다 「죽었다」 로 읽어서, 무리끊기 의 지켜보기 고리가
   * 첫 바퀴에 빠져나가고 마지막 SIGKILL 까지 통째로 건너뛰었다 — 그 자리
   * 머리말이 「권한이 없으면 살아 있는데도 「없다」 가 나오고, 그러면 안 죽이고
   * 지나간다」 라며 걷어냈다고 적은 바로 그 길이다. 고침이 반만 붙어 있었다.
   */
  try { process.kill(-pid, 0); return true; } catch (탈) { if (탈?.code === 'EPERM') return true; }
  try { process.kill(pid, 0); return true; } catch (탈) { return 탈?.code === 'EPERM'; }
}
 ...
 * 임시 파일이나 소켓을 치울 틈이 없다. 얼마나 기다릴지(`늦게`)는 부르는 쪽이
 * 정한다 — 끝나는 길에서는 짧게, 사람이 「끝내」 라고 시킨 자리에서는 길게.
 *
...
export function 무리끊기(pid, { 곧장 = false, 늦게 = 800 } = {}) {
   ...
   */
  const 조각 = 20;
  for (let 쓴시간 = 0; 쓴시간 < Math.min(늦게, 못재고기다릴최대); 쓴시간 += 조각) {
...
/*
 * ── 그 지켜보기는 유닉스에서 **한 번도 안 끝났다** (2.0.0 16회차) ──────────
 *
 * 위 고리의 머리말은 「얌전히 끝나는 무리는 첫 20ms 에 사라지므로 보통은 거의
 * 안 기다린다」 고 적어 뒀다. 윈도우에서만 참인 말이었다.
 *
 * 우리가 띄운 자식은 **죽어도 우리가 거두기(reap) 전까지 좀비로 남는다.**
 * 거두는 일은 이벤트 루프에서만 일어나는데, 위 고리는 `Atomics.wait` 로 그
 * 루프를 세워 놓고 묻는다. `무리살아있나` 는 좀비를 「산다」 로 답하므로
 * (`kill(pid, 0)` 은 좀비에게도 성공한다) 답은 영영 안 바뀌고, 고리는 **늘
 * `늦게` 를 다 쓴다.**
 *
 * 값이 얼마였나. 사람이 「끝내」 를 한 번 칠 때마다 6초였다 — 리눅스 CI 에서
 * test/jobs.test.js 의 「오래된 것에는 안 머문다」(400ms 아래여야 한다)가
 * **6037ms** 로 빨개진 것이 이것이고, 화면에는 아무 말도 안 난다.
 *
 * 그래서 둘로 가른다.
 *
 *   · 기다릴 수 있는 자리(끝내기 — async) → 무리끊기기다려(). 진짜로 쉬어
 *     주니 루프가 돌아 자식이 거둬지고, 같은 물음이 그때부터 바르게 답한다.
 *   · 못 기다리는 자리(프로그램이 끝나는 길) → 위 동기 고리. 다만 **못 재는
 *     기다림은 최선이 아니라 모두가 내는 값**이라 짧게 못 박는다. SIGKILL 은
 *     그대로 나가므로, 신호를 무시하는 손자를 살려 보내는 일은 없다.
 */
// 검사도 이 값을 쓴다 (test/jobs.test.js 의 「상한이 유닉스 갈래에서도
// 지켜진다」). 거기서 400 을 손으로 적어 두면, 이 값을 고친 날 그 검사는
// **조용히 아무것도 안 가르는 검사**가 된다.
...
async function 무리끊기기다려(pid, { 곧장 = false, 늦게 = 800 } = {}) {
  ...
  for (let 쓴시간 = 0; 쓴시간 < 늦게; 쓴시간 += 조각) {
    if (!무리살아있나(pid)) return;
    await new Promise((r) => setTimeout(r, 조각));
  }
  신호주기(pid, 'SIGKILL');   // 위와 같은 까닭으로 묻지 않고 보낸다
}
 ...
 */
export function 나무죽이기(kid, { 파이프끊기: 끊을까 = true, 곧장 = false, 상한 = 1500, 뿌리살리기 = true } = {}) {
  if (!kid) return true;
  let 나무끊었나 = true;
  /*
   * ── 끝난 것으로 본 아이에게는 taskkill 을 안 보낸다 (2.0.2 · 28회차 flake) ──────────
   *
   * Node 는 아이가 끝났다고 알리는 그 자리에서 프로세스 손잡이를 닫는다. 손잡이가 닫히면 윈도는
   * 그 번호를 **곧바로 딴 프로세스에게 준다.** 그 뒤의 `taskkill /pid N /t` 는 우리 아이가 아니라
   * 그 번호를 새로 받은 **남의 나무**를 끊는다 — 윈도 CI 에서 옆 대조군이 말없이 멈춘 것(28회차)이
   * 이 모양이다. 상한에 잘려 다시 오는 길(뿌리살리기)에서 특히 그렇다: 첫 taskkill 이 뿌리를
   * 죽이고 나서 두 번째가 같은 번호로 간다.
   *
   * 잃는 것은 없다. 뿌리가 끝난 뒤에는 /t 가 어차피 그 아래를 못 훑는다(남은무리끊기 머리말).
   * 아직 안 끝난 아이는 손잡이가 열려 있어 번호가 안 바뀐다 — 그때만 보낸다.
   */
  const 끝난아이 = typeof kid.exitCode === 'number' || typeof kid.signalCode === 'string';
  if (process.platform === 'win32' && kid.pid) {
    ...
    /*
     * **상한은 여기도 넘긴다.** 윈도우 갈래(바로 위)는 부르는 쪽이 정한 상한을
     * 그대로 쓰는데, 유닉스 갈래만 그 값을 버리고 무리끊기의 기본 800ms 를 썼다.
     * 재 보니 상한 5000 을 줘도 100 을 줘도 걸린 시간이 똑같았다 — 위 머리말이
     * 「상한은 부르는 쪽이 정한다」 고 적어 놓고 한쪽에서만 지키고 있었다.
     *
     * 값이 다르다. 끝나는 길(process 'exit')에서는 일감마다 몇 초씩 붙들면 deel 이
     * 멈춘 것처럼 보이고, 사람이 「끝내」 라고 시킨 자리에서는 안 죽는 서버 하나가
     * 훨씬 비싸다. 그 판단을 부르는 쪽이 이미 하고 있었다.
     */
    무리끊기(kid.pid, { 곧장, 늦게: 상한 });
  }
...
/**
 * 나무죽이기 의 **기다릴 수 있는** 판 (무리끊기기다려 머리말).
 *
 * 윈도우 갈래는 taskkill 을 이미 동기로 기다리므로 그대로 넘긴다. 유닉스에서만
 * 지켜보기를 진짜 쉬면서 한다 — 그래야 죽은 자식이 거둬지고 「아직 사나」 가
 * 바르게 답한다.
 */
export async function 나무죽이기기다려(kid, 몫 = {}) {
  const { 파이프끊기: 끊을까 = true, 곧장 = false, 상한 = 1500 } = 몫;
  if (!kid || process.platform === 'win32' || !kid.pid) return 나무죽이기(kid, 몫);
  await 무리끊기기다려(kid.pid, { 곧장, 늦게: 상한 });
  try { kid.kill(); } catch { /* 이미 죽음 */ }
  if (끊을까) 파이프끊기(kid);
  try { kid.unref(); } catch { /* 없으면 그만 */ }
  return true;
...
/*
 * ── 갓 띄운 셸은 나무를 **나중에** 세운다 (2.0.0 6회차) ─────────────────────────
 *
 * 윈도우의 Git Bash 는 `bin/bash.exe`(발사대) → `usr/bin/bash.exe` → 명령 으로 한 겹씩 띄운다.
 * 띄우고 몇 ms 안에 taskkill /t 를 보내면 그 순간 있는 나무(발사대뿐)만 죽고, 곧이어 뜬 손자가
 * 부모 없이 남는다. 뿌리가 죽은 뒤로는 그 손자를 가리킬 길이 없다(남은무리끊기 머리말).
 * 재 보니 띄우고 0ms 에 끊으면 4번 중 1~2번, 30ms 에 1번 남았고 150ms 부터 0 이었다(옛 코드도 같음).
 * 바쁜 컴퓨터에서는 그 창이 넓어져 150~600ms 에 끊은 판도 남았다. 검사 판마다 이 파일의 아이가
 * 남던 것이 이것이다.
 *
 * 그래서 끄는 쪽에서 갓 띄운 것에 1초를 채워 준다. 모델이 띄운 것은 띄우기 가 이미 1.5초를
 * 기다렸고 왕복이 수 초라 거의 안 걸린다 — 곧장 끄는 판(사람이 띄우자마자 나감 · 검사)만 기다린다.
 */
const 나무설틈 = 1000;
...
/**
 * 뿌리(셸)는 이미 끝났는데 그 아래 남은 것이 파이프를 물고 있을 때, 남은 것을 끊는다.
 * Bash(tools/index.js)와 끝내기 가 같이 쓴다 (사냥5 M2).
 *
 * 유닉스는 띄울 때 만든 무리가 우두머리가 죽은 뒤에도 남아 있어 `-pid` 로 무리째 닿는다.
 *
 * 윈도우는 **못 한다.** taskkill /t 는 살아 있는 뿌리부터 나무를 훑는데, 뿌리가 끝나면
 * 가리킬 자리가 없다. 부모 번호(ParentProcessId)로 거꾸로 찾아도 소용없었다 — Git Bash
 * 는 `bin/bash.exe → usr/bin/bash.exe → 손자` 로 한 겹이 더 있고, 가운데 것이 이미 끝나
 * 사슬이 끊겨 있다(재 봄). 그래서 false 를 돌려주고, 부르는 쪽이 「못 껐다」 고 말한다.
 *
 * @returns {boolean} 끊으러 갈 길이 있었나
 */
export function 남은무리끊기(kid) {
  if (process.platform === 'win32' || !kid?.pid) return false;
  무리끊기(kid.pid);
  return true;
}
...
class 일감 {
  ...
  담기(buf) {
    ...
    /*
     * 한 조각이 **혼자** 상한을 넘으면 그 조각의 뒤만 남긴다.
     *
     * 위 되풀이는 조각이 둘 이상일 때만 버린다. 그래서 한 조각이 혼자 넘치면
     * **아무것도 못 버리는데 아래에서 잘렸다고 적는다.** 그 사이에 읽으면
     * 멀쩡한 글에 "앞이 잘렸습니다" 가 붙고 읽은 자리도 0 으로 돌아간다 —
     * 모델은 없는 앞을 찾으러 처음부터 다시 읽는다. (다음 조각이 들어오면
     * 그때는 둘이 되어 버려지므로, 거짓말은 그 한 판만이다. 그래도 그 한
     * 판이 되풀이를 부르는 자리다 — 읽기() 머리말 참고.)
     *
     * 그리고 버릴 수 있게 만들어 두는 것이 맞다. 상한 위에 남겨 두면 그
     * 다음 조각 하나에 256KB 가 통째로 날아간다.
     *
     * 베껴 담는다. subarray 는 원래 덩이를 그대로 붙잡고 있어서, 큰 조각
     * 하나가 들어온 판에서는 놓으려는 그 메모리가 안 놓인다.
     */
    if (this.바이트 > 출력상한 && this.조각들.length === 1) {
      const 하나 = this.조각들[0];
      this.조각들[0] = Buffer.from(하나.subarray(하나.length - 출력상한));
      this.바이트 = this.조각들[0].length;
    }
    /*
     * 여기 오면 **반드시 무엇인가 버렸다** — 그래서 「안 버렸으면」 갈래를
     * 두지 않는다. 위에서 상한을 넘은 것을 확인했고, 조각이 둘 이상이면
     * 되풀이가 버리고 하나뿐이면 그 하나를 줄였다. 늘 참인 조건을 남기면
     * 어긋내기가 「그 줄을 지워도 아무 검사도 안 깨진다」 고 말하는데, 그건
     * 맞는 말이라 고칠 수도 없다 — 대신 읽는 사람은 안 버리는 길이 따로
     * 있는 줄로 읽는다.
     */
    this.앞잘림 = true;
    this.막버렸다 = true;
  ...
  전체글() {
    ...
    try {
      ...
      /*
       * ── ASCII 뿐인 동안은 못 박지 않는다 (사냥5 L1) ───────────────────────
       *
       * 「글이 나온 뒤」 를 글자가 있느냐로만 봤다. 그래서 dev 서버가 흔히 그러듯 영어 배너가
       * 먼저 나오면 그 순간 utf-8 로 못 박혔고, 나중에 한글 윈도우 콘솔이 CP949 로 한글을
       * 뱉자 **처음부터 다시 읽어도** 영영 깨졌다 — 통째로 새로 풀면 멀쩡한 바이트인데.
       *
       * ASCII(0x7F 이하)만 있으면 어느 인코딩으로 풀어도 같은 글자·같은 글자 수라, 안 못
       * 박아도 읽은 자리(읽은글자)가 어긋나지 않는다. 판정에 쓸 바이트가 처음 나왔을 때 박는다.
       */
      if (r.text && 다.some((b) => b > 127)) this.인코딩 = r.encoding;
...
export async function 띄우기(명령, { cwd, 설명 = null, 기다림 = 1500, 남길것 = [] } = {}) {
  ...
  kid.on('error', (err) => {
    ...
    j.상태 = '끝남';
    j.종료코드 = -1;
    // 왜 표를 세우나 — 바로 아래 close 머리말.
    j.띄우기실패 = true;
  });
  kid.on('close', (code, sig) => {
    // 파이프까지 다 닫혔다 — 셸만이 아니라 출력을 문 것이 하나도 안 남았다는 뜻이다.
    // 끝내기 가 「진짜 끝났나」 를 이것으로 본다 (사냥5 M2 · 끝내기 머리말).
    j.닫힘 = true;
    if (j.상태 === '죽임') return;
    j.상태 = '끝남';
    /*
     * 띄우기 자체가 실패한 판은 **덮지 않는다.**
     *
     * 명령이 없으면(ENOENT) Node 는 error 를 낸 다음 close 를 code=null 로
     * 부른다. 그러면 위에서 세워 둔 -1 이 null 로 덮이고, null 은 이 자리에서
     * 「시그널로 죽었다」 와 구별이 안 된다. 안 뜬 까닭이 종료코드에서 지워진다.
     */
    if (j.띄우기실패) { 끝난것치우기(); return; }
...
export function 목록() {
  return [...일감들.values()].map((j) => ({
    ...
    // 셸은 끝났는데 파이프가 안 닫혔다 — 셸이 뒤로 띄운 것이 아직 돈다 (사냥5 M2).
    셸끝남: j.상태 === '도는중' && !!j.kid && !j.닫힘 && !아직사나(j.kid),
  }));
}
...
export async function 끝내기(번호) {
  const j = 하나(번호);
  if (!j) return null;
  // 갓 띄운 것이면 셸이 나무를 다 세울 틈을 준다 (아래 나무설틈). 그 사이 끝났으면 바로 아래 「이미 끝난 것」 으로 간다.
  const 갓 = 나무설틈 - (Date.now() - j.띄운때);
  if (j.상태 === '도는중' && 갓 > 0) await new Promise((r) => setTimeout(r, 갓));
  /*
   * 이미 끝난 것도 **안 읽은 출력은 건네고** 지운다 (사냥5 L2).
   *
   * 여기는 곧장 지웠다. 뜨고 조금 돌다 죽은 일감에 stop 을 주면, 죽기 직전 남긴 말 —
   * 이 머리말이 「제일 중요한 몇 줄」 이라 부르는 그것 — 을 한 번도 안 보여 주고 버렸다.
   * 지우고 나면 다시 읽을 길도 없다.
   */
  if (j.상태 !== '도는중') {
    const 남은 = j.읽기({});
    일감들.delete(j.번호);
    return { 이미: true, 명령: j.명령, 남은: 남은.글, 종료코드: j.종료코드, 시그널: j.시그널 };
  }
  j.상태 = '죽임';
  /*
   * 사람이 시킨 자리라 넉넉히 기다린다.
   ...
   */
  const 넉넉히 = 6000;
  // 여기는 async 라 **진짜로 기다릴 수 있다** (무리끊기기다려 머리말). 동기로
  // 기다리면 유닉스에서는 죽은 자식이 거둬지지 않아 이 6초를 매번 다 쓴다.
  await 나무죽이기기다려(j.kid, { 파이프끊기: false, 상한: 넉넉히 });
  if (!j.닫힘) await 죽는말기다리기(j.kid, 죽는말기다림);
  /*
   * 아직 살아 있으면 여기서 끊는다.
   *
   ...
   */
  /*
   * ── 셸은 죽었는데 **파이프가 안 닫힌** 판도 안 끝난 것이다 (사냥5 M2) ──────────
   *
   * 여기는 `아직사나(j.kid)` — 셸의 exitCode — 만 봤다. `node gc.cjs & echo done` 처럼
   * 셸이 뒤로 띄운 손자가 출력을 물고 있으면, 셸은 이미 끝나 있어 taskkill /t 가 나무를
   * 못 찾고(「그런 프로세스 없음」 = 이미 죽음으로 친다), 아래 죽었나 도 참이 되어
   * 「끝냈습니다」 가 나갔다. 손자는 그대로 돌았다.
   *
   * 그래서 「끝났나」 를 'close'(닫힘)로 본다. 셸이 살아 있으면 예전처럼 곧장 끊고, 셸이
   * 이미 없으면 남은 것을 끊으러 간다 — 유닉스는 무리가 우두머리 없이도 남아 있어 무리째
   * 끊을 수 있다. 윈도우는 뿌리가 사라지면 그 아래를 가리킬 길이 없다(남은무리끊기 머리말).
   */
  if (!j.닫힘) {
    if (아직사나(j.kid)) 나무죽이기(j.kid, { 파이프끊기: false, 곧장: true, 상한: 넉넉히, 뿌리살리기: false });
    else 남은무리끊기(j.kid);
    /*
     * 죽인 **직후에 살았나 물으면 안 된다.**
     *
     ...
     */
    if (!j.닫힘) await 죽는말기다리기(j.kid, 죽는말기다림);
    // 셸은 끝났는데 아직 안 닫혔으면 바쁜 컴퓨터가 파이프를 비울 틈을 한 번 더 준다.
    // 1.5초로는 모자랐다 — 검사 여덟 개를 한꺼번에 돌린 판에서 멀쩡히 죽은 자식을 「안 끝났습니다」
    // 로 적었다. 이 기다림은 진짜로 손자가 파이프를 문 판에서만 끝까지 쓰인다.
    if (!j.닫힘 && !아직사나(j.kid)) await 죽는말기다리기(j.kid, 3000);
  }
  /*
   * ── 안 죽은 놈의 파이프는 **끊지 않는다** ──────────────────────────────
   *
   * 여기서 늘 끊고 있었다. 그런데 바로 아래에서 「그러고도 안 죽었으면 목록에
   * 남긴다 — 다시 시킬 수 있어야 한다」 고 한다. 파이프를 이미 끊어 놓고
   * 남기면, 목록에는 `도는중` 으로 뜨는데 그놈이 뱉는 말은 다시는 안 들어온다.
   * 사람은 화면을 보며 기다리고, 자식은 다음 쓰기에서 EPIPE 를 맞는다.
   * 남겨 두기로 한 까닭이 「다시 볼 수 있게」 인데 정작 볼 길을 끊던 셈이다.
   */
  // 셸의 종료만이 아니라 파이프가 닫혀야 끝난 것이다 (위 「파이프가 안 닫힌 판」).
  // 흉내 자식(검사)처럼 close 를 못 내는 것도 있어, 셸이 살아 있으면 여전히 안 끝난 것으로 본다.
  const 죽었나 = j.닫힘 === true || (!아직사나(j.kid) && !j.kid?.stdout?.readable && !j.kid?.stderr?.readable);
  if (죽었나) 파이프끊기(j.kid);
  const 남은 = j.읽기({});
  /*
  ...
  if (!죽었나) {
    ...
    // 셸은 끝났는데 남은 것이 문 판이면 그렇다고 따로 말한다 — 할 수 있는 일이 다르다.
    return { 이미: false, 안죽음: true, 뒤에남음: !아직사나(j.kid), 명령: j.명령, 초: j.산햇수(), 남은: 남은.글 };
  }
  일감들.delete(j.번호);
...
export function 모두끝내기() {
  ...
  // 갓 띄운 것이 있으면 제일 어린 것이 나무설틈 만큼 살 때까지 한 번만 기다린다 — 끝나는 길이라 동기로.
  const 제일갓 = Math.max(0, ...[...일감들.values()].filter((j) => j.상태 === '도는중').map((j) => 나무설틈 - (Date.now() - j.띄운때)));
  if (제일갓 > 0) 동기로쉬기(제일갓);
 ...
 * 작은 로컬 모델은 `"stop": "false"` · `"no"` · `"0"` 처럼 글자로 보낸다.
 * `!!'false'` 는 **참**이다 — 끄지 말라고 적어 보낸 부름이 도는 서버를 껐다.
 * 위 머리말이 없애겠다고 적어 둔 「시킨 적 없는 일을 조용히 하는」 고장이,
 * 이름(`action`)이 아니라 **값** 쪽에 그대로 남아 있던 자리다.
 *
 * 그래서 아는 꼴은 뜻대로 읽고, 모르는 꼴(`"아마도"`)은 **거짓으로 두고 말한다.**
 * 모르는 값으로 끄는 쪽이 훨씬 나쁘다 — 끈 것은 못 되돌리고, 안 끈 것은
 * 다음 걸음에서 고쳐 부르면 된다.
 */
const 참인말 = new Set(['true', '1', 'yes', 'y', 'on', '참', '예', '네']);
const 거짓인말 = new Set(['false', '0', 'no', 'n', 'off', 'none', 'null', '거짓', '아니오', '아니요', '']);

/** @returns {{값: boolean, 모름?: string}} */
function 참거짓(v) {
  if (v == null) return { 값: false };
  if (typeof v === 'boolean') return { 값: v };
  // 숫자는 0 만 거짓이다. NaN 은 「무엇을 보냈는지」 를 적어 주는 쪽으로 보낸다.
  if (typeof v === 'number') return Number.isFinite(v) ? { 값: v !== 0 } : { 값: false, 모름: String(v) };
  const s = String(v).trim().toLowerCase();
  if (참인말.has(s)) return { 값: true };
  if (거짓인말.has(s)) return { 값: false };
  return { 값: false, 모름: String(v) };
}

/**
 * 준 인자에서 아는 것만 골라낸다.
 *
 * `못읽은값` 은 `[받은 칸 이름, 받은 값]` 들이다 — 참·거짓 자리에 참도 거짓도
 * 아닌 것이 왔을 때. 부르는 쪽이 **그 칸 이름 그대로** 적어 줘야 모델이 무엇을
 * 고쳐 부를지 안다(`kill` 로 보냈는데 `stop` 을 받았다고 적으면 못 고친다).
 *
 * @returns {{날번호: any, 번호: number|null, 끝내기: boolean, 처음부터: boolean,
 *            못읽은값: [string, string][], 준것: string[], 모르는것: string[]}}
 */
export function 일감인자(args) {
  const 준것 = Object.keys(args ?? {});
  const 하나씩 = (이름) => {
    for (const n of 인자이름[이름]) if (args?.[n] != null) return { 칸: n, 값: args[n] };
    return { 칸: null, 값: null };
  };
  const 날번호 = 하나씩('번호').값;
  const 못읽은값 = [];
  const 참거짓칸 = (이름) => {
    const { 칸, 값 } = 하나씩(이름);
    const r = 참거짓(값);
    if (r.모름 !== undefined) 못읽은값.push([칸 ?? 이름, r.모름]);
    return r.값;
  };
  return {
    // 날것을 같이 내놓는다 — 숫자가 아닐 때 **무엇을 받았는지** 적어야 한다.
    날번호,
    /*
     * 숫자도 숫자 글자도 아닌 것은 **숫자로 안 읽는다.** 날 `Number()` 였을 때
     * 자바스크립트가 `true` 를 1 로, `[]` 를 0 으로 조용히 바꿔서 `Jobs({job:true,
     * stop:true})` 가 1번 일감을 끊었다 — 모델이 가리킨 적도 없는 서버를. 위 참거짓()
     * 이 stop·처음부터 에 이미 세워 둔 문턱이고, 번호 칸만 그 밖에 있었다. NaN 으로
     * 두면 아래 「번호는 숫자여야 합니다 (받은 것: …)」 갈래가 받아 준다.
     */
    번호: 날번호 == null ? null
      : (typeof 날번호 === 'number' || typeof 날번호 === 'string' ? Number(날번호) : NaN),
    끝내기: 참거짓칸('끝내기'),
    처음부터: 참거짓칸('처음부터'),
    못읽은값,
    준것,
...
export const JOBS_TOOL = {
  ...
  async run(args) {
    ...
    /*
     * 몇 개만 못 알아들은 판도 **말해 준다.**
     *
     * 위 갈래는 「하나도」 못 알아들었을 때만 걸린다. 그래서
     * `{job: 1, action: 'stop'}` 은 걸리지 않고 — `action` 을 조용히 버린 뒤
     * 출력만 읽어 **성공으로** 돌려줬다. 끄라고 시킨 모델은 껐다고 여기고
     * 넘어가는데 서버는 그대로 돈다. 위 머리말이 없애겠다고 적어 둔 바로 그
     * 고장이, 인자를 하나 더 붙인 판에 그대로 남아 있었다 (34차 리뷰).
     *
     * 오류로 막지는 않는다 — 알아들은 것은 하라고 시킨 것이 맞다. 대신 버린
     * 이름을 적어 보내서, 뜻이 거기 있었으면 다음 걸음에서 고쳐 부르게 한다.
     */
    const 버린말 = 받은것.모르는것.length
      ? `\n(못 알아들어 버린 인자: ${받은것.모르는것.join(', ')} — Jobs 는 job · stop · from_start 만 받습니다.`
        + ' 끝내려던 것이면 stop: true 로 다시 불러라.)'
      : '';
    /*
     * 이름은 아는데 **값**이 참도 거짓도 아닌 판. 같은 자세로 말해 준다.
     *
     * 끄지는 않는다(일감인자 머리말) — 모르는 값으로 끈 것은 못 되돌리고,
     * 안 끈 것은 다음 걸음에서 고쳐 부르면 된다. 대신 받은 칸 이름과 받은
     * 값을 그대로 적어, 뜻이 거기 있었으면 고쳐 부르게 한다.
     */
    const 못읽은말 = 받은것.못읽은값.length
      ? `\n(참·거짓으로 못 읽은 값: ${받은것.못읽은값.map(([칸, 값]) => `${칸}=${JSON.stringify(값)}`).join(', ')}`
        + ' — true·false 로 보내세요. 끝내려던 것이면 stop: true 로 다시 불러라.)'
      : '';
    const 번호 = 받은것.번호;
    ...
    if (번호 != null && !Number.isFinite(번호)) {
      /*
       * 받은 것을 **실제로 읽은 그 칸**에서 적는다.
       *
       * 여기가 `args?.번호` 였다. 설명서에 실린 이름은 `job` 이라 모델은 거의
       * 늘 `job` 으로 보내는데, 그러면 이 말은 `(받은 것: undefined)` 가 된다.
       * 무엇을 잘못 보냈는지 알려 주려고 있는 줄이 정작 그것만 안 알려 줬다.
       */
      return { error: `번호는 숫자여야 합니다 (받은 것: ${JSON.stringify(받은것.날번호)}). 번호 없이 Jobs 를 불러 목록을 보세요.` };
    }
    ...
    if (번호 == null) {
       ...
       * 목록이 비어도 **하던 말은 한다.**
       *
       * 여기가 그 자리에서 곧장 돌아서느라 버린말·못읽은말 이 통째로 빠졌다.
       * 위 두 머리말이 「몇 개만 못 알아들은 판도 말해 준다」 고 적어 두었는데,
       * 하필 **일감이 하나도 없을 때**만 그 약속이 안 지켜졌다. 일감이 없다는
       * 것은 대개 「끄려던 것이 이미 없다」 거나 「아직 안 띄웠다」 인데, 그때
       * 오타를 안 알려 주면 모델은 제 인자가 맞는 줄 알고 그대로 다시 부른다.
       */
      if (!ls.length) return { content: '뒤에서 도는 명령이 없습니다.' + 버린말 + 못읽은말, summary: 세말('count', 0) };
      const 줄들 = ls.map((j) => {
        const 상 = j.상태 === '도는중'
          ? `도는중 ${j.초}초${j.셸끝남 ? ' (셸은 끝남 — 셸이 뒤로 띄운 것이 아직 출력을 물고 돕니다)' : ''}`
          : j.시그널 ? `${j.시그널} 로 죽음` : `끝남 (종료코드 ${j.종료코드})`;
      ...
      return { content: 줄들.join('\n') + 버린말 + 못읽은말, summary: 세말('count', ls.length), 일감수: ls.length };
    }
    ...
    if (받은것.끝내기) {
      ...
      if (r.이미) {
        // 안 읽은 마지막 출력을 같이 준다 — 지우고 나면 다시 읽을 길이 없다 (끝내기 머리말 · 사냥5 L2).
        return {
          content: `${번호}번은 이미 끝나 있었습니다 (${끝난말(r)}): ${r.명령}`
            + (r.남은 ? `\n\n안 읽은 마지막 출력:\n${뒤만(r.남은, 2000).글}` : '') + 버린말 + 못읽은말,
          summary: 말('sum.jobEndedAlready'),
        };
      }
      /*
      ...
      if (r.안죽음) {
        /*
         * 셸은 끝났는데 셸이 뒤로 띄운 것이 남은 판은 따로 말한다 (사냥5 M2). 한 번 더 시켜도
         * 윈도우에서는 같은 결과라, 「다시 시켜 보세요」 만 적으면 헛걸음을 시킨다.
         */
        const 뒤에남은말 = !r.뒤에남음 ? '\n죽이라고 시켰는데 아직 살아 있습니다. 목록에 그대로 두었으니 한 번 더 시켜 보세요.'
          : process.platform === 'win32'
            ? '\n셸은 끝났지만 셸이 뒤로 띄운 프로세스가 아직 출력을 물고 돕니다. 윈도우에서는 셸이 먼저 끝나면'
              + ' 그 아래를 찾아 끌 길이 없습니다 — 작업 관리자에서 찾아 끄세요. 출력은 계속 읽을 수 있게 목록에 남겨 두었습니다.'
            : '\n셸은 끝났지만 셸이 뒤로 띄운 프로세스가 아직 출력을 물고 돕니다. 무리째 끊으라고 했는데 아직 살아 있습니다.'
              + ' 목록에 그대로 두었으니 한 번 더 시켜 보세요.';
        return {
