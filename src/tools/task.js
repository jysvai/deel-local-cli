 ...
 */
export function 하위요약({ 목적, 모드, 끝, 글자수, 모델 = null, 에이전트 = null, 보인이름 = (p) => p }) {
  const 줄 = [];
  const w = workMode(모드);
  const 왜끝났나 = {
    done: '끝냄',
   ...
   */
  /*
   * 어느 정의가 한 일인지도 적는다 (agent/agents.js).
   *
   * 모델 이름을 적는 것과 같은 까닭이다. 부모는 이 요약만 보고 이어서 일하는데,
   * 「리뷰어가 봤다」 와 「그냥 하위가 봤다」 는 믿을 만한 정도가 다르다.
   */
  줄.push(`하위 작업 "${목적}"${에이전트 ? ` · ${에이전트}` : ''} · ${w.name} 모드${모델 ? ` · ${모델}` : ''} · ${끝?.steps ?? 0}걸음 · ${왜끝났나}`);
