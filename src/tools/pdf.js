  ...
  묶음펼치기() {
    ...
    for (const 번호 of 볼것) {
      ...
      for (let k = 0; k < N; k += 1) {
        ...
        /*
         * 최신 xref 가 「이 객체는 파일에 낱개로 있다」 고 정해 준 것은
         * 묶음이 덮지 못한다.
         *
         * 눌린 PDF(1.5+)를 고쳐 저장하면 고친 객체만 **낱개로** 뒤에
         * 덧붙는다. 그때 옛 묶음도 파일에 그대로 남아 있어서, 묶음을 펼쳐
         * 덮어 버리면 아래 풀기() 가 묶음 쪽을 먼저 보므로 **고치기 전 값을
         * 읽는다.** 자리만 지켜 놓고 여기를 안 지키면 반쪽만 고친 것이다.
         */
        if (this.xref정함.has(객체번호)) continue;
 ...
 *
 * 넷씩 끊어 읽고, 모자란 자리는 **맨 앞**을 0 으로 메운다. 뒤를 메우면
 * `<41>`('A') 가 `4100` — 곧 「䄀」 이라는 엉뚱한 한자가 된다. 두 자리로 적는
 * 파일이 규격 위반이긴 하지만, 위반을 만났을 때 없는 한자를 만들어 내는 것과
 * 「A」 를 돌려주는 것 중에 무엇이 나은지는 분명하다.
 *
 * 메우는 자리가 **맨 앞 한 곳**이어야 한다. 뒤쪽 토막만 메우면 `<410042>` 가
 * `4100`·`0042` 로 끊겨 첫 글자가 다시 「䄀」 이 된다 — 고친 자리가 도로
 * 살아난다.
 */
function 열여섯글자(글) {
  const s = String(글 ?? '').replace(/[^0-9A-Fa-f]/g, '');
  if (!s) return '';
  const 남 = s.length % 4;
  const 짝 = 남 ? s.padStart(s.length + (4 - 남), '0') : s;
  let 나온것 = '';
