...
export function html보기(글, { 있는파일 = () => true } = {}) {
   ...
   *
   * 닫는 태그 무늬는 아래 참조 훑기와 **같은 잣대**여야 한다. 한쪽만 넓히면
   * 같은 글을 두 자리가 다르게 읽는다 — 19회차에 아래만 고치고 여기를 지나칠
   * 뻔했다. 한 자리를 고칠 때 같은 파일에 같은 병이 또 있나를 먼저 본다.
   */
  const 뼈 = String(글)
    .replace(/<!--[\s\S]*?-->/g, '')
  ...
  const 참조볼글 = String(글)
    .replace(/<!--[\s\S]*?-->/g, '')
    /*
     * 닫는 태그에 속성이 붙어도 브라우저는 닫는 것으로 본다 — webfetch.js 와 같은 잣대.
     *
     * 그런데 그 닫는 태그를 **그대로 남기면** 안 된다. `</script src="x.js">`
     * 의 src 가 아래 참조 훑기에 걸려 「x.js 가 없습니다」 라는 **거짓 탈**이
     * 된다(2차 눈이 재현해 줬다). 여는 태그는 진짜 참조라 남기고, 닫는
     * 태그는 이름만 남겨 껍데기로 적는다.
     */
