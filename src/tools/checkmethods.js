 ...
 * ── 복수형은 test·spec 만 ───────────────────────────────────────────────
 *
 * 8차 리뷰가 물었다 — `checks-roles.mjs` · `benches-roles.py` 는 왜 안 되나.
 * 안 되는 것이 맞다. `tests-` · `specs-` 는 실제로 쓰는 이름이고
 * (`tests-integration.mjs`), `checks-` · `benches-` 는 아니다. 없는 것을
 * 늘리면 모델이 없는 명령을 부르고, 실패를 받고, 또 부른다.
 *
 * 앞 판 검사 주석이 「앞쪽 검사말도 복수형을 받는다」 고 뭉뚱그려 적어서
 * 넷 다 되는 것처럼 읽혔다. 되는 둘과 안 되는 둘을 검사에 나란히 못박아 뒀다.
 *
...
/*
 * ── 여기서 만든 글은 그대로 **셸 명령**이 된다 ──────────────────────────
 *
 * tools/verify.js 가 이 목록을 「전부 Bash 로 돌려라」 와 함께 모델에게 붙인다.
 * 그런데 파일 이름을 정하는 것은 우리가 아니라 **그 저장소**다. 남의 저장소를
 * 열어 보는 것이 이 프로그램의 일이니, 거기 적힌 이름은 믿을 글이 아니다.
 *
 *   sh scripts/test-a;echo x.sh     명령이 둘이 된다
 *   sh scripts/qa-b$(id).sh         sh 가 뜨기도 전에 돈다
 *   node test/my unit.test.js       빈칸 하나로 엉뚱한 파일을 찾다 죽는다
 *
 * 그래서 감싸면 한 낱말로 남는 것은 겹따옴표로 감싸고, 감싸도 안 되는 것
 * (겹따옴표·백틱·$·역슬래시·줄바꿈)은 **아예 안 알려 준다.** 홑따옴표는
 * 겹따옴표 안에서 그냥 글자라 감싸는 쪽으로 남긴다 — 막을 까닭이 없는 것을
 * 막으면 그것도 없는 것을 말하는 셈이다.
 */
const 감싸도새는글자 = /["`$\\\r\n]/;
const 그냥써도되는길 = /^[A-Za-z0-9._\-/¡-￿]+$/;
function 셸에실을길(길) {
  if (감싸도새는글자.test(길)) return null;
  return 그냥써도되는길.test(길) ? 길 : `"${길}"`;
}
