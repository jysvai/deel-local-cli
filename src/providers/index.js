...
/*
 * ── 붙여 넣은 열쇠의 겉을 다듬는다 (2.0.0 6회차 · Gemini 제공자6) ──────────
 *
 * 앞뒤 빈칸, 그리고 **양끝이 같은** 따옴표 한 쌍을 뗀다. `.env` 의 `OPENAI_API_KEY="sk-…"`
 * 에서 값을 따옴표째 긁어 오는 일이 흔한데, 그러면 앞머리 무늬(`^sk-`)가 하나도 안 맞아
 * 어디 열쇠인지 모른다고 했고, 골라서 넘어가도 따옴표째 저장돼 첫 요청이 401 이었다 —
 * 화면은 「열쇠가 틀렸거나 만료됐다」 라고 하니 사람은 멀쩡한 열쇠를 새로 발급받는다.
 * 열쇠 글자에는 따옴표가 안 들어가므로 양끝 한 쌍만 뗀다. 한쪽만 있거나 짝이 안 맞으면
 * 손대지 않는다 — 무엇이 잘못 붙었는지 짐작해 고치지 않는다.
 */
export function 열쇠다듬기(열쇠) {
