...
export function 인증서설정(prof) {
   ...
   */
  /*
   * 개인키만 적은 것도 **버리지 않는다** (사냥5 B5-09). 여기서 null 로 돌리면 사람이 적은
   * key 가 조용히 사라지고, 게이트웨이는 「인증서를 내라」 는 악수 실패만 준다. 들고 가서
   * 인증서찾기 가 「반쪽이다」 라고 말하게 한다.
   */
  if (!편것.cert && !편것.pfx && !편것.ca && !편것.key) return null;
  return 편것;
}
...
export function 인증서찾기(주소) {
  ...
  if (!설정) return null;
  /*
   * 반쪽만 적었으면 붙기 전에 말한다 (사냥5 B5-09).
   *
   * 인증서만 적고 개인키를 안 적으면 Node 는 인증서를 **아예 안 낸다.** 게이트웨이는
   * 「인증서를 내라」 는 경보로 끊고, 그 화면으로는 무엇이 빠졌는지 알 길이 없다. 개인키만
   * 적은 것도 같다. pfx 는 한 덩이에 둘 다 들었으므로 따지지 않는다. 경로가 틀린 것과 같은
   * 「설정을 고칠 일」 이라 같은 코드로 던진다 — 화면(http.js normalizeError)이 말을 그대로 싣고,
   * 다시 불러 봐야 소용없다는 것도 같다.
   */
  if (!설정.pfx && !!설정.cert !== !!설정.key) {
    const 있는것 = 설정.cert ? '인증서(cert)' : '개인키(key)';
    const 빠진것 = 설정.cert ? '개인키(key)' : '인증서(cert)';
    throw Object.assign(
      new Error(`클라이언트 ${있는것}만 적혀 있고 ${빠진것}가 없습니다 — 프로필 "인증서" 에 둘을 함께 적거나 pfx 한 덩이로 적으세요`),
      { code: 'CERT_READ' },
    );
  }
  const 옵션 = {};
