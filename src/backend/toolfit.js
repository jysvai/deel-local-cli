...
function 이름다듬기(원래, 쓴것) {
  ...
  if (새것 !== 원본) 새것 = `${새것.replace(/_+$/, '')}_${지문(원본).slice(0, 6)}`;
  if (!새것 || !/[a-zA-Z0-9]/.test(새것)) 새것 = `tool_${지문(원본).slice(0, 6)}`;
  if (새것.length > 이름최대) {
    /*
...
/** 짧은 지문 여덟 자. 암호용이 아니라 이름이 안 겹치게 하려는 것뿐이다. */
function 지문(글) {
  let h = 2166136261;
  const s = String(글);
