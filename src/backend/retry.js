...
//   · ECONNREFUSED 는 서버가 꺼진 것이고, 주소가 틀린 것(ENOTFOUND)은 백 번 물어도
//     틀리다. 둘 다 안 부른다.
//   · **머리말 시계(5분)** 가 끝난 것도 안 부른다. 서버가 받아 놓고 답을 안 한
//     자리라, 5분을 또 기다릴 일이 아니다.
//   · 다만 **아직 붙지도 못한 것**은 부른다 (아래 못붙은코드). 연결 시계는 10초라
//     다시 해 보는 값이 싸고, 소켓이 없었으니 서버에는 아무것도 안 갔다 — 다시
//     불러도 두 벌이 될 것이 없다. 무선이 끊겼다 붙는 동안이 전부 여기다.
//
// 얼마나 기다리나:
 ...
 * ── 아직 **붙지도 못한** 것 ─────────────────────────────────────────────
 *
 * 위의 것들은 붙었다가 끊긴 자리다. 여기는 소켓이 아예 안 생긴 자리 —
 * 손이 닿기 전이라 서버에는 아무것도 안 갔다. 그래서 **다시 부르는 것이
 * 제일 안전한 종류**다. 두 벌이 될 답도, 반쯤 흘러간 글자도 없다.
 *
 * 왜 따로 두나: 이 파일은 여태 「시간 초과는 다시 안 부른다」 였고, 그건
 * **머리말 시계(5분)** 를 두고 한 말로는 옳았다. 5분을 기다렸다가 또 5분을
 * 기다리는 것은 사람을 붙드는 짓이다.
 *
 * 그런데 그 규칙이 **붙지도 못한 것**까지 같이 덮고 있었다. 연결 시계는
 * 10초다(undici 의 UND_ERR_CONNECT_TIMEOUT · backend/http.js 의 연결기본).
 * 10초 만에 못 붙은 것을 두 번 더 해 보는 값은 1초·2초 기다림이지 5분이
 * 아니다. 그런데도 여태 턴이 통째로 죽었다 — 사람은 같은 말을 다시 치고,
 * 그 턴에 읽어 둔 것은 날아갔다.
 *
 * 무선이 잠깐 끊기거나, 노트북이 깨어나거나, VPN 이 다시 붙는 동안이 전부
 * 여기다. 몇 초 뒤에는 되는 것들이라 **한 세션이 거기서 끝날 이유가 없다.**
 *
 * `ENOTFOUND` 는 여기 없다. 이름이 틀린 것은 백 번 물어도 틀리다. 반대로
 * `EAI_AGAIN` 은 규격이 「지금은 못 하겠다, 다시 물어라」 라고 정해 둔
 * 코드라 여기 있다 — 이름이 같아도 뜻이 정반대다.
 */
const 못붙은코드 = new Set([
  'UND_ERR_CONNECT_TIMEOUT',   // undici 가 10초 안에 소켓을 못 얻음
  'ETIMEDOUT',                 // OS 가 TCP 연결에서 손을 뗌
  'EAI_AGAIN',                 // 이름 풀이가 **잠깐** 안 됨 (ENOTFOUND 와 다르다)
  'ENETUNREACH',               // 망이 잠깐 없음 (깨어나는 중·VPN 이 붙는 중)
  'EHOSTUNREACH',
  'ENETDOWN',
  'ENETRESET',
  'CONNECT_TIMEOUT',           // 우리가 노드 길에서 붙인 이름 (backend/http.js)
]);
...
export function 다시부를까({ status = 0, code = null, attempt = 1 } = {}, 정책 = 기본정책()) {
  ...
  const c = String(code ?? '');
  return !!code && (다시부를코드.has(c) || 못붙은코드.has(c));
}

/** 아직 붙지도 못한 실패인가. 화면이 무슨 말을 할지 여기서 갈린다. */
export function 못붙은것인가(code) {
  return 못붙은코드.has(String(code ?? ''));
}
