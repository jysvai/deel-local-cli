...
export function 전선말(카드) {
  ...
  /*
   * ── 짐작한 것과 **배운 것**을 갈라 적는다 ──────────────────────────────
   *
   * 여태 이 줄은 둘을 똑같이 보여 줬다. 그러면 「이 모델은 생각을 안 한다」
   * 와 「3주 전 400 한 번을 우리가 그렇게 읽었다」 가 화면에서 구별이 안
   * 된다. 뒤쪽은 **틀릴 수 있는 것**이고(이 파일의 배울전선 머리말), 틀렸을
   * 때 사람이 알아볼 유일한 단서가 이 표다.
   *
   * 칸 이름을 새로 옮기지 않는다 — 위에서 이미 옮긴 이름표를 그대로 쓴다.
   * 그래야 말 하나를 고칠 때 두 자리가 갈라지지 않는다.
   */
  const 배운것 = new Set(카드.배운칸 ?? []);
  const 배운이름 = [
    ['생각형식', 말('wire.think')],
    ['눈금', 말('wire.rungs')],
    ['캐시', 말('wire.cache')],
  ].filter(([칸]) => 배운것.has(칸)).map(([, 이름]) => 이름);
  if (배운이름.length) 조각.push(`${말('wire.learned')} ${배운이름.join('·')}`);
  return 조각.join(' · ');
}
