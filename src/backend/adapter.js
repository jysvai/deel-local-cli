 ...
 * ── 이 요청이 **어느 자리로 갔나** ──────────────────────────────────────
 *
 * 게이트웨이 뒤에는 대개 같은 모델이 여러 자리에 올라가 있다(리전 여럿,
 * 열쇠 여럿). 그런데 **프리픽스 캐시는 자리마다 따로다.** 그래서 어제까지
 * 92% 로 읽히던 대화가, 다음 요청이 다른 자리로 가는 순간 0% 가 된다 —
 * 우리가 보낸 것은 한 글자도 안 달라졌는데.
 *
 * 실제로 그 줄을 봤다. 같은 122.4K 프리픽스가 3분 41초 만에 다시 나갔고,
 * 그 한 번이 그날 제일 비싼 요청이었다. 화면에는 아무 말도 안 났다.
 *
 * 그래서 **서버가 말해 줄 때만** 적어 둔다. LiteLLM 은 어느 자리가 받았는지
 * 를 응답 머리로 알려 준다(`x-litellm-model-id`) — 이 값이 턴 사이에 바뀌면
 * 캐시가 끊겼다는 뜻이고, 그건 짐작이 아니라 잰 것이다.
 *
 * 없으면 아무 말도 안 한다. 모르는 머리는 그냥 없는 것이고, 없는 것을 두고
 * 「자리가 안 바뀌었다」 고 말하지도 않는다.
 *
 * **값은 화면에 안 찍는다.** 이 값은 사내 배포 이름이거나 리전 주소일 수
 * 있다. 우리가 쓸 것은 「바뀌었나」 하나뿐이라, 바뀌었다는 사실만 올린다.
 */
/*
 * ── 왜 이름이 하나뿐인가 ────────────────────────────────────────────────
 *
 * 세션 이름을 **내보낼** 때는 아는 이름을 다 단다(위 세션머리). 머리는 모르면
 * 무시되니 이름이 늘어도 잃을 것이 없어서다. 여기는 반대다 — **받아서 읽는**
 * 자리라, 이름 하나를 잘못 고르면 없는 일을 있다고 말하게 된다.
 *
 * 그래서 「어느 자리가 받았나」 를 뜻한다고 **문서에 적힌** 이름만 읽는다.
 * 그럴듯한 일반형들을 같이 읽어 봤다가 도로 뺐다:
 *
 *   x-deployment-id       일반 웹·컨테이너 인프라(배포 해시)가 같은 이름을
 *                         쓴다. 모델과 아무 상관 없이 배포할 때마다 바뀌므로,
 *                         멀쩡한 캐시를 두고 「자리가 바뀌었다」 를 찍는다.
 *   x-ms-deployment-name  Azure 의 **논리** 배포 이름이라 물리 자리가 바뀌어도
 *                         그대로다. 읽어 봐야 알아내는 것이 없고, 있는데 없다고
 *                         말하는 쪽이라 더 나쁘다.
 *
 * 모르면 아무 말도 안 하는 편이 낫다. 안 뜨는 경고는 사람이 다른 데를 보게
 * 하지 않지만, 틀린 경고는 진짜 경고까지 안 읽게 만든다.
 */
const 자리머리들 = ['x-litellm-model-id'];

export function 간자리(머리) {
  if (!머리?.get) return null;
  for (const 이름 of 자리머리들) {
    const v = 머리.get(이름);
